...
def _desktop_python() -> Optional[str]:
    ...
    path = os.environ.get("AIARB_DESKTOP_PY_RUNTIME", "").strip()
...
def _ensure_plugin_site_on_path() -> None:
    ...
    os.environ["AIARB_PLUGIN_SITE"] = site_dir
...
class PluginLoader:
    ...
    @staticmethod
    def _is_builtin_path(source_path: Path) -> bool:
        """Return True if *source_path* is inside the built-in plugins dir."""
        try:
            from ..config.utils import get_builtin_plugins_dir

            builtin_dir = get_builtin_plugins_dir()
            return source_path.resolve().is_relative_to(
                builtin_dir.resolve(),
            )
        except Exception:
            return False
    ...
    def discover_plugins(self) -> List[Tuple[PluginManifest, Path]]:
        ...
        When the same plugin id appears in multiple directories (e.g. a
        built-in plugin overridden by a user-installed copy), the last
        directory in ``plugin_dirs`` wins.  This lets users override
        built-in plugins by installing a newer version into their own
        plugins directory.

        Returns:
            List of (manifest, plugin_dir) tuples
        """
        # Ordered dict keyed by plugin id; later entries replace earlier
        # ones so the last plugin_dir in self.plugin_dirs takes precedence.
        discovered_map: Dict[str, Tuple[PluginManifest, Path]] = {}
        ...
        for plugin_dir in self.plugin_dirs:
            ...
            for item in plugin_dir.iterdir():
                ...
                try:
                    ...
                    discovered_map[manifest.id] = (manifest, item)
    ...
    def _check_version_compatibility(
        ...
        """Check plugin compatibility with current AIArb version.

        Uses left-closed, right-open semantics: ``>=min, <max``.
        When ``aiarb_version`` is absent, falls back to legacy
    ...
    async def _load_backend_module(
        ...
        try:
            ...
            if manifest.aiarb_version is not None:
                qv_dict = manifest.aiarb_version.model_dump()
            else:
                qv_dict = {
    ...
    async def _load_plugin_unlocked(
        ...
        if not compatible:
            ...
            record = PluginRecord(
                ...
                builtin=self._is_builtin_path(source_path),
            )
    ...
    def _install_requirements(
        ...
        the case for uv-managed venvs created by the AIArb script
    ...
    def _install_requirements_frozen(
        ...
        if python is None:
            raise RuntimeError(
                ...
                "(AIARB_DESKTOP_PY_RUNTIME not set). Reinstall AIArb "
    ...
    async def unload_plugin(
        ...
        removes the plugin's tools from ``aiarb.agents.tools``.
    ...
    def _cleanup_plugin_tools(
        ...
        try:
            ...
            tools_module = sys.modules.get("aiarb.agents.tools")
