...
class AIArbVersionConstraint(BaseModel):
    """AIArb version compatibility range (left-closed, right-open).
...
class PluginManifest(BaseModel):
    ...
    """Localised name mapping (e.g. ``{"zh-CN": "看板", "en-US": "Kanban"}``).

    Populated automatically when ``name`` in ``plugin.json`` is a dict
    rather than a plain string.  The frontend uses this to render the
    plugin name in the user's selected language.
    """

    description_i18n: Optional[Dict[str, str]] = None
    """Localised description mapping, same semantics as ``name_i18n``."""
    ...
    aiarb_version: Optional[AIArbVersionConstraint] = None
    ...
    def _normalise_input(cls, data: Any) -> Any:
        ...
        # Preserve localised name/description dicts as ``*_i18n`` before
        # coercing the display string.  This lets the API pass the full
        # mapping to the frontend so it can pick the right language.
        for key in ("name", "description"):
            i18n_key = f"{key}_i18n"
            val = data.get(key)
            if isinstance(val, dict) and i18n_key not in data:
                data[i18n_key] = val
...
class PluginRecord:
    ...
    builtin: bool = False
    """True when the plugin ships inside the aiarb package and cannot be
