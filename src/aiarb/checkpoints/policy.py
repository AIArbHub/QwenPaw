...
GIT_REQUIRED_MESSAGE = (
    "AIArb checkpoints require Git, but the git executable was not found on "
    "PATH. Install Git from https://git-scm.com/downloads, ensure `git` is "
    "available in your terminal, and then restart AIArb."
)
...
# Other AIArb-owned paths are runtime implementation details. Keep this
...
AIARB_RUNTIME_STATE_FILES = frozenset(
    {
        ".bootstrap_completed",
        ".reme_store_v1",
        ".skill.json.lock",
...
AIARB_STATE_SUFFIXES = (
    ".json.tmp",
    ".lock",
    ".weixin-migrate.bak",
)

# Public aggregate constants answer "is this AIArb-owned state?". They
# deliberately include checkpoint-owned session and memory paths.
AIARB_STATE_FILES = frozenset(
    {*CHECKPOINT_STATE_FILES, *AIARB_RUNTIME_STATE_FILES},
)
AIARB_STATE_DIRS = (
    *CHECKPOINT_STATE_DIRS,
    *AIARB_RUNTIME_STATE_DIRS,
...
# categories. These are not AIArb state, but checkpointing them would be
...
EXCLUDE_PATTERNS = (
    ".git/",
    *DEVELOPMENT_ARTIFACT_PATTERNS,
    *(f"/{name}" for name in sorted(AIARB_RUNTIME_STATE_FILES)),
    *(f"/{name}" for name in AIARB_RUNTIME_STATE_DIRS),
    *(f"/*{suffix}" for suffix in AIARB_STATE_SUFFIXES),
)
...
def is_aiarb_state_path(rel: str) -> bool:
    """Return whether *rel* is AIArb runtime state, not user content."""
    normalized = (rel or "").replace("\\", "/").lstrip("/")
    if not normalized:
        return False
    if "/" not in normalized and normalized in AIARB_STATE_FILES:
        return True
    if "/" not in normalized and normalized.endswith(AIARB_STATE_SUFFIXES):
        return True
    return any(normalized.startswith(prefix) for prefix in AIARB_STATE_DIRS)
...
def session_file_path(
    ...
    *,
    ...
    """Return the AIArb 2.0 JSON session path for one session."""
    if not session_id:
...
def session_key(*, channel: str, user_id: str, session_id: str) -> str:
    """Return a bounded, collision-resistant key for a AIArb session."""
...
def latest_user_query(
    ...
    *,
    ...
    """Return the latest persisted user text for one AIArb 2.0 session."""
