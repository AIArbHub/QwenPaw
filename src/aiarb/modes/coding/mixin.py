...
The internal AIArb workspace (configs, sessions, memory) is at:
...
class CodingModeMixin:
    ...
    At runtime this class is mixed into ``AIArbAgent`` and combined
    with ``Agent`` via MRO. Coding Mode prompt injection is handled by
    :class:`~aiarb.runtime.prompt_contributors.CodingModeContributor`.
    """
    ...
    # ------------------------------------------------------------------
    # Tool registration hook (called from AIArbAgent._create_toolkit)
    # ------------------------------------------------------------------
