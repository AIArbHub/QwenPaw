...
from aiarb.tauri.env import (
    DESKTOP_APP_ENV,
    DESKTOP_CORS_ORIGINS_ENV,
    DESKTOP_READY_PREFIX,
    ensure_desktop_cors_origins,
)
from aiarb.tauri.sidecar_logging import install_sidecar_logging
...
def _bundled_python() -> str:
    ...
    python = (os.environ.get("AIARB_DESKTOP_PY_RUNTIME") or "").strip()
...
def _child_env_with_plugin_site(env: "dict | None") -> "dict | None":
    ...
    """
    site_dir = (os.environ.get("AIARB_PLUGIN_SITE") or "").strip()
    if not site_dir:
        return env
...
def _reexec_as_bundled_python(args: Sequence[str]) -> None:
    ...
    plugin-installed deps (``AIARB_PLUGIN_SITE``) importable.
    """
    python = _bundled_python()
    if not python:
        print(
            "aiarb-backend is the desktop backend, not a Python "
...
def _install_subprocess_guard() -> None:
    ...
    * On Windows, suppress console windows for child processes that don't pass
    ...
    if getattr(subprocess.Popen, "_aiarb_guarded", False):
        return
    ...
    setattr(subprocess.Popen, "_aiarb_guarded", True)


def _ensure_aiarb_app_not_loaded() -> None:
    if "aiarb.app._app" in sys.modules:
        raise RuntimeError(
            "aiarb app imported before desktop CORS origins were set",
        )


def _sync_loaded_aiarb_constant_cors_origins() -> None:
    constant_module = sys.modules.get("aiarb.constant")
...
def _install_desktop_runtime() -> None:
    ...
    # from aiarb.constant.CORS_ORIGINS at import time.
    _ensure_aiarb_app_not_loaded()
    ensure_desktop_cors_origins()
    _sync_loaded_aiarb_constant_cors_origins()
...
def _run_backend_server(log_level: str) -> None:
    ...
    from aiarb.config.utils import write_last_api
    from aiarb.constant import LOG_LEVEL_ENV, WORKING_DIR
    from aiarb.utils.logging import (
        SuppressPathAccessLogFilter,
        setup_logger,
    )
    from aiarb.utils.port import get_stable_port, write_port_file
    ...
    os.environ.pop("AIARB_RELOAD_MODE", None)
    ...
    from aiarb.tauri.backend_guard import reconcile_singleton_backend

    reconcile_singleton_backend(WORKING_DIR)
    if normalized_log_level in ("debug", "trace"):
        from aiarb.cli.main import log_init_timings
    ...
    from aiarb.app._app import app as fastapi_app
...
def main() -> None:
    ...
    if not config_path.exists():
        from aiarb.cli.init_cmd import init_cmd
    ...
    # the same guard in cli/app_cmd.py for `aiarb app`).
    from aiarb.utils.platform import auto_disable_sandbox_on_windows
