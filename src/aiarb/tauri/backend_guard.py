...
by the previous launch (verified to actually be a AIArb backend, to guard
...
def _looks_like_backend(proc: psutil.Process) -> bool:
    """Best-effort check that *proc* is a AIArb desktop backend.
    ...
    try:
        ...
        name = ""
    if "aiarb-backend" in name:
        return True
    try:
        exe = (proc.exe() or "").lower()
    except (psutil.Error, OSError):
        exe = ""
    if "aiarb-backend" in exe:
        return True
    try:
...
def _terminate_previous_backend(pid_file: Path) -> None:
    ...
    if not _looks_like_backend(proc):
        logger.info(
            "Recorded backend pid %s is not a AIArb backend "
            "(likely PID reuse); leaving it alone",
            pid,
        )
        return

    logger.info("Terminating orphaned AIArb backend pid %s", pid)
    try:
        proc.terminate()
        try:
