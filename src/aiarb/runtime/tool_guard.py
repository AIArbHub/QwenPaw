...
class GuardedFunctionTool:
    """Permission-checked tool wrapper using aiarb's tool-guard engine.

    Routes every tool invocation through
    ``aiarb.security.tool_guard.engine`` which evaluates guard rules and
    ...
    importing this module does not require the framework package to be
    importable at definition time (the runtime package imports the framework
...
async def _guarded_tool_check_permissions(
    ...
    **_extra_kwargs: Any,
    ...
    """Drive aiarb's tool-guard engine + ApprovalService for one tool call.

    Signature matches the framework's
    :meth:`PermissionEngine.check_permission` call site
    (:file:`aiarb/framework/permission/_engine.py:212`):
    ...
    or keyword args the framework might add in future releases without
    breaking us.
    ...
    """
    del context  # aiarb's guard doesn't read PermissionContext yet
    from aiarb.framework.permission import (
        PermissionBehavior,
        PermissionDecision,
    )
