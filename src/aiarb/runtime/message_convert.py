# -*- coding: utf-8 -*-
"""Message conversion between AgentRequest and the framework Msg."""
...
def _request_input_to_msgs(
    ...
    the framework 2.0 ``Msg`` objects.

    Handles text, image, audio, video, and file content blocks.
    """
    try:
        from aiarb.framework.message import Msg, TextBlock, DataBlock
        from aiarb.framework.message import URLSource
    except Exception:
        logger.error(
            "Failed to import aiarb.framework.message; user input will be dropped",
            exc_info=True,
        )
        return []
