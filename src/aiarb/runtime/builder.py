...
:class:`AgentBuilder` fully constructs a :class:`AIArbAgent` for each
request.  It obtains tools from the per-workspace
:class:`AIArbLocalWorkspace` (via ``list_tools``), the system prompt
...
class AgentBuilder:
    ...
    async def build_toolkit(
        ...
        *,
        ...
        :class:`AIArbLocalWorkspace` via ``list_tools()``.
    ...
    async def build(  # pylint: disable=too-many-statements,too-many-branches
        ...
        """Construct a fully-wired :class:`AIArbAgent` for one request.

        Integrates all per-workspace registries: AIArbLocalWorkspace
        ...
        """
        from aiarb.framework.agent import ReActConfig

        from ..agents.react_agent import AIArbAgent
    ...
    async def _collect_driver_tools_and_prompts(
        ...
        from ..drivers.adapters.framework_tool import build_driver_agent_tools
    ...
    def _build_context_config(agent_config: Any) -> Any:
        """Map AIArb's ``ContextCompactConfig`` to AS ``ContextConfig``."""
        ...
        try:
            ...
            # the framework otherwise applies its own 50k-token split afterwards,
            # creates a replacement ToolResultBlock, and drops AIArb's
            # block-scoped truncation metadata.  Make that second cap
            # non-binding while unified pruning is enabled; when pruning is
            # disabled, retain the framework's default safety net.
            tool_result_limit = (
                2**63 - 1
                if trc.enabled
    ...
    def _scroll_recall_runnable(agent_config: Any, governor: Any) -> bool:
        ...
        which requires BOTH the ``AIARB_ALLOW_UNSANDBOXED_RECALL`` env var
    ...
    def _scroll_repl_runnable(agent_config: Any, governor: Any) -> bool:
        ...
        ``AIARB_ALLOW_UNSANDBOXED_RECALL`` env var and
