...
from aiarb.constant import SECRET_DIR, WORKING_DIR
from aiarb.security.secret_store import decrypt, encrypt, is_encrypted
...
def load_envs(
    ...
    try:
        ...
        if isinstance(data, dict):
            ...
            has_corrupted = any(
                is_encrypted(raw[k]) and not decrypted[k]
                for k in raw
            )
            if has_corrupted:
                cleaned = {
                    k: v for k, v in decrypted.items()
                    if v or not is_encrypted(raw.get(k, ""))
                }
                _rewrite_encrypted(path, cleaned)
                return cleaned
            if has_plaintext:
