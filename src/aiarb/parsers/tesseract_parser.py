...
class TesseractParser:
    ...
    def _find_tesseract() -> str:
        ...
        Search order:
        1. Bundled in Tauri desktop app (AIARB_DESKTOP_OCR_TOOLS env var)
        ...
        5. System PATH
        """
        # 1. Tauri desktop app: AIARB_DESKTOP_OCR_TOOLS points to ocr-tools/
        ocr_tools = os.environ.get("AIARB_DESKTOP_OCR_TOOLS", "")
        if ocr_tools:
        ...
        for candidate in [
            ...
            Path(os.environ.get("APPDATA", "")) / "aiarb" / "bin" / "tesseract" / "tesseract.exe",
        ]:
            if candidate.is_file():
                tessdata = candidate.parent / "tessdata"
                if tessdata.is_dir():
    ...
    def _find_poppler() -> str | None:
        ...
        for candidate in [
            ...
            Path(os.environ.get("APPDATA", "")) / "aiarb" / "bin" / "poppler" / "bin",
        ]:
            if candidate.is_dir():
                pdftoppm = candidate / ("pdftoppm.exe" if os.name == "nt" else "pdftoppm")
                if pdftoppm.is_file():
