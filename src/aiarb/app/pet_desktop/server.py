# -*- coding: utf-8 -*-
"""Local HTTP bridge for AIArb lifecycle events."""
...
def _token_required() -> bool:
    """Whether the local bridge enforces ``X-AIArb-Pet-Token``.
    ...
    setting ``AIARB_PET_REQUIRE_TOKEN=0``.
    """
    return os.environ.get("AIARB_PET_REQUIRE_TOKEN", "1") != "0"
...
def build_app(
    ...
    app = FastAPI(title="AIArb Pet Desktop")
    ...
    async def event(
        payload: PetEvent,
        request: Request,
        x_aiarb_pet_token: str | None = Header(default=None),
    ) -> dict[str, Any]:
        _check_token(x_aiarb_pet_token)
