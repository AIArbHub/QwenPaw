...
def _request_meta(runner: Any, request: Any) -> dict[str, Any]:
    return {
        ...
        "agent_name": getattr(runner, "agent_name", "AIArb"),
...
def patch_agent_runner() -> None:
    ...
    _PATCHED = True
    logger.info("AIArb Pet patched Workspace.stream_query")
...
def _wire_existing_channels(patched_fn) -> None:
    ...
    for ws in workspaces.values():
        ...
        for channel in getattr(cm, "channels", []) or []:
            ...
            def _make_proxy(_runner, _fn):
                ...
                _proxy.__qualname__ = "aiarb_pet.patched_channel_process"
                return _proxy
    ...
    if rewired:
        logger.info(
            "AIArb Pet rewired %d channel _process refs to patched "
            "stream_query",
            rewired,
        )
...
def _patch_make_process_factory(patched_fn) -> None:
    ...
    try:
        from aiarb.app.channels import utils as ch_utils
    except Exception:
        logger.debug(
...
def restore_agent_runner() -> None:
    ...
    try:
        ...
        pass
    _PATCHED = False
    logger.info("AIArb Pet restored Workspace.stream_query")
