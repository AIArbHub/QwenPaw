...
async def create_driver_service(
    ...
    from ..approvals.driver_gate import AIArbDriverApprovalGate
    from ..mail.driver_config import (
        is_managed_aiarbmail_card,
        sync_aiarbmail_driver_card,
    )

    # Upgrade legacy aiarbmail cards before DriverManager can launch them.
    ...
    existing_mail_card = (
        ws.workspace_dir / "drivers" / "mcp" / "aiarbmail.yaml"
    )
    ...
    if should_sync_mail_card and not await asyncio.to_thread(
        ...
        ws.workspace_dir,
        mail,
        getattr(ws._config, "backend", "aiarb"),
    ):
        logger.warning(
            "aiarbmail DriverCard could not be synchronized for agent %s; "
