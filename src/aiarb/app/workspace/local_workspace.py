# -*- coding: utf-8 -*-
"""AIArbLocalWorkspace — routes tool management to ToolRegistry.

Subclasses the framework's :class:`LocalWorkspace` so that
:meth:`list_tools` returns AIArb's own tools (managed by
:class:`ToolRegistry`) instead of the framework's built-in six.
...
from aiarb.framework.workspace import LocalWorkspace as FrameworkLocalWorkspace

if TYPE_CHECKING:
    from ...runtime.tool_registry import ToolRegistry


class AIArbLocalWorkspace(FrameworkLocalWorkspace):
    ...
    async def list_tools(  # type: ignore[override]
        ...
        *,
        ...
        """Return AIArb tools, replacing framework built-ins.
