...
def _list_plugins_from_disk() -> list[dict]:
    """Read plugin manifests directly from plugin directories on disk.
    ...
    Scans both the built-in plugins directory and the user plugins
    directory; user plugins override built-in plugins with the same id.
    """
    from ...config.utils import get_all_plugin_dirs, get_builtin_plugins_dir
    ...
    # Keyed by plugin id; later directories (user) override earlier (builtin).
    result_map: dict[str, dict] = {}

    for plugin_dir in plugin_dirs:
        if not plugin_dir.exists():
            continue
        is_builtin = plugin_dir.resolve() == builtin_dir.resolve() or (
        ...
        for item in sorted(plugin_dir.iterdir()):
            ...
            # Extract i18n dicts from raw manifest (may be dicts or strings).
            raw_name = manifest.get("name", plugin_id)
            raw_desc = manifest.get("description", "")
            name_i18n = raw_name if isinstance(raw_name, dict) else None
            desc_i18n = raw_desc if isinstance(raw_desc, dict) else None

            result_map[plugin_id] = {
                "id": plugin_id,
...
@router.get(
    "/market/search",
    summary="Search plugins from AIArb Platform",
)
async def search_market_plugins(
    ...
    """Proxy plugin search to AIArb Platform to avoid CORS."""
    import httpx
