...
def _write_reserved_upload(upload: UploadFile, target: Path) -> int:
    ...
    temporary = target.with_name(
        f".{target.name}.{secrets.token_hex(6)}.aiarb.tmp",
    )
    size = 0
    try:
        upload.file.seek(0)
...
def _filesystem_name_rules(directory: Path) -> tuple[bool, bool]:
    ...
    :mod:`aiarb.services.fs_name_rules`, unchanged in behaviour. It stays
...
def _extract_and_merge_zip(data: bytes, workspace_dir: Path) -> None:
    ...
    tmp_dir = None
    try:
        tmp_dir = Path(tempfile.mkdtemp(prefix="aiarb_upload_"))
        with zipfile.ZipFile(io.BytesIO(data)) as zf:
...
async def download_workspace(request: Request):
    ...
    filename = f"aiarb_workspace_{agent.agent_id}_{timestamp}.zip"
...
@router.post(
    ...
    description=(
        ...
        "the zip are left unchanged (e.g. aiarb.db, runtime dirs). "
