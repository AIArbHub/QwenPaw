...
async def lifespan(  # pylint: disable=too-many-statements,too-many-branches
    ...
    try:
        ...
        message = (
            "AIArb startup failed because restore artifact cleanup did not "
    ...
    try:
        from ..sop.store import ensure_builtin_skills

        ensure_builtin_skills()
    except Exception:  # noqa: BLE001 - SOP init must never block startup
        logger.warning("SOP built-in skills init failed", exc_info=True)
    ...
    async def _background_startup():  # pylint: disable=too-many-statements
        try:
            ...
            # Built-in plugins (shipped with the package) are loaded from
            # BUILTIN_PLUGINS_DIR and cannot be uninstalled.
            ...
            from aiarb.runtime.commands.control import register_command
...
_CONSOLE_STATIC_ENV = "AIARB_CONSOLE_STATIC_DIR"
...
async def post_desktop_shutdown(
    x_aiarb_desktop_shutdown_token: str | None = Header(default=None),
):
    ...
    if (
        ...
        or x_aiarb_desktop_shutdown_token is None
        or not hmac.compare_digest(
            x_aiarb_desktop_shutdown_token,
            expected_token,
        )
    ):
...
if os.path.isdir(_CONSOLE_STATIC_DIR):
    ...
    @app.get(
        "/{full_path:path}",
        name="aiarb_console_spa_catchall",
    )
