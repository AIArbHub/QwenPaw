...
variable ``AIARB_AUTH_ENABLED`` is set to a truthy value (``true``,
...
_RUNTIME_TOKEN_ENV = "AIARB_RUNTIME_INTERNAL_TOKEN"
_RUNTIME_TOKEN_HEADER = "x-aiarb-runtime-token"
...
def is_auth_enabled() -> bool:
    ...
    Returns ``True`` when ``AIARB_AUTH_ENABLED`` is set to a truthy
    ...
    """
    env_flag = EnvVarLoader.get_str("AIARB_AUTH_ENABLED", "").strip().lower()
...
def auto_register_from_env() -> None:
    ...
    Called once during application startup.  If ``AIARB_AUTH_ENABLED``
    is truthy and both ``AIARB_AUTH_USERNAME`` and ``AIARB_AUTH_PASSWORD``
    ...
    username = EnvVarLoader.get_str("AIARB_AUTH_USERNAME", "").strip()
    password = EnvVarLoader.get_str("AIARB_AUTH_PASSWORD", "").strip()
