...
def _token_required() -> bool:
    return os.environ.get("AIARB_TS_REQUIRE_TOKEN", "1") != "0"
...
def build_app(
    ...
    app = FastAPI(title="AIArb Text Selection Desktop")
    ...
    def health() -> dict[str, Any]:
        return {
            "ok": True,
            "service": "aiarb-text-selection-desktop",
    ...
    @app.get("/config")
    def get_ts_config(
        x_aiarb_ts_token: str | None = Header(default=None),
    ) -> dict[str, Any]:
        _check_token(x_aiarb_ts_token)
        return get_config()
