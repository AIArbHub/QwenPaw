...
Lifecycle hooks:
    ts_startup()  — called during AIArb background startup
    ts_shutdown() — called during AIArb graceful shutdown
"""
...
def ts_startup() -> None:
    """Boot the text selection built-in feature at AIArb startup."""
    try:
        ...
        import os
        autostart = os.environ.get("AIARB_TS_AUTOSTART", "0")
...
def ts_shutdown() -> None:
    """Stop the text selection desktop process on AIArb shutdown."""
    try:
