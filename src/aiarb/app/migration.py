...
_DEFAULT_AGENT_DESCRIPTION = "Default AI Arb agent"
...
def _do_migrate_legacy_workspace() -> bool:
    ...
    default_agent_config = AgentProfileConfig(
        id="default",
        name="Default Agent",
        description="Default AI Arb agent (migrated from legacy config)",
...
def _apply_legacy_qa_disable_for_migration(config) -> None:
    ...
    if getattr(ref, "enabled", True):
        ...
        logger.info(
            ...
            "(new AIArb builtin QA slot was created)",
            legacy_id,
        )
...
def _copy_builtin_persona_files(agent_id: str, workspace_dir: Path) -> None:
    """Copy bundled persona MD files from aiarb.builtins.<agent_id>.
    ...
    pkg_name = f"aiarb.builtins.{agent_id}"
    try:
...
def ensure_builtin_arbitration_agents() -> None:
    ...
    - Custom persona MD files (PROFILE.md, SOUL.md, AGENTS.md) from the
      ``aiarb.builtins.<agent_id>`` package
