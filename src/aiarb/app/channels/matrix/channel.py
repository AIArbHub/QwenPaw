# -*- coding: utf-8 -*-
"""
MatrixChannel: AIArb BaseChannel implementation for Matrix (via matrix-nio).
...
logger = logging.getLogger("aiarb.channels.matrix")
...
# Known AIArb slash commands — used to decide whether to strip
# @mention prefix
...
class MatrixChannel(BaseChannel):
    """AIArb channel that connects to a Matrix homeserver via matrix-nio."""
    ...
    def from_config(
        ...
        return cls(
            ...
            device_name=raw.get("device_name", "aiarb-worker"),
    ...
    async def _on_room_event(
        ...
        # Skip history prepend for slash commands — AIArb's command parser
    ...
    async def _on_room_media_event(self, room: MatrixRoom, event: Any) -> None:
        ...
        if mxc_url:
            ...
            # issues ($→%24 breaks the framework's image extension check).
            eid = event.event_id[:8].lstrip("$")
            filename = body or f"matrix_media_{eid}"
            filename = f"{eid}_{filename}"
            local_path = await self._download_mxc(mxc_url, filename)
    ...
    def build_agent_request_from_native(self, native_payload: Any) -> Any:
        ...
        # in the same room share one session (AIArb keys session state on
