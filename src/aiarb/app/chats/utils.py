...
def build_env_context(
    ...
    parts.append(
        ...
        f"You operate in AIArb, an open-source agent "
        f"framework built by the AIArb team from Qwen lab.",
    )
    parts.append(
...
# pylint: disable=too-many-branches,too-many-statements, too-many-nested-blocks
def framework_msg_to_message(
    messages: Union[Msg, List[Msg]],
) -> List[Message]:
    """
    Convert the framework Msg(s) into one or more runtime Message objects.

    Args:
        messages: the framework message(s) from streaming.
