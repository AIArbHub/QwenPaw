# -*- coding: utf-8 -*-
"""Secret-free aiarbmail DriverCard generation."""
...
import yaml
from aiarbmail_mcp.providers import ENTERPRISE_PROVIDERS
...
_MAIL_MCP_MODULE_ARGS = ["-m", "aiarbmail_mcp"]
_INTERNAL_MAIL_MCP_ARGS = ["--internal-mail-mcp"]


def is_managed_aiarbmail_card(path: Path) -> bool:
    """Return whether *path* is a AIArb-generated mail DriverCard."""
    if not path.is_file():
        return False
    try:
    ...
    return bool(
        ...
        and endpoint.get("args") == ["-m", "aiarbmail_mcp"],
    )


def resolve_aiarbmail_endpoint() -> tuple[str, list[str]]:
    """Resolve the command and arguments for the aiarbmail MCP server."""
    override = os.environ.get("AIARBMAIL_PYTHON", "").strip()
    if override:
        return override, list(_MAIL_MCP_MODULE_ARGS)
    if getattr(sys, "frozen", False):
        executable = Path(sys.executable)
        cli_executable = executable.with_name(f"aiarb{executable.suffix}")
        return str(cli_executable), list(_INTERNAL_MAIL_MCP_ARGS)
    try:
        if importlib.util.find_spec("aiarbmail_mcp") is not None:
...
def resolve_aiarbmail_command() -> str:
    """Resolve the executable used to launch the aiarbmail MCP server."""
    return resolve_aiarbmail_endpoint()[0]


def _load_managed_aiarbmail_card(path: Path) -> DriverCard | None:
    """Load an existing managed card whose user policy must be retained."""
    if not is_managed_aiarbmail_card(path):
        return None
    try:
        return load_card(path)
    except DriverCardError as exc:
        logger.warning(
            "Failed to preserve invalid aiarbmail DriverCard %s: %s",
...
def build_aiarbmail_env(
    mail: AgentMailConfig | None,
    workspace_dir: Path | None = None,
) -> dict[str, Any]:
    """Build secret-free DriverCard env bindings for aiarbmail.
    ...
    env: dict[str, Any] = {
        "AIARBMAIL_EMAIL": f"{credential.name}@{credential.domain}",
        "AIARBMAIL_AUTH_CODE": {
            "source": "credential",
            "credential": "mail",
            "field": "auth_code",
    ...
    if profile is not None:
        env["AIARBMAIL_IMAP_HOST"] = profile.imap_host
        env["AIARBMAIL_IMAP_PORT"] = str(profile.imap_port)
        env["AIARBMAIL_SMTP_HOST"] = profile.smtp_host
        env["AIARBMAIL_SMTP_PORT"] = str(profile.smtp_port)
    if workspace_dir is not None:
        env["AIARBMAIL_STATE_DIR"] = str(workspace_dir / "mail_state")
        env["AIARBMAIL_WORKSPACE_DIR"] = str(workspace_dir)
    return env


def generate_aiarbmail_driver_card(
    workspace_dir: Path,
    mail: AgentMailConfig | None = None,
) -> bool:
    """Persist mail secrets and publish a secret-free aiarbmail card."""
    try:
        card_path = workspace_dir / "drivers" / "mcp" / "aiarbmail.yaml"
        existing = _load_managed_aiarbmail_card(card_path)
        (workspace_dir / "mail_state").mkdir(parents=True, exist_ok=True)
        save_agent_mail_credentials(workspace_dir, mail)
        env = build_aiarbmail_env(mail, workspace_dir)
        command, args = resolve_aiarbmail_endpoint()
        ...
        logger.warning(
            "Failed to generate aiarbmail driver card for %s: %s",
...
def sync_aiarbmail_driver_card(
    workspace_dir: Path,
    mail: AgentMailConfig | None,
    backend: str,
    *,
    ...
    embedded ``AIARBMAIL_AUTH_CODE`` as a plaintext scalar.
    """
    del force_rewrite  # Compatibility with existing callers.
    driver_card = workspace_dir / "drivers" / "mcp" / "aiarbmail.yaml"
    if backend != "aiarb" or mail is None:
        driver_card.unlink(missing_ok=True)
        save_agent_mail_credentials(workspace_dir, None)
        return True
    if not generate_aiarbmail_driver_card(workspace_dir, mail):
