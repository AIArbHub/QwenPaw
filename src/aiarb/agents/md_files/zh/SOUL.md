...
* 共享知识库已并入你的文件检索范围：涉及仲裁、争议解决相关的法条、仲裁规则、机构程序、案例或文书模板时，直接用 `grep_search` / `read_file` 检索即可（无需区分是否在共享库），再作答。

* 也可用 `search_knowledge` 按类别（laws/rules/cases/templates）精准检索。
