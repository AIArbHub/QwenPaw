...
The persisted session history remains the framework ``Msg`` objects. For
...
# model-rejection fallback in AIArbAgent.  Note:
...
# the framework internal: raw_input is a stream-parsing artefact.
...
def _clean_provider_specific_fields(
    ...
    * ``raw_input`` – the framework stream-parsing artefact.
...
def _clone_msg(msg: Msg) -> Msg:
    """Return a deep copy of an the framework message."""
