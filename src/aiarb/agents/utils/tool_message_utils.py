...
def _mark_tool_call_as_failed(block: Any) -> None:
    ...
    from aiarb.framework.message import ToolCallState
...
def _tool_call_json_error_result(block: Any) -> Any | None:
    ...
    from aiarb.framework.message import TextBlock, ToolResultBlock, ToolResultState
...
def _reorder_tool_results(msgs: list) -> list:
    ...
    for msg in msgs:
        if isinstance(msg.content, list):
            ...
            # the framework 2.0 self-paired assistant msg
...
def _repair_empty_tool_inputs(
    ...
    This fixes a bug in the framework 1.0.16dev where stream_tool_parsing
                ...
                if not input_field and raw_input and raw_input != "{}":
                    try:
                        ...
                        if isinstance(parsed, dict) and parsed:
                            # All the framework 2.0 formatters expect
...
def _sanitize_tool_messages(msgs: list) -> list:
    ...
    for msg in msgs:
        ...
        # (the framework 2.0 style: tool_call + tool_result in the same
