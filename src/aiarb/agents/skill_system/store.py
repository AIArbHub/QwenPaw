...
_REQUIREMENTS_METADATA_NAMESPACES = ("openclaw", "aiarb", "clawdbot")
...
def _extract_emoji_from_metadata(metadata: Any) -> str:
    ...
    if not isinstance(metadata, dict):
        return ""
    aiarb = metadata.get("aiarb")
    if isinstance(aiarb, dict):
        return str(aiarb.get("emoji", "") or "")
    return ""
...
def extract_zip_skills(data: bytes) -> tuple[Path, list[tuple[Path, str]]]:
    ...
    tmp_dir = Path(tempfile.mkdtemp(prefix="aiarb_skill_upload_"))
...
def staged_skill_dir(skill_name: str) -> Iterator[Path]:
    ...
    temp_root = Path(
        tempfile.mkdtemp(prefix=f"aiarb_skill_stage_{skill_name}_"),
    )
