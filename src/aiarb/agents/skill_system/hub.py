...
def _github_cache_ttl() -> float:
    raw = EnvVarLoader.get_str("AIARB_GITHUB_CACHE_TTL", "")
    if raw:
        try:
...
def _hub_http_timeout() -> float:
    raw = EnvVarLoader.get_str("AIARB_SKILLS_HUB_HTTP_TIMEOUT", "30")
    try:
...
def _hub_http_retries() -> int:
    raw = EnvVarLoader.get_str("AIARB_SKILLS_HUB_HTTP_RETRIES", "3")
    try:
...
def _hub_http_backoff_base() -> float:
    raw = EnvVarLoader.get_str("AIARB_SKILLS_HUB_HTTP_BACKOFF_BASE", "0.8")
    try:
...
def _hub_http_backoff_cap() -> float:
    raw = EnvVarLoader.get_str("AIARB_SKILLS_HUB_HTTP_BACKOFF_CAP", "6")
    try:
...
def _build_async_client() -> httpx.AsyncClient:
    ...
    return httpx.AsyncClient(
        ...
        headers={"User-Agent": "aiarb-skills-hub/1.0"},
        limits=httpx.Limits(
...
def _extract_aiarb_skill_spec(
    ...
    """Parse a AIArb plaza skill URL into (owner, skill_name, version)."""
    parsed = urlparse(url)
    host = (parsed.netloc or "").lower()
    ...
    # accepts both `@the framework` and `the framework`.
    owner = parts[1].strip()
    skill_name = parts[2].strip()
    if not owner or not skill_name:
        return None
...
def _extract_aliyun_skill_spec(url: str) -> str | None:
    ...
    Accepts URLs synthesised by `aiarb.market.providers.aliyun`, of the
...
async def _fetch_bundle_from_aiarb_url(
    bundle_url: str,
    requested_version: str,
) -> tuple[Any, str]:
    spec = _extract_aiarb_skill_spec(bundle_url)
    if spec is None:
        raise ConfigurationException(
            config_key="skills_hub.bundle_url",
...
async def _fetch_bundle_from_aliyun_url(
    ...
    try:
        from aiarb.market.providers.aliyun import (
...
PROVIDERS: list[tuple[InstallOrigin, _ProviderMatcher, _ProviderFetcher]] = [
    ...
    (
        "aiarb",
        _extract_aiarb_skill_spec,
        _fetch_bundle_from_aiarb_url,
    ),
    (
        "modelscope",
