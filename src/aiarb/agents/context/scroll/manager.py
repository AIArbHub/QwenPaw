...
subclassing the agent, it is injected into :class:`AIArbAgent` and drives the
two delegated hooks.
...
from aiarb.framework.message import Msg, SystemMsg, TextBlock, UserMsg
...
class ScrollContextManager:
    ...
    def _recall_page_stub(
        ...
        page = cls._block_metadata(block).get("aiarb_recall_page", {})
        next_cursor = (
    ...
    def _persist_guarded(self, agent: Any) -> bool:
        ...
        the event loop because its the framework hook is synchronous and its
    ...
    async def compress(
        ...
        # 2) Trigger check (reuse the framework's own token accounting). The
        ...
        # 3c) Sanitize: the framework's pairing-safe split only guarantees
        #    intra-message block-level pairing. Standalone tool_result
        #    messages (the framework 1.x flat-timeline format) can still be
    ...
    def _persist_new(  # pylint: disable=too-many-branches
        ...
        the framework 2.0 extends the last assistant Msg in place (one Msg per
    ...
    def _is_continuation_stub(msg: Any) -> bool:
        ...
        if not isinstance(metadata, dict):
            return False
        tag = metadata.get(AIARB_MESSAGE_TAG_KEY)
    ...
    def _active_turn_tail(self, agent: Any) -> list[Msg]:
        ...
        the framework's token-based split may evict the latest user request when
    ...
    def _restore_full_tail_messages(
        ...
        the framework's compression splitter can divide one message's content
    ...
    def _repair_dangling_user_boundary(
        ...
        the framework's token split optimizes for a recent-tail token budget, so
