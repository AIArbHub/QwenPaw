# -*- coding: utf-8 -*-
"""Serialize the framework ``Msg`` blocks into ``conversation_history`` rows."""
...
from ....constant import AIARB_MESSAGE_TAG_KEY
...
def msg_to_entries(msg: Msg) -> list[LogEntry]:
    ...
    if non_result or not results:
        ...
        if isinstance(msg_meta, dict):
            tag = msg_meta.get(AIARB_MESSAGE_TAG_KEY)
        entries.append(
            LogEntry(
                kind="model_turn"
                ...
                metadata=({AIARB_MESSAGE_TAG_KEY: str(tag)} if tag else {}),
