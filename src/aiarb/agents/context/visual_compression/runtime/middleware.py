...
_TRANSFORM_EXECUTOR = ThreadPoolExecutor(
    max_workers=2,
    thread_name_prefix="aiarb-visual-compact",
)
...
def _model_key(current_model: Any) -> str | None:
    """Return AIArb's exact per-model capability-cache key."""
    try:
...
def _agent_will_strip_media(
    ...
    """Whether AIArb will discard media for this exact model call."""
    formatter = getattr(agent, "formatter", None)
    if bool(getattr(formatter, "_aiarb_force_strip_media", False)):
        return True
