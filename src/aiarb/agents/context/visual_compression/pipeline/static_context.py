# -*- coding: utf-8 -*-
"""AIArb system/tool planning and atomic visual replacement."""
...
from .tool_schemas import plan_aiarb_tool_documentation
...
_ENV_TAIL_HEADER = (
    "[AIArb environment context supplied by the host; "
    "this text was not written by the user.]"
)
_ENV_TAIL_FOOTER = "[End AIArb environment context.]"
_AIARB_ENV_BLOCK = re.compile(
    r"^====================\n"
...
def _extract_aiarb_env_context(text: str) -> tuple[str, str]:
    """Remove AIArb's complete env block from one system prompt."""
    matches = [
        match
        for match in _AIARB_ENV_BLOCK.finditer(text)
        if all(marker in match.group(0) for marker in _AIARB_ENV_MARKERS)
    ]
    if len(matches) != 1:
        return text, ""
    match = matches[0]
...
def _plan_system_partition(
    ...
    *,
    ...
    for index, message in enumerate(messages):
        ...
        if not env_tail:
            static_text, env_context = _extract_aiarb_env_context(text)
            if env_context:
...
def compress_static_context(  # pylint: disable=R0912,R0915
    ...
    *,
    ...
    new_tools = tools
    if tools:
        new_tools, tool_documentation = plan_aiarb_tool_documentation(tools)
    ...
    render_payload = (
        ...
        "===================\n"
        "AIArb rendered this session's configuration and tool parameter "
