...
INBOX_EMITTED_METADATA_KEY = "_aiarb_inbox_emitted"
...
def _to_reme_session_id(session_id: str) -> str:
    ...
    ReMe 0.4 uses ``session_id`` as a filename component. AIArb channel
...
class ReMeLightMemoryManager(BaseMemoryManager):
    ...
    ReMe uses the AIArb workspace root as its vault.  Daily memory,
    ...
    def _install_reme_result_hook(self) -> None:
        """Expose AIArb inbox delivery to ReMe background steps."""
        if self._reme is None:
            return
