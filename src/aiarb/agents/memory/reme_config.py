# -*- coding: utf-8 -*-
"""Embedded ReMe application configuration for AIArb memory.

ReMe's standalone CLI normally loads YAML such as
``reme/config/default.yaml`` or ``reme/config/aiarb.yaml``.  AIArb embeds
...
def build_reme_app_config(
    *,
    ...
    """Build ReMe ``Application`` kwargs for embedded AIArb usage."""
...
def _base_config() -> dict[str, Any]:
    """Return the ReMe config shape used by AIArb."""
...
def _base_components() -> dict[str, Any]:
    return {
        ...
        "agent_wrapper": {
            "default": {
                "backend": "agentscope",  # reme-ai library expects this exact value
                "as_llm": "default",
...
def _apply_embedding_config(
    ...
    """Map AIArb embedding config into ReMe component config."""
    ...
    if embedding_config.backend == "openai":
        components["as_embedding"]["default"]["pass_dimensions"] = embedding_config.use_dimensions
...
def _is_embedding_enabled(embedding_config: EmbeddingModelConfig) -> bool:
    ...
    # Keep enablement aligned with the framework credential requirements.
    backend = embedding_config.backend
...
def _embedding_credential(
    ...
    """Build the the framework credential payload for the selected backend."""
    backend = embedding_config.backend
    if backend in _OPENAI_COMPAT_EMBEDDING_BACKENDS:
