...
# once AIArb's minimum AgentScope version provides stable, request-unique
# identifiers; AIArb should then preserve the upstream identifiers.
...
_FORMATTER_SEEN_MEDIA_KEYS: ContextVar[set[str] | None] = ContextVar(
    "aiarb_formatter_seen_media_keys",
    default=None,
)
...
def _normalize_messages_for_formatter(
    ...
    strip_audio = bool(
        getattr(formatter_instance, "_aiarb_force_strip_audio", False),
    )
...
def _create_file_block_support_formatter(
    ...
    class FileBlockSupportFormatter(base_formatter_class):
        ...
        async def format(self, msgs):
            ...
            self._aiarb_last_wire_media_count = 0
            self._aiarb_last_wire_audio_count = 0
            ...
            require_reasoning = getattr(
                self,
                "_aiarb_require_reasoning_content",
                False,
            )
            ...
            self._aiarb_last_wire_media_count = _count_wire_media_blocks(
                wire_messages,
            )
            self._aiarb_last_wire_audio_count = _count_wire_audio_blocks(
                wire_messages,
            )
...
def _bind_provider_id_to_model(
    ...
    bind_provider_id = getattr(model, "bind_aiarb_provider_id", None)
...
def _install_model_formatter(
    ...
    """Install and return the AIArb formatter for one model."""
