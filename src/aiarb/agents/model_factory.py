...
from aiarb.framework.formatter import FormatterBase, OpenAIChatFormatter
from aiarb.framework.message import TextBlock
from aiarb.framework.model import ChatModelBase
...
from aiarb.framework.formatter import OpenAIResponseFormatter
...
def _normalize_messages_for_formatter(
    ...
    if getattr(formatter_instance, "_aiarb_force_strip_media", False):
...
def _format_anthropic_video_data_block(block: Any) -> dict | None:
    ...
    the framework's stock Anthropic formatter drops every non-image
...
def _promote_tool_result_videos(
    ...
    Mirrors the image promotion that the framework's formatter does
...
def _fix_image_mime_types(messages: list[dict]) -> None:
    ...
    the framework derives MIME from the file extension literally
...
def _reasoning_by_assistant_segment(
    ...
    tool result or hint. the framework can keep several reasoning/tool cycles in
...
def _create_file_block_support_formatter(
    ...
    in tool results, which are not natively supported by the framework.
...
def create_model_and_formatter(
        ...
        if not global_model:
            raise ProviderError(
                message=(
                    ...
                    "Please configure a model using 'aiarb models config' "
    ...
    # the framework 2.0 ChatModelBase has its own retry loop
...
def _create_formatter_instance(
    ...
    the framework 2.0 attaches each model's default formatter at construction
    ...
    Returns:
        ...
        the model's native one, plus aiarb extensions for media
        promotion and file blocks).
    """
    base_formatter = getattr(model, "formatter", None)
    if not isinstance(base_formatter, FormatterBase):
        # All the framework 2.0 ChatModelBase subclasses default to a real
        ...
        raise TypeError(
            ...
            f"formatter. the framework 2.0 models should default to their "
