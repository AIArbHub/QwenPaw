...
aiarb envelope schema and the ACP transport stack.  Eagerly importing
them here would slow down every consumer of ``aiarb.agents.acp`` —
including ``delegate_external_agent`` which only needs
``tool_adapter`` — so we expose ``AIArbACPAgent`` /
``run_aiarb_agent`` / ``ACPService`` / ``*_acp_service`` via
...
_LAZY_EXPORTS = {
    "AIArbACPAgent": ".server",
    "run_aiarb_agent": ".server",
    "ACPService": ".service",
...
# fine — verified by ``from aiarb.agents.acp import ACPService`` working.
__all__ = [
    "ACPErrors",
    "ACPConfigurationError",
    "ACPProtocolError",
