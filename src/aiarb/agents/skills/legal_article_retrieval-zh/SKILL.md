...
| 主体信息 | 你（劳动者/消费者/申请人等）vs 对方（单位/卖家/被申请人等） |
| 核心事实 | 发生了什么？时间、地点、行为                 |
...
**常用基准数据库**：
- 国家法律法规数据库（`https://flk.npc.gov.cn`，全国人大常委会维护，官方免费来源）
- 北大法宝（含英文版）
- 北大法意
- 月旦知识库
- 万律网（westlawchina）
...
当任务涉及中国大陆法律、行政法规、地方性法规、自治条例和单行条例、司法解释、部门规章、地方政府规章、法律解释、决定、命令、规范性文件或具体法条原文，且用户没有明确指定使用北大法宝、元典 MCP、威科、法意等商业数据库时，默认优先使用**国家法律法规数据库**（`https://flk.npc.gov.cn`）作为法规和法条交叉检索的主来源。

默认路由顺序：
1. 用户指定的数据源优先。例如用户明确要求北大法宝或元典 MCP，则按用户指定执行。
2. 已连接且可用的专业法规 MCP 可作为第一执行工具，但返回结果必须保留来源链接、效力状态和条文原文。
3. 未指定来源时，优先使用已配置的北大法宝 MCP；国家法律法规数据库仅在用户明确指定、北大法宝不可用/无结果或存在重大效力冲突时按需使用，不构成默认或强制检索步骤。
4. 国家法律法规数据库无法覆盖或无法访问时，再使用司法部、国务院部门官网、地方人大/政府官网、最高法/最高检官网等官方来源补充。
5. 非官方网页、媒体文章、AI 摘要只能用于线索发现，不得作为法条原文的唯一依据。

国家法律法规数据库检索策略：
- 优先调用 LegalWork 运行时的 `knowledge_legal_external_sources`，直接获取国家法律法规数据库结构化候选；不要先让模型凭记忆猜法规名，也不要先走普通搜索引擎。
- 同一查询应自动组合标题精确、标题模糊、正文模糊三类检索，并按 `bbbs` 去重；多策略重复命中的结果优先级更高。
- 结果排序优先级：现行有效 > 标题精确命中 > 标题/正文多策略命中 > 法源位阶高 > 制定机关权威。
- 已知法规名称：优先做标题精确检索，必要时再做标题模糊检索。
- 只知道主题关键词：先做标题模糊检索，再做正文检索。
- 需要具体条文：先定位法规，再读取法规结构并抽取具体条、款、项；运行时返回 `条文原文` 时优先使用该文本，并保留“按条号抽取/按关键词抽取”的来源说明；无法定位时用正文关键词跨法规检索。
- 查询“第X条”时，运行时应下载官方 DOCX 并抽取对应条文全文；只返回目录命中但无原文时，必须继续下载原文或在报告中标注“仅目录命中，待逐字核验”。
- 默认优先返回 `现行有效` 文本；若任务要求历史版本、废止依据或修订沿革，应同时检索 `已修改`、`已废止`、`尚未生效` 状态。
- 每次引用必须记录：法规名称、制定机关、效力层级、公布日期、施行日期、时效性、条文原文、来源链接或可复核的法规详情页。
- 禁止把第三方 skill 的限速策略作为 LegalWork 默认动作；批量检索只做分页、缓存和失败重试，不主动设置人为限速或等待降速。

> **【案例示范】高速公路与居民区距离案**
> **检索工具**：默认优先使用已配置的北大法宝/元典 MCP 检索法规与案例。国家法律法规数据库及其他官方网站仅在用户明确指定、商业库无结果或存在重大效力冲突时按需核验；不得为访问这些网站自动调用用户浏览器。
...
**核心逻辑**：以请求权基础为出发点，检视申请人仲裁请求能否成立，规范寻找先于法律事实认定。
...
| **步骤4：时间审查**  | 时间效力（生效、终止、溯及力）实体法不溯及既往，实体司解具实质溯及力，程序法即时适用 | 建受理件时间轴,注意一/二/再审适用范围 |
...
| **程序从新原则**   | 诉讼过程中程序法变更   | 5步：确认程序阶段→确认程序法变更→判断是否"正在进行"→检索适用规定→判断适用 | 《立法法》第93条但书 | 混淆实体与程序       | 二审程序适用新《仲裁法》      |
...
执行下列检索方法前,先判断用户是否指定了数据源,以及运行环境中是否存在**法规检索类工具**(例如已连接的法规库 MCP 服务、检索 API 或本地法规库):

1. **用户指定来源**:用户明确指定北大法宝、元典 MCP、威科、法意、地方官网等来源时,按用户指定来源执行;如果该来源不可访问,说明失败原因并请求是否改用官方来源。
2. **专业工具可用**:存在已连接的法规库 MCP/API/本地法规库时,必须将设计好的检索式交给该工具,以其返回的真实结果(法律名称、文号、效力级别、生效/失效日期、条文正文、来源链接)作为后续涵摄、引用与报告的数据来源。不得用记忆中的"法条"替代或补充检索结果。
3. **未指定商业库或专业工具不可用**:默认使用国家法律法规数据库 `https://flk.npc.gov.cn`。可通过公开检索、web_search/web_fetch、API、脚本或浏览器访问执行;优先 API/结构化结果,浏览器仅作为 API 失败、动态页面需要交互或用户要求 UI 操作时的回退。
4. **国家法律法规数据库也不可用**:照常完成检索式设计与方法论步骤,但凡涉及具体条文之处一律标注 `[待查]`,并在报告中说明"未能访问法规库,以下检索式需人工执行"。**绝不编造条文编号、内容或文号。**
5. **工具无关**:本约定只依赖"输入检索词→返回真实法条"这一抽象能力,不绑定任何特定供应商。具体接入方式(如北大法宝 MCP)见本技能目录下的 [`README.md`](./README.md)。

国家法律法规数据库执行要点：

| 场景 | 默认检索动作 | 复核动作 |
|------|--------------|----------|
| 已知法规名 | 标题精确检索, 限定现行有效优先 | 对比公布日期、施行日期、时效性 |
| 主题找法 | 标题模糊检索后做正文检索 | 按法源位阶筛选, 排除噪声 |
| 查某法第几条 | 先定位法规详情, 再按条号抽取条文 | 核对款、项、但书是否完整 |
| 跨法规找具体条文 | 正文关键词检索, 必要时分批处理 | 记录检索式、页码/批次和遗漏风险 |
| 历史版本/废止 | 同时检索现行有效、已修改、已废止、尚未生效 | 标注适用时间范围和替代规则 |
...
> - 赔偿计算规则：以房屋价值为基数，酌减申请人过错或自然减损部分
...
| 事项 | 期限 | 依据 | 注意事项 |
| :----- | :-------- | :-------------- | :--------------------------- |
| 民事仲裁时效 | 3年（一般） | 《民法典》第188条 | 自权利人知道或者应当知道权利受到损害以及义务人之日起计算 |
| 特别仲裁时效 | 1年/2年/4年等 | 《民法典》第189-194条 | 海上货物运输合同、国际货物买卖合同等 |
| 除斥期间 | 依具体规定 | 各单行法 | 性质为形成权，期满权利消灭 |
| 行政复议期限 | 60日 | 《行政复议法》第20条 | 知道具体行政行为之日起计算 |
| 行政诉讼期限 | 6个月/15日 | 《行政诉讼法》第46条 | 不服复议决定的15日，直接申请仲裁的6个月 |
...
关于法律分析方面，可能是因为立法滞后或立法导向问题，**没有法律对建设高速公路途经居民区的情况下，应保持多少距离的限制性的具体规定**。然而，安全是相互的，从设置高速公路30米的控制区、安全技术评估报告、环评报告等规定，均对高速公路建设的安全性做了调整。但是，这些规定，**难以作为仲裁员认定兴建高速公路越距违法的依据**。

而之所以仲裁庭支持申请人诉请，本律师猜测，主要是因为仲裁员的内心认为拆迁安置补偿属于高速公路建设过程中必要的成本，其次是基于仲裁员对安全的重视。这类裁决的请求权基础为《民法通则》106条的有案例⑧⑩；案例⑨则引用了《民法通则》117条和《侵权责任法》85条。关于赔偿额计算方面，仲裁机构均以房屋价值为基数，并指出申请人有一定的过错或以房屋价值自然减损为由，予以酌减。
...
**法律适用风险提示**：
- **正面后果**：可依据相邻关系主张侵权损害赔偿，仲裁机构可能支持补偿请求
