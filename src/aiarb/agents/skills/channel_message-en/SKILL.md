...
description: Use this skill to proactively send a one-way message to a user/session/channel, usually only when the user explicitly asks to send to a channel/session or when proactive notification is needed. First query sessions with aiarb chats list, then push with aiarb channels send.
metadata:
...
### Should Not Use
- If you are simply replying in the current session, **do not use `aiarb channels send`**
...
```bash
aiarb chats list --agent-id <your_agent> --channel <channel>
```

You can also filter by user:

```bash
aiarb chats list --agent-id <your_agent> --user-id <user_id>
```
...
`aiarb channels send` requires all of the following:
- `--agent-id`
- `--channel`
- `--target-user`
- `--target-session`
...
`aiarb channels send` only sends — it does not wait for a reply.
...
```bash
aiarb chats list --agent-id notify_bot --channel feishu
...
```bash
aiarb chats list --agent-id task_bot --channel console
...
```bash
aiarb chats list --agent-id analyst_bot --user-id alice
...
If you are replying to the user in the current session, do not use `aiarb channels send`.
...
```bash
aiarb chats list --agent-id <your_agent>
```
...
```bash
aiarb channels list --agent-id <your_agent>
```
...
- **aiarb agents chat**: sends to another agent, two-way, returns a reply
- **aiarb channels send**: sends to a user/session/channel, one-way, no reply

**Selection principle**:
- Need to collaborate with another agent → `aiarb agents chat`
- Need to proactively push a message to a user → `aiarb channels send`
...
- `--target-user`: target user ID (obtained from `aiarb chats list`)
- `--target-session`: target session ID (obtained from `aiarb chats list`)
