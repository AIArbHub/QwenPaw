...
name: 演绎推理
description: |
  基于形式逻辑执行严密的法律演绎推理，用于检验法律论证的形式有效性。当用户明确要求“分析案件逻辑”、“验证法律推理”、“提取三段论”或“判断法律论证的有效性”时调用。
  注意：本技能输出的是逻辑分析格式（大前提-小前提-结论），主要用于内部推理校验；起草上诉状、代理词、辩护词等仲裁文书时，不应机械套用此格式，而应转化为自然、有说服力的法律语言。
tags: ['法律推理']
---
...
**重要提示：** 本技能输出的是逻辑分析格式（大前提-小前提-结论），主要用于内部推理校验；起草上诉状、代理词、辩护词等仲裁文书时，不应机械套用此格式，而应转化为自然、有说服力的法律语言。仲裁文书应围绕争议焦点，先阐明主张，再结合事实、证据与法律依据展开论证。
...
2. **小前提 (Fact, f/F):**
   ...
   - **法律小前提 (F):** 具有法律意义的事件、行为、状态（律师或仲裁员对自然事实的定性，如“A与B成立运输合同关系”）。
1. **结论（假设） (Conclusion, C):** 裁判文书得出的部分或最终结论（如：有罪/无罪、违约/不违约）。部分结论包括中间结论或者有待验证的结论假设，比如仲裁员总结出的争议焦点，比如当事人或者律师的初步推断
示例：
【大前提】：空
【小前提-法律事实】：空
...
### 第三步：输出 
每次调用本技能，必须严格按照以下格式输出两部分内容（本输出为逻辑分析草稿，不应直接作为上诉状、代理词等仲裁文书的正文）：
#### 1. 可解释的推理链条
使用清晰的 P-F-C 结构图或层级列表，展示结论是如何通过中间结论、法律小前提、自然小前提一步步回溯、链接到相应大前提的。明确标出每一步所使用的中项。

> **注意：** 若本技能的结果用于起草仲裁文书，应将 P-F-C 结构转化为自然语言论证：先提出主张，再陈述关键事实与证据，最后援引法律规范得出结论。不得在文书中逐条标注“大前提”“小前提”“结论”。
...
在案件分析、代理词、裁判说理或合同条款论证中使用时，先从工作区材料提取“规则大前提—事实小前提—结论”，再检查中项是否一致、前提是否缺失、结论是否偷换。输出推理链表和谬误标注，可作为文书论证修改清单。
