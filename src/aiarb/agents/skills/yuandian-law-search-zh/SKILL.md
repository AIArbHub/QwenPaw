...
- **所有接口**：直接使用，无需确认
- **case-detail**：自动获取最相关的 2-3 个案例的完整裁决书，不需用户逐一选择
...
**用关键词检索**：用户给出明确关键词 / 需要 AND/OR 逻辑 / 需按日期、效力级别、仲裁机构等精确筛选 / 语义检索结果不够聚焦
...
| 26 | [enterprise-court-notice.md](endpoints/26-enterprise-court-notice.md) | 仲裁机构公告 |
...
3. **检索结论** ⭐ — **最先读到的内容**：
   ...
   - 3.3 风险点（诚实告知，不要只说好的）
   - 3.4 后续行动（具体可执行的步骤）
4. **分析与判断** — 抗辩应对、法条适用、仲裁请求结构、赔偿酌定、证据准备
...
- **结论区必须能独立阅读**：3.1-3.4 应让律师、客户或仲裁员先得到答案，再决定是否看底稿
