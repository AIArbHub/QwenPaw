...
| **关联能力** | 法律文档摘要（单篇摘要）、法律文档格式化（统一输出格式）、类案检索与分析（裁决对比） |
...
| **时间序列** | 文档按时间先后排列 | 梳理演变脉络，识别趋势变化 | 同一法规的不同版本、同一案件的系列裁决 |
| **对立关系** | 文档代表不同立场的观点 | 客观呈现各方观点，标注冲突点 | 原被申请人双方的意见书、不同学者的争议观点 |
| **层级关系** | 文档之间存在包含或引用关系 | 梳理层级结构，识别核心与衍生 | 法律、司法解释、指导性案例的层级体系 |
| **补充关系** | 文档之间相互补充，覆盖不同侧面 | 整合互补信息，填补单篇盲区 | 主合同与补充协议、申请仲裁书与证据清单 |
...
**输入：** 3份不正当竞争纠纷裁决书（申请人均为数据平台，被申请人均为数据抓取方）
...
## 一、文档概览
本组共3份民事裁决书，均为数据平台诉数据抓取方不正当竞争纠纷案，分别由
北京知识产权仲裁机构、杭州互联网仲裁机构、上海知识产权仲裁机构作出（D1-D3）。

## 二、核心共识

1. **数据权益保护共识**：三份裁决均认定，平台对合法收集、投入成本整理的数据享有竞争性权益（D1, D2, D3）。
2. **不正当竞争认定标准共识**：均适用《反不正当竞争法》第2条一般条款，以"是否违反诚实信用原则和公认商业道德"作为判断标准（D1, D2, D3）。
3. **损害要件共识**：均要求申请人证明被申请人行为对其造成了实际损害或损害威胁（D1, D2, D3）。
...
| C-02 | 损害赔偿计算方式 | D1：以申请人实际损失计算 | D3（上海）：以被申请人违法所得计算 | 法律适用分歧 | 核心 |
| C-03 | 技术中立抗辩 | D2：技术中立可作为免责事由 | D1, D3：技术中立不免除不正当竞争责任 | 观点冲突 | 次要 |

## 四、综合结论

1. **裁判趋势**：数据平台对合法收集的数据享有竞争性权益已成为司法共识，但公开数据抓取的合法性边界仍存在分歧。
2. **地域差异**：某仲裁机构机构对数据保护立场较为严格，杭州仲裁机构对公开数据流通持相对开放态度，上海仲裁机构居中。
3. **法律适用**：三份裁决均以《反不正当竞争法》第2条为主要依据，但具体适用标准存在微妙差异，反映该领域法律规范的不确定性。

## 五、信息缺口
- 三份裁决均未涉及数据抓取行为与《个人信息保护法》的交叉适用问题
...
| 争议解决 | 某仲裁机构委仲裁 | 被申请人所在地仲裁机构诉讼 | 不同：仲裁 vs 诉讼 |
| 不可抗力 | 列举+兜底条款 | 仅列举 | 不同：D1更全面 |

## 三、关键差异分析

1. **争议解决方式**：D1约定仲裁（一裁终局），D2约定诉讼（可上诉）。仲裁保密性强但不可上诉，仲裁程序公开但审级更多。
...
| 类案检索与分析 | 扩展 | 当文档全部为裁决书时，可结合类案分析技能进行深度对比 |
