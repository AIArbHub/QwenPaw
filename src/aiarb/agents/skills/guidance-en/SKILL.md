---
name: guidance
description: "Answer user questions about AIArb installation and configuration: first locate and read local documentation, then distill the answer; if local information is insufficient, fall back to the official website documentation."
metadata:
...
# AIArb Installation and Configuration Q&A Guide

Use this skill when the user asks about **AIArb installation, initialization, environment configuration, dependency requirements, or common configuration options**.
...
```bash
DOCS_DIR=$(python3 -c "from aiarb.constant import DOCS_DIR; print(DOCS_DIR or '')" 2>/dev/null)
```
...
DOCS_DIR=$(find ~/.aiarb/memory/ -type d -name "docs")
```
...
Run the following script logic to obtain the variable $AIARB_ROOT:

```bash
# Get the absolute path of the binary
AIARB_PATH=$(which aiarb 2>/dev/null || whereis aiarb | awk '{print $2}')

# Logical deduction: if the path contains .aiarb/bin/aiarb, the root is three levels up
# Example: /path/to/AIArb/.aiarb/bin/aiarb -> /path/to/AIArb
if [[ "$AIARB_PATH" == *".aiarb/bin/aiarb" ]]; then
    AIARB_ROOT=$(echo "$AIARB_PATH" | sed 's/\/\.aiarb\/bin\/aiarb//')
else
    # Fallback: try to get the parent of the parent directory
    AIARB_ROOT=$(dirname $(dirname "$AIARB_PATH") 2>/dev/null || echo ".")
fi

echo "Detected AIArb Root: $AIARB_ROOT"
```

Verify and list the documentation directory:
Use the derived $AIARB_ROOT to locate the documentation:

```bash
# Construct the standard documentation path
DOCS_DIR="$AIARB_ROOT/website/public/docs/"
...
if [ -d "$DOCS_DIR" ]; then
    ...
    find "$AIARB_ROOT" -type d -name "docs" | grep "website/public/docs"
fi
```

**If project documentation does not exist, search the working directory**

If documentation is still not found, search for available documentation content under the aiarb installation path:
