---
name: cron
description: Use this skill only for scheduled or recurring tasks. Manage jobs with aiarb cron list/create/get/state/update/pause/resume/delete/run, and always pass --agent-id explicitly.
metadata:
...
All `aiarb cron` commands **must** include:
...
```bash
# List tasks
aiarb cron list --agent-id <agent_id>

# View task details
aiarb cron get <job_id> --agent-id <agent_id>

# View task status
aiarb cron state <job_id> --agent-id <agent_id>

# Create a task
aiarb cron create --agent-id <agent_id> ...

# Delete a task
aiarb cron delete <job_id> --agent-id <agent_id>

# Pause / Resume a task
aiarb cron pause <job_id> --agent-id <agent_id>
aiarb cron resume <job_id> --agent-id <agent_id>

# Run an existing task once immediately
aiarb cron run <job_id> --agent-id <agent_id>

# Update an existing task
aiarb cron update <job_id> --agent-id <agent_id> ...
```
...
```bash
aiarb cron create --agent-id <agent_id> -f job_spec.json
```
...
5. Create the task with aiarb cron create
...
- Before modifying/pausing/deleting, run `aiarb cron list --agent-id <agent_id>` first
- To troubleshoot issues, use `aiarb cron state <job_id> --agent-id <agent_id>`
...
- Before creating, you can run `aiarb chats list --agent-id <agent_id>` to get valid `target-user` and `target-session`
