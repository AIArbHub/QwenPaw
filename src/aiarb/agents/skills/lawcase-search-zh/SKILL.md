...
name: 案例搜索
description: 中国法律案例检索与类案分析技能。当用户需要：(1) 检索法律案例、判例、裁判文书，(2) 查找类似案例/类案，(3) 分析特定法条的司法适用案例，(4) 了解某类纠纷的裁判趋势，(5) 为诉讼/仲裁寻找支持性判例，(6) 检索特定仲裁机构/地域/时间范围内的案例时使用。支持关键词检索、法条检索、案由检索，自动生成类案检索总结。适用于劳动争议、合同纠纷、知识产权、不正当竞争等各类法律案件的案例检索需求。
dependency:
  python:
    - python-docx==0.8.11
    - pandas==2.0.3
...
3. **不允许编造**：案例案号、仲裁机构、文书类型、链接/原文必须可核验；无法核验时必须明确标注“未核验”。  
...
- 步骤2：给出**规则/法条要点** + **≥3条可检索的案例线索（案号/仲裁机构/关键词）** + **优化后的关键词方向**  
- 步骤3：整理**多组**关键词（每组3~7个），并输出 `keywords_groups`（每组含 `keywords_str`，空格分隔）  
- 步骤4：获得包含案号/仲裁机构/链接等信息的候选案例（search_result.json）  
...
- 仲裁机构观点，用户希望仲裁机构如何认定/支持/驳回（用1句话复述）
- 限制条件，如时间范围/地域/仲裁机构层级/案由/当事人身份等（如有）
- 相关法规
...
2) **案例线索（≥3条）**：每条至少包含“仲裁机构/案号线索/可用于检索的独特关键词”，并**把每条线索转写成可检索的锚点信息 + 至少2组关键词**：
   - 锚点:（尽量补全）
     - 仲裁机构： 仲裁机构全称/简称（如“江西省金溪县人民法院”/“金溪仲裁机构、金溪”）
     - 当事人：（优先“吴某/周某”，**禁止仅用单字姓**）
     - 案由/争议点：（如“彩礼纠纷/返还彩礼/抢票外挂”）
     - 独特短语：1~2个独特短语/行为词（来自线索文本，用于定位，避免空泛）
     - 结论：支持类（予以支持、承担责任、应当赔偿、构成侵权），驳回类（不予支持、驳回仲裁请求、不承担责任、不构成）、维持原判、改判、发回重审等表明诉讼结果的
   ...
   **关键词质量门槛（硬规则）**：每一组 关键词 必须同时满足：
   - 含 ≥1 个**定位锚点**（仲裁机构全称/简称/案号片段/当事人之一）
   - 含 ≥1 个**争议锚点**（案由/行为/核心争点词）
   - 禁止仅用“省/市”级泛地域；优先“县/区/市辖区/仲裁机构简称”等更细粒度锚点
   - 禁止“单字姓 + 泛案由”这种弱特征组合（如“吴 周 彩礼”）
   - 如用户要求特定裁决结果时使用「结论」关键词。
   - 关键词: 用空格分隔（将写入 search_request.json 的 `input`）
...
### 2.2 探查典型案例
```
联网搜索: "[法律名称] [条款号] 典型案例 裁决 [年份]"
联网搜索: "[具体行为类型] [法律名称] 裁决书"
```

### 2.3 获取裁决书验证（可选，用于确认案例有效性）
```
网页读取: [裁决书链接]
在裁决书中搜索完整法条表述，如"《中华人民共和国反不正当竞争法》第十二条"
```
...
| **DocumentAttr** | 文书类型 | 仅填**标准类型**（如“裁决书”）。若不确定，保留在全文中。 |
| **CaseGist** | 裁判要点 | 仅提取明确的核心裁判摘要。若不确定，保留在全文中。|
| **PlaintiffClaims** | 仲裁请求 | 特定范围检索。仅当需限定查找“申请人主张了什么”时提取；否则保留在全文中。|
| **DefenseViewpoint** | 辩方观点 | 特定范围检索。仅当需限定查找“被申请人抗辩了什么”时提取；否则保留在全文中。|
| **ControversialFocus** | 争议焦点 | 特定范围检索。仅当需限定查找“争议焦点”段落时提取；否则保留在全文中。|
| **Identified** | 本院认为 | 特定范围检索。仅当需限定查找“仲裁机构说理/认定”段落时提取；否则保留在全文中。|
| **Ascertain** | 本院查明 | 特定范围检索。仅当需限定查找“仲裁庭查明事实”段落时提取；否则保留在全文中。|
