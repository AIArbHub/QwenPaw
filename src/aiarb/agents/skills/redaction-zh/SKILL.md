...
| **关键法源** | 《个人信息保护法》第4条（个人信息定义）、第6条（最小必要原则）、第72条（匿名化处理）；《数据安全法》第21条（数据分类分级）；《民法典》第1034条（隐私权与个人信息保护）；《最高人民法院关于适用〈中华人民共和国仲裁法〉的解释》第520条（裁判文书上网的隐名处理）；《最高人民法院关于人民法院在互联网公布裁判文书的规定》第8-11条（隐名处理规则） |
...
| **当事人信息** | 申请人/被申请人/第三人/申请人/被申请人全称 | 公司名含"有限公司""股份公司"等后缀 | **高** |
| **代理人信息** | 律师姓名、律所名称、执业证号 | "律师""代理人""辩护人"等称谓 | **中高** |
| **司法人员** | 仲裁员、检察官、办案秘书、鉴定人姓名 | "首席仲裁员""审判员""检察员"等称谓 | **中** |
...
AIArb 在通用脱敏策略基础上，内置两套仲裁专用策略：

#### 仲裁内部分析策略（`arbitration_internal`）

适用场景：仲裁代理人内部分析、仲裁庭合议、案件研讨

| 实体类型 | 脱敏模式 | 说明 |
|---|---|---|
| 当事人姓名/公司名 | 令牌化（T） | 保留结构线索，同一主体全称/简称/代词统一 token |
| 仲裁员姓名 | 令牌化（T） | 仲裁员甲/乙/丙 |
| 代理律师姓名 | 令牌化（T） | 代理律师甲/乙/丙 |
| 证人姓名 | 令牌化（T） | 证人甲/乙/丙 |
| 身份证号/银行卡号 | 完全掩码（M） | |
| 手机号/邮箱 | 完全掩码（M） | |
| 地址 | 部分掩码（P） | 保留到城市级别 |
| **金额** | **保留** | 仲裁请求金额、裁决金额须保留用于分析 |
| **日期** | **保留** | 合同签订日、组庭日、开庭日、裁决日须保留 |
| **案号** | **保留** | 仲裁案号须保留用于检索 |
| **证据编号** | **保留** | 证据一/证据二等编号须保留用于引用 |
| 车牌 | 掩码（M） | |
| 商业秘密 | 可配置 | 需人工复核决定 |

#### 仲裁公开发布策略（`arbitration_publish`）

适用场景：仲裁裁决书公开、案例库入库、学术研究发布

| 实体类型 | 脱敏模式 | 说明 |
|---|---|---|
| 当事人姓名 | 完全掩码（M） | |
| 当事人公司名 | 角色替换（R） | A公司/B公司 |
| 代词/简称 | 角色替换（R） | 申请人/被申请人 |
| 仲裁员姓名 | 完全掩码（M） | |
| 代理律师姓名 | 完全掩码（M） | |
| 证人姓名 | 完全掩码（M） | |
| 身份证号/手机号/邮箱/地址/银行卡 | 完全掩码（M） | |
| 微信号/支付宝/IP | 完全掩码（M） | |
| **金额** | **保留** | 仲裁裁决金额属于公开信息 |
| **日期** | **保留** | |
| **证据编号** | **保留** | |
| 案号 | 可配置 | 公开案例保留，未公开案例掩码 |

#### 仲裁当事人聚类增强

仲裁文书当事人称谓极其混乱（申请人/甲方/该公司/其/被申请人之一），脱敏后如果映射不一致，后续 AI 分析会错乱。本模块内置：

- **仲裁角色词识别**：申请人、被申请人、反请求申请人、反请求被申请人、申请人一/二、被申请人一/二、首席仲裁员、独任仲裁员
- **仲裁案号识别**：京仲案字第XXXX号、CIETAC年份编号、XXXX年XX字第XXXX号
- **Canonical/Mention 聚类 + 可逆映射**：同一主体的全称/简称/代词统一映射到同一 token
- **`mapping.enc` 反向还原**：支持对 AI 二次分析结果做代词反向还原

### 1.5 实体聚类（Entity Clustering）
...
| **P1** | 语境敏感信息 | 根据上下文语义识别非格式化的敏感信息（个人姓名结合"申请人""被申请人"等语境关键词） |
...
| **公开发布** | M（完全掩码） | M（完全掩码） | M（完全掩码） | S（删除） | G（保留大致范围） | P（保留仲裁机构+年份） | M（完全掩码） |
...
```bash
python -m aiarb.redaction.agent restore mapping.enc ai_analysis.md --output restored.md
```
...
**场景特点**：法律文书（申请仲裁状、答辩状、代理词、法律意见书等）需要发送给仲裁机构、对方当事人或其代理人，需要在保护敏感信息与保持文书可读性之间取得平衡。

**脱敏原则**：
- 当事人姓名：保留姓氏，名用"某"代替（如"张某某"），或使用"申请人""被申请人"等诉讼地位替代
...
- 仲裁机构或仲裁庭要求披露的：应在保密令（Confidentiality Order）保护下披露
...
| **案号** | 仲裁机构案号格式 | `（?\d{4}）?[：:]\s*[^\s]{0,10}(民初\|民终\|刑初\|刑终\|知行初\|行初\|行终\|执\|民再\|刑再)\s*\d{1,6}号?` |
...
| 信息类型 | 语境关键词 |
|----------|-----------|
| **当事人姓名** | 申请人、被申请人、第三人、申请人、被申请人、上诉人、被上诉人、再审申请人、被申请人、申请人、被执行人 |
| **代理人姓名** | 委托代理人、辩护人、仲裁代理人、特别授权、一般授权、指派、律师事务所 |
| **司法人员姓名** | 首席仲裁员、审判员、、办案秘书、仲裁员、检察官、检察员、助理检察员 |
...
| **过度脱敏** | 将案号、仲裁机构名称等公开信息也脱敏 | 文档失去法律效力 | 区分公开信息与隐私信息 |
...
**原始文本：**
> 申请人张三（身份证号：110101199001011234，手机号：13812345678）与被申请人李四（身份证号：320102198512095678，手机号：13987654321）房屋买卖合同纠纷一案，本院于2024年1月15日受理。申请人张三诉称，其于2023年6月1日与被申请人李四签订《房屋买卖合同》，约定申请人以人民币300万元购买被申请人位于北京市海淀区中关村大街1号院的房屋一套。申请人已支付定金50万元，但被申请人拒绝办理过户手续。

**脱敏后文本（部分掩码策略）：**
> 申请人张**（身份证号：110101********1234，手机号：138****5678）与被申请人李**（身份证号：320102********5678，手机号：139****4321）房屋买卖合同纠纷一案，本院于2024年1月15日受理。申请人张**诉称，其于2023年6月1日与被申请人李**签订《房屋买卖合同》，约定申请人以人民币300万元购买被申请人位于北京市海淀区[具体地址已脱敏]的房屋一套。申请人已支付定金50万元，但被申请人拒绝办理过户手续。
...
**脱敏后文本（令牌化策略）：**
> 申请人买方A（身份证号：ID_CARD_001，手机号：PHONE_001）与被申请人卖方B（身份证号：ID_CARD_002，手机号：PHONE_002）房屋买卖合同纠纷一案，本院于2024年1月15日受理。申请人买方A诉称，其于2023年6月1日与被申请人卖方B签订《房屋买卖合同》，约定申请人以人民币AMT_001万元购买被申请人位于北京市海淀区ADDR_001的房屋一套。申请人已支付定金AMT_002万元，但被申请人拒绝办理过户手续。
...
**脱敏后文本（完全掩码策略）：**
> 申请人***（身份证号：***，手机号：***）与被申请人***（身份证号：***，手机号：***）房屋买卖合同纠纷一案，本院于2024年1月15日受理。申请人***诉称，其于2023年6月与被申请人***签订《房屋买卖合同》，约定申请人以人民币***万元购买被申请人位于北京市的房屋一套。申请人已支付定金***万元，但被申请人拒绝办理过户手续。

## 八、局限性与风险提示

1. **识别精度限制**：基于规则和模式的识别方法无法保证100%的敏感信息识别率。非标准的格式、拼写错误、故意混淆的写法、跨页断行等情况可能导致漏识别。建议对高安全要求的场景进行人工复核。

2. **语义理解限制**：AI对语境的理解有限，难以准确识别所有通过语义暗示的敏感信息。例如"我的老同学在朝阳仲裁机构工作"这样的表述可能间接泄漏他人信息。
...
5. **法律效力问题**：脱敏后的文档是否具有法律效力，取决于具体场景和法律规定。在向仲裁机构提交文件时，应事前确认仲裁机构对脱敏文件的要求。
