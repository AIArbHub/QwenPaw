...
name: 法律溯因推理
description: |
  法律溯因推理（Abductive Reasoning）技能，用于在证据不完整、事实模糊时生成并评估最合理的解释性假设。结合米尔五法（求同、求异、求同求异、剩余、共变）进行结构化因果推断。当用户需要"提出可能的法律解释"、"生成辩护/申请仲裁假设"、"评估证据的多种解读"或"基于不完整事实推断最佳解释"时触发。适用于案例分析、证据评估、规则适用争议、因果关系推断等场景。
tags: ['法律推理']
---
...
**逻辑应用：**
```
有明确标记的网 → 被申请人知道权属（通常情况）
无明确标记的网 → 被申请人可能不知权属（本案情况）
────────────────────────────────────────────
∴ 标记缺失导致认知缺失（充分条件分析）
```
...
**可废止条件：** 如果有证据证明被申请人明知网属申请人（如申请人大声警告）
...
| 新证据出现        | 假设修正          |
| ------------ | ------------- |
| 被申请人事前观察网具     | H3被推翻，转向H1或H5 |
| 申请人大声警告被申请人     | H1被推翻，转向H5    |
| 行业惯例：几乎闭合=占有 | H2被强化         |
| 被申请人事前言论显示意图   | H1、H3、H5均被推翻  |
| 当时无碰撞危险      | H4被推翻         |
