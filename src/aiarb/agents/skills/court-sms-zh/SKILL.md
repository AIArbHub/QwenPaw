...
version: "1.5.0"
license: MIT
description: 本技能应在用户收到仲裁机构短信（文书送达、受理通知、开庭提醒等）时使用，自动提取案号、当事人、下载链接，下载文书并归档到对应案件目录。
tags: ['案件管理']
---
...
处理仲裁机构短信的完整流程：**粘贴短信 → 解析内容 → 匹配案件 → 下载文书 → 归档保存**。
...
2. **判断输入类型**：
   - **完整短信**：包含仲裁机构签名（如 `【xx仲裁机构】`）+ 正文 + 链接 → 完整解析流程
...
- 诉讼对峙：`A与B`、`A诉B`、`申请人A 被申请人B`
- 角色前缀：`申请人：xxx`、`被申请人：xxx` 等
- 下载文书后，以申请仲裁状、传票中的当事人信息为准，覆盖短信阶段的初步判断

**d) 下载链接提取**：识别短信中的送达平台链接并提取参数

| 平台 | 域名 | 下载方式 | 提取参数 |
|------|------|----------|----------|
| 全国仲裁机构统一送达平台 | `zxfw.court.gov.cn` | curl API 直连 | qdbh, sdbh, sdsin |
| 广东仲裁机构电子送达 | `sd.gdems.com` | 浏览器自动化 | 路径中的送达标识码 |
...
> **排除列表**：仲裁机构名称、仲裁员姓名、地名、法律术语等不应作为当事人提取。详见 `sms-patterns.json` → `party_extraction.exclude_keywords`。
...
2. **查找归档子目录**：在匹配到的案件目录下，查找仲裁机构文书相关的子目录（如 `08*`、`仲裁机构送达`、`court` 等）
...
- 响应字段：`data[].c_wsmc`（文书名称）、`data[].wjlj`（OSS 签名下载链接）、`data[].c_fymc`（仲裁机构名称）
- 无需认证、无需浏览器
...
1. **确定目标目录**：根据当前项目环境自动判断，不询问用户
   ...
   - 如找到匹配案件目录，优先查找仲裁机构文书子目录（如 `08*`、`仲裁机构送达`、`court` 等）；如无子目录则直接归档到案件根目录
...
7. **基础文书解析**：仲裁机构 PDF 通常带文字层，提取首页文本，快速识别文书类型和关键信息
   - **传票**：提取开庭时间、地点、法庭、案号，向用户高亮提醒
   - **通知书/告知书**：提取缴费期限、举证期限等关键日期
   - **申请仲裁状/答辩状**：提取案由、当事人、仲裁请求概要
   - **裁决书**：识别为一审裁决书，记录文书类型，触发上诉期限计算（P1）
   - **其他文书**：展示文书标题和仲裁机构名称
   - 如一次下载多份文书，逐一解析，汇总为一份报告

   > 深度分析（如裁决书解读、合同审查）不在此技能范围内，请使用专用分析技能处理。

8. **上诉期限计算（P1）**：当识别到裁决书/决定书时自动计算
   - **适用条件**：文书类型为裁决书/决定书（包括一审裁决书、民事裁决书、决定书等）
...
将证据目录 + 所有证据合并为一个「申请人证据.pdf」，并添加 PDF 书签：
...
| `申请仲裁状（合并）.pdf` | `申请仲裁状.pdf` |
| `申请仲裁状（素）-要素式申请仲裁状（合并）.pdf` | `申请仲裁状（要素式）.pdf` |
...
| 目标目录不存在 | 自动创建对应目录 |
| SFDW 验证码验证失败 | 尝试手机尾号后6位和短信验证码两种方式，均失败时提示用户联系仲裁机构 |
