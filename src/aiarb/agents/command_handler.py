...
class CommandHandler(ConversationCommandHandlerMixin):
    ...
    async def _process_compact(
        ...
        Delegates to the framework's native ``Agent.compress_context()``.
        ...
        compress_stats: dict = {}
        try:
            # Agent-backed mode: ``AIArbAgent.compress_context`` already
            # routes to scroll or native by itself. Standalone mode builds a
            # bare the framework ``Agent`` whose ``compress_context`` is always
    ...
    async def _get_current_system_prompt(self) -> str:
        ...
        In agent-backed mode, ask the framework for its dynamic system prompt
