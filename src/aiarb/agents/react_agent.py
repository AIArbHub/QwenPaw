# -*- coding: utf-8 -*-
"""AIArb Agent - Main agent implementation.

This module provides the main AIArbAgent class built on ReActAgent,
...
class AIArbAgent(CodingModeMixin, Agent):
    """AIArb Agent with integrated tools, skills, and memory management.
    ...
    def __init__(
        ...
        *,
        ...
        # Bypass agentscope's built-in permission engine — aiarb uses
    ...
    def load_state_dict(self, state_dict: dict, strict: bool = True) -> None:
        ...
        if isinstance(memory_raw, dict):
            from aiarb.app.chats.utils import parse_legacy_memory_state
    ...
    def _set_formatter_media_strip(self, enabled: bool) -> None:
        ...
        if formatter is None:
            return
        setattr(formatter, "_aiarb_force_strip_media", enabled)
    ...
    def _set_formatter_audio_strip(self, enabled: bool) -> None:
        ...
        if formatter is None:
            return
        setattr(formatter, "_aiarb_force_strip_audio", enabled)
    ...
    def _last_wire_request_had_media(self) -> bool:
        ...
        if formatter is None:
            return False
        count = getattr(formatter, "_aiarb_last_wire_media_count", 0)
        return (
    ...
    def _last_wire_request_had_audio(self) -> bool:
        ...
        if formatter is None:
            return False
        count = getattr(formatter, "_aiarb_last_wire_audio_count", 0)
        return (
            isinstance(count, int)
            and not isinstance(count, bool)
            and (count > 0)
    ...
    def _attach_fallback_notices(
        ...
        ``aiarb_model_fallbacks``/``aiarb_actual_model`` from event
        or message metadata; this is the only point where AIArb still
        ...
        if not isinstance(metadata, dict):
            ...
            try:
                ...
                return
        metadata["aiarb_model_fallbacks"] = [
            dict(event) for event in sink["events"]
        ]
        if sink.get("actual_model"):
            metadata["aiarb_actual_model"] = dict(sink["actual_model"])
    ...
    def _is_explicit_media_capability_error(exc: Exception) -> bool:
        ...
        if AIArbAgent._is_content_safety_error(exc):
            return False
