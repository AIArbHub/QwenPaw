...
from aiarb.framework.agent import Agent, ReActConfig
from aiarb.framework.event import (
    ModelCallEndEvent,
    TextBlockDeltaEvent,
    TextBlockEndEvent,
    TextBlockStartEvent,
...
class AIArbAgent(CodingModeMixin, Agent):
    ...
    This agent extends the framework 2.0 ``Agent`` with:
    ...
    def __init__(
        ...
        *,
        ...
        # native the framework compression (see compress_context /
        # _save_to_context).
        ...
        # Bypass the framework's built-in permission engine — aiarb uses
        # its own PolicyGuardedTool.check_permissions for tool-guard.
        from aiarb.framework.permission import PermissionMode
    ...
    async def compress_context(
        ...
        compression. Otherwise fall back to the framework's native path, gated on
