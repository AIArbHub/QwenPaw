...
- This **attaches to an external browser**, not a new process launched by AIArb
...
### 1. Managed CDP launched by AIArb
...
These browsers are launched and owned by AIArb. `stop` will:
...
These browsers are not launched by AIArb. `stop` will only:
