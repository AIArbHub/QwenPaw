...
WORKTREE_REL = Path(".aiarb") / "worktrees"
REGISTRY_REL = Path(".aiarb") / "fork_registry.json"
...
INTEGRATION_PROJECT_REL = Path(".aiarb") / "fork_integration_project"
ACTIVE_SCOPE_REL = Path(".aiarb") / "fork_active_scope"
...
def project_dir_from_worktree(worktree_path: Path) -> Path:
    """``<project>/.aiarb/worktrees/<id>`` → ``<project>``."""
...
def _fork_finalize_lock_path(project_dir: Path, branch: str) -> Path:
    ...
    return project_dir / ".aiarb" / "fork_locks" / f"{digest}.lock"
...
def _ensure_git_identity(wt: Path) -> None:
    ...
    Disabled when ``AIARB_FORK_AUTO_GIT_IDENTITY=0``. Prefers existing
    repo/global git config and does not overwrite a configured identity.
    """
    if os.environ.get("AIARB_FORK_AUTO_GIT_IDENTITY", "1") == "0":
        return
    for key, value in (
        ("user.email", "aiarb-fork@localhost"),
        ("user.name", "AIArb Fork"),
    ):
