...
Implements the the framework ``Offloader`` protocol so the native
...
if TYPE_CHECKING:
    from aiarb.framework.message import Msg, ToolResultBlock

logger = logging.getLogger(__name__)


class AIArbOffloader:
    ...
    * ``offload_tool_result`` — tool output to
    ...
    async def offload_context(
        ...
        ``session_id`` is required by the the framework ``Offloader``
        protocol but intentionally unused here: AIArb archives
