...
def get_aiarb_version() -> str:
    """Return the installed AIArb package version, or ``'unknown'``."""
    try:
...
def generate_backup_id() -> str:
    ...
    Format: ``aiarb-{version}-{YYYYMMDDTHHmmssZ}-{short8}``

    Example: ``aiarb-1.2.3-20260420T093000Z-ab12cd34``
    """
    ver = re.sub(r"[^a-zA-Z0-9._-]", "_", get_aiarb_version())
    ts = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
    short = uuid4().hex[:8]
    return f"aiarb-{ver}-{ts}-{short}"
...
def finalize_backup_meta(meta, agent_count: int) -> None:
    ...
    meta.aiarb_version = get_aiarb_version()
