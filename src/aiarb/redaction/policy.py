...
class RedactionMode(str, Enum):
    ...
    # LegalWork A5 新增策略
    GENERALIZE = "generalize"       # G 泛化：具体值→类别描述
    SHIELD = "shield"             # S 屏蔽：删除整段，标注 [已屏蔽]
    RANDOMIZE = "randomize"         # R 随机置换：同类池随机选取
    DATE_SHIFT = "date_shift"       # D 日期偏移：±N天随机偏移


class RedactionStrengthLevel(str, Enum):
    """脱敏强度分级（借鉴 LegalWork 的 L1-L4 体系）。

    每个级别是一个策略组合模板，自动映射到实体类型的脱敏模式。
    """
    L1_HIGH = "L1"        # 高度：FULL_MASK + SHIELD + GENERALIZE
    L2_MEDIUM = "L2"      # 中度：PARTIAL_MASK + SHIELD
    L3_LIGHT = "L3"       # 轻度：TOKENIZE + (加密映射)
    L4_NOMINAL = "L4"     # 名义：RANDOMIZE + DATE_SHIFT


# 强度级别 → 默认实体策略映射
_STRENGTH_LEVEL_DEFAULTS: Dict[str, Dict[str, RedactionMode]] = {
    RedactionStrengthLevel.L1_HIGH.value: {
        "person_name": RedactionMode.FULL_MASK,
        "company_name": RedactionMode.GENERALIZE,
        "party_alias": RedactionMode.SHIELD,
        "id_number": RedactionMode.FULL_MASK,
        "phone_number": RedactionMode.FULL_MASK,
        "email": RedactionMode.FULL_MASK,
        "address": RedactionMode.GENERALIZE,
        "bank_account": RedactionMode.FULL_MASK,
        "case_number": RedactionMode.GENERALIZE,
        "amount": RedactionMode.GENERALIZE,
        "date": RedactionMode.KEEP,
        "license_plate": RedactionMode.FULL_MASK,
        "arbitrator_name": RedactionMode.FULL_MASK,
        "lawyer_name": RedactionMode.FULL_MASK,
        "witness_name": RedactionMode.FULL_MASK,
    },
    RedactionStrengthLevel.L2_MEDIUM.value: {
        "person_name": RedactionMode.PARTIAL_MASK,
    ...
    RedactionStrengthLevel.L3_LIGHT.value: {
    ...
    RedactionStrengthLevel.L4_NOMINAL.value: {
        "person_name": RedactionMode.RANDOMIZE,
        "company_name": RedactionMode.RANDOMIZE,
        "party_alias": RedactionMode.KEEP,
        "id_number": RedactionMode.PARTIAL_MASK,
        "phone_number": RedactionMode.PARTIAL_MASK,
        "email": RedactionMode.PARTIAL_MASK,
        "address": RedactionMode.DATE_SHIFT,
        "bank_account": RedactionMode.PARTIAL_MASK,
        "case_number": RedactionMode.KEEP,
        "amount": RedactionMode.KEEP,
        "date": RedactionMode.DATE_SHIFT,
    },
}
...
class RedactionPolicyEngine:
    ...
    def __init__(self, policy_name: str = "external_client",
        ...
        Args:
            policy_name: 预定义策略名（external_client / internal_legal_analysis /
                         public_release / arbitration_internal / arbitration_publish）。
            custom_rules: 自定义实体→模式覆盖。
            strength_level: 强度级别（L1/L2/L3/L4），如果提供则覆盖 policy_name
                           的默认规则，使用 _STRENGTH_LEVEL_DEFAULTS 中的组合。
        """
        self.policy_name = policy_name
        self.strength_level = strength_level
        self.rules = self._load_rules(policy_name, custom_rules, strength_level)
        ...
        self._random_pool = self._init_random_pool()

    def _load_rules(self, policy_name: str,
                    custom_rules: Optional[Dict[str, RedactionMode]],
                    strength_level: Optional[str] = None) -> Dict[str, RedactionMode]:
        """加载策略规则。

        优先级：strength_level > custom_rules > policy_name 预设。
        """
        # 优先使用 strength_level 组合
        if strength_level and strength_level in _STRENGTH_LEVEL_DEFAULTS:
            base_rules = _STRENGTH_LEVEL_DEFAULTS[strength_level].copy()
        else:
    ...
    def apply(self, entity: Any) -> str:
        ...
        # LegalWork A5 新增策略
        if mode == RedactionMode.GENERALIZE:
            return self._generalize(original_text, entity_type)

        if mode == RedactionMode.SHIELD:
            return self._shield(original_text)

        if mode == RedactionMode.RANDOMIZE:
            return self._randomize(original_text, entity_type, cluster_id)

        if mode == RedactionMode.DATE_SHIFT:
            return self._date_shift(original_text)
    ...
    def _generalize(self, text: str, entity_type: str) -> str:
        """泛化策略 (G)：将具体值替换为上级类别描述。

        示例：
        - 地址 "朝阳区建国路88号" → "某市某区某街道"
        - 金额 "1,234,567.89元" → "大额（约123万元）"
        - 案号 "(2024)京仲案字第1234号" → "仲裁案号"
        - 公司名 "北京小米科技有限公司" → "某科技公司"
        """
        if entity_type == "address":
            return "某市某区某街道"
        if entity_type == "company_name":
            # 保留行业线索
            for industry in ["科技", "金融", "建设", "贸易", "投资", "教育", "医疗"]:
                if industry in text:
                    return f"某{industry}公司"
            return "某公司"
        if entity_type == "party_alias":
            return "某方当事人"
        ...
        if entity_type == "amount":
            ...
            if m:
                try:
                    ...
                    if "亿" in text:
                        return "大额（亿元级）"
                    if "万" in text or num >= 100000:
                        return "大额（约{:,.0f}万元）".format(num / 10000)
                    if num >= 10000:
                        return "中额（约{:,.0f}万元）".format(num / 10000)
                    return "小额（约{:.0f}元）".format(num)
                except ValueError:
                    pass
            return "某金额"
        if entity_type == "license_plate":
            return "某地车牌"
        # 默认：保留类型标注
        return f"[已泛化: {entity_type}]"

    def _shield(self, text: str) -> str:
        """屏蔽策略 (S)：删除整段敏感文本，标注屏蔽字数。

        与 FULL_MASK 的区别：FULL_MASK 用 █ 替换，保持长度；
        SHIELD 删除内容并标注屏蔽了多长。
        """
        return f"[已屏蔽: {len(text)}字]"

    def _randomize(self, text: str, entity_type: str,
                   cluster_id: Optional[str] = None) -> str:
        """随机置换策略 (R)：从同类实体池中随机选取替代值。

        与 TOKENIZE 的区别：TOKENIZE 生成语义化 token（某米科技），
        RANDOMIZE 从随机池中选取一个完全不同的值（某讯科技）。
        """
        # cluster 级缓存
        if cluster_id and cluster_id in self._replace_pool:
            return self._replace_pool[cluster_id]
        ...
        pool = self._random_pool.get(entity_type, [])
        if not pool:
            # 没有随机池，降级为 tokenize
            return self._tokenize(text, entity_type, cluster_id)

        import random
        result = random.choice(pool)

        # 避免与原文相同
        if result == text and len(pool) > 1:
            result = random.choice([x for x in pool if x != text])
    ...
    def _date_shift(self, text: str) -> str:
        """日期偏移策略 (D)：对日期做 ±N 天随机偏移，保持格式不变。

        示例：
        - "2024年3月15日" → "2024年3月12日" (偏移 -3 天)
        - "2024年6月" → "2024年5月" (偏移 -1 月)
        """
        import re
        import random
        ...
        # 匹配各种日期格式
        patterns = [
            (re.compile(r"(\d{4})年(\d{1,2})月(\d{1,2})日"), "day"),
            ...
            def replacer(match):
                if shift_type == "day":
                    ...
                    try:
                        ...
                        return f"{new_dt.year}年{new_dt.month}月{new_dt.day}日"
                    except ValueError:
    ...
    def _init_random_pool(self) -> Dict[str, List[str]]:
        """初始化随机置换池。
