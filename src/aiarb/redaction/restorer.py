...
class RedactionRestorer:
    ...
    def __init__(self, mapping_path: Optional[str] = None,
        ...
        Args:
            mapping_path: 映射文件路径（优先使用）
            mapping_json: 映射文件 JSON 字符串（当 mapping_path 不可用时使用）
        """
        ...
        if mapping_path:
            ...
            self.load_mapping_from_string(mapping_json)
    ...
    def load_mapping_from_string(self, mapping_json: str) -> bool:
        """从 JSON 字符串加载映射数据。

        兼容 base64 编码格式。
        """
        try:
            content = mapping_json.strip()
            ...
            print(f"[RedactionRestorer] 从字符串加载映射失败: {e}")
            return False
