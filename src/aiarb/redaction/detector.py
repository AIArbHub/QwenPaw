...
class RedactionDetector:
    ...
    ENTITY_TYPES = [
        ...
        # 仲裁特有实体类型
        "arbitrator_name",  # 仲裁员姓名
        "lawyer_name",      # 律师/代理人姓名
        "witness_name",    # 证人姓名
        "amount",           # 金额（可保留或脱敏）
        "date",             # 日期（可保留或偏移）
        "evidence_id",     # 证据编号
    ...
    def _detect_party_aliases(self, text: str,
        ...
        role_alias_pattern = re.compile(
            ...
            r"申请人|被申请人|反请求申请人|反请求被申请人|"
            r"申请人一|申请人二|被申请人一|被申请人二)"
        )
    ...
    def _init_regex_patterns(self) -> Dict[str, List[Dict]]:
        ...
        return {
            ...
            "amount": [
                {
                    # 金额：人民币XX元/XXX万元
                    "pattern": re.compile(r"[\u4eba\u4e3a\u6309]?\d{1,3}(?:,\d{3})*(?:\.\d{1,2})?\u5143"),
                    "confidence": 0.75,
                },
                {
                    # 大写金额：人民币\u62fe\u4f0d\u4eb0\u4eb0\u5143
                    "pattern": re.compile(r"[\u4eba\u4e3a\u6309]?[\u58f9\u8d30\u53c1\u8086\u4f0d\u9646\u67d2\u6377\u634c\u7396][\u4eb0\u4f70\u62fe\u4f0d\u4eb0\u4e07\u4ebf]*\u5143"),
                    "confidence": 0.70,
                },
            ],
            "date": [
            ...
            "evidence_id": [
                {
                    # 证据编号：证据一/证据1/证1/Exhibit 1
                    "pattern": re.compile(r"\u8bc1\u636e[\u4e00-\u9faf\d]{1,3}\u53f7?"),
                    "confidence": 0.80,
                },
                {
                    "pattern": re.compile(r"\u8bc1\d{1,3}\u53f7?"),
                    "confidence": 0.80,
                },
            ],
            "alipay_id": [
            ...
            "case_number": [
                ...
                # 仲裁案号格式
                {
                    # 北京仲裁委/贸仲等：京仲案字第XXXX号
                    "pattern": re.compile(r"[京津沪深]\u4ef2\u6848\u5b57\u7b2c\d{4,6}\u53f7"),
                    "confidence": 0.90,
                },
                {
                    # 中国贸仲：CIETAC + 年份 + 编号
                    "pattern": re.compile(r"[A-Z]{4,6}\d{4}\d{4,6}\u53f7"),
                    "confidence": 0.85,
                },
                {
                    # 普通仲裁案号：XXXX年XX字第XXXX号
                    "pattern": re.compile(r"\d{4}\u5e74[\u4e00-\u9faf]{1,4}\u5b57\u7b2c\d{3,6}\u53f7"),
                    "confidence": 0.85,
                },
            ],
        }
    ...
    def _get_detection_method(self, entity_type: str) -> str:
        ...
        regex_types = [
            ...
            "case_number", "amount", "date", "evidence_id",
        ]
        dictionary_types = ["company_name", "address"]
        ner_types = [
            "person_name", "trade_secret", "medical_info", "minor_info",
            "arbitrator_name", "lawyer_name", "witness_name",
        ]
