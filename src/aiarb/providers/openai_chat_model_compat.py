...
from aiarb.local_models.tag_parser import (
...
class OpenAIChatModelCompat(OpenAIChatModel):
    ...
    * ``provider_id`` — canonical AIArb provider identity used to scope
    ...
    * ``extra_generate_kwargs`` — merged into every ``_call_api`` invocation
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        credential_provider_id = credential_id.removeprefix("aiarb-")
        if (
            provider_id
        ...
        self._aiarb_provider_id = provider_id or credential_provider_id

    @property
    def aiarb_provider_id(self) -> str:
        """Canonical provider ID used for provider-scoped metadata."""
        return self._aiarb_provider_id
    ...
    def bind_aiarb_provider_id(self, provider_id: str) -> None:
        ...
        outside AIArb's factory.  Inside the factory, the resolved provider
        instance is authoritative for metadata, accounting, and retry keys.
        """
        if not provider_id:
            raise ValueError("provider_id must not be empty")
        self._aiarb_provider_id = provider_id
    ...
    async def _relay_stream_tool_call_extras(
        ...
        ``ToolCallBlock``, but AIArb's transient attribute intentionally is
            ...
            if sanitized_response.extra_contents:
                for block in parsed.content:
                    ...
                    if ec:
                        attach_transient_tool_call_extra(
                            block,
                            provider_id=self._aiarb_provider_id,
                            extra_content=ec,
                        )
