# -*- coding: utf-8 -*-
"""Legacy session deserialization for the framework 1.x payloads.
...
def msg_from_dict(data: Mapping[str, Any]) -> Any:
    """Build an :class:`aiarb.framework.message.Msg` from a saved dict.

    Handles both 1.x (``timestamp``) and 2.0 (``created_at``) shapes.
    Skips fields that are not recognised so partially-corrupt fixtures
    don't break loading.
    """
    from aiarb.framework.message import Msg  # local import to ease shim usage
    ...
    # messages to text/data blocks.  1.x aiarb routinely stored
