...
from ....config.context import get_current_workspace_dir, get_current_work_dir
...
_TOOL_FILE_PARAMS: dict[str, tuple[str, ...]] = {
    ...
    # framework built-ins (may be enabled by users)
    "view_text_file": ("file_path", "path"),
    "write_text_file": ("file_path", "path"),
}

_SECRET_DIR_CURRENT_NAME = ".aiarb.secret"
_SECRET_DIR_LEGACY_NAME = ".aiarb.secret"
_SECRET_DIR_LEGACY2_NAME = ".copaw.secret"
...
_COMPAT_SECRET_DIRS: tuple[str, ...] = (
    ...
    _with_platform_trailing_sep(Path.home() / _SECRET_DIR_LEGACY2_NAME),
)
...
def _workspace_root() -> Path:
    ...
    Prefers work_dir (where tools write conversation documents) so that
    the guardian checks the same paths tools actually resolve to.
    """
    return Path(
...
def _is_file_guard_enabled() -> bool:
    ...
    try:
        from aiarb.config import load_config
