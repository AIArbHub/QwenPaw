# -*- coding: utf-8 -*-
"""Framework runtime adapter for Driver capabilities.
...
framework ``ToolBase`` instances for the agent runtime.
"""
...
from aiarb.framework.permission import PermissionBehavior, PermissionDecision
from aiarb.framework.tool import ToolBase, ToolChunk
...
class DriverCapabilityTool(ToolBase):
    """Expose one Driver capability as a framework ToolBase instance."""
...
async def build_driver_agent_tools(
    ...
    """Build framework tools and prompt hints from active Drivers.
    ...
    try:
        ...
        logger.debug(
            "Failed to build framework tools from Driver capabilities",
            exc_info=True,
        )
        return [], []
