...
This module provides drop-in replacements for the framework's MCP clients
that solve the CPU leak issue caused by cross-task context manager exits.

The issue occurs when using the framework's StatefulClientBase in uvicorn/FastAPI:
...
class _MCPClientMixin:
    ...
    async def list_tools(self):
        ...
        flaky MCP client from killing the user's turn — the framework's
...
class StdIOStatefulClient(_MCPClientMixin):
    ...
    Drop-in replacement for the framework.mcp.StdIOStatefulClient that solves
    ...
    API-compatible with the framework.mcp.StdIOStatefulClient for drop-in
    replacement.
    """
...
class HttpStatefulClient(_MCPClientMixin):
    ...
    Drop-in replacement for the framework.mcp.HttpStatefulClient that solves
    the CPU leak issue by running the entire context manager lifecycle in
    a single dedicated background task.
