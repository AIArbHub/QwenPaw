...
from aiarb.pawapp import PawApp, get_ctx
from aiarb.pawapp.task import SSEChannel

logger = logging.getLogger(__name__)

# ── Storage (shared by HTTP routes and agent tools) ──────────────────
# Use a writable directory under the user's working dir (~/.aiarb/) so
# the plugin can persist data even when shipped as a read-only built-in.
def _resolve_data_dir() -> Path:
    """Return a writable data directory for this plugin.

    Prefers ``~/.aiarb/app_data/agent-kanban/``; falls back to the
    plugin's bundled ``data/`` directory if the working dir is not
    available (e.g. during unit tests).
    """
    try:
        from aiarb.constant import WORKING_DIR

        data_dir = WORKING_DIR / "app_data" / "agent-kanban"
    except Exception:
...
class _Txn:
    ...
    lock on ``.issues.lock`` guards other ``aiarb app`` processes sharing
...
async def list_kanban_approvals() -> Dict[str, Any]:
    ...
    try:
        ...
        from aiarb.app.approvals.display import (
