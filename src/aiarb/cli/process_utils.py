...
def _matches_aiarb_cli_command(command: str, *subcommands: str) -> bool:
    """Return whether command line looks like a AIArb CLI invocation."""
    lowered = f" {command.lower()}"
    return any(
        pattern in lowered
        for subcommand in subcommands
        for pattern in (
            f" -m aiarb {subcommand}",
            f" aiarb {subcommand}",
            f"__main__.py {subcommand}",
            f'aiarb.exe" {subcommand}',
            f"aiarb.exe {subcommand}",
        )
    )


def _is_aiarb_service_command(command: str) -> bool:
    """Return whether the command line looks like a local AIArb app."""
    return _matches_aiarb_cli_command(command, "app")


def _is_aiarb_wrapper_process(name: str, command: str) -> bool:
    """Return whether the process looks like a AIArb CLI wrapper."""
    lowered_name = name.lower().removesuffix(".exe")
    return lowered_name == "aiarb" or _matches_aiarb_cli_command(
        command,
        "app",
        "desktop",
    )
...
def _candidate_hosts(host: str | None) -> list[str]:
    """Return host variants that can reach a local AIArb service."""
    if not host:
        return []
