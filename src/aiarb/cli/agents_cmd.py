...
def _check_task_status(
    ...
    try:
        ...
        if status == "finished":
            ...
            click.echo(
                f"  aiarb agents chat --background --task-id {task_id}",
            )
...
def agents_group() -> None:
    ...
    \b
    Examples:
      aiarb agents chat --from-agent bot_a --to-agent bot_b --text "..."
      aiarb agents create --name "Research Bot" --agent-id research_bot
      aiarb agents delete research_bot
      aiarb agents list
    """
...
def list_agents(ctx: click.Context, base_url: Optional[str]) -> None:
    ...
    Examples:
      aiarb agents list
      aiarb agents list --base-url http://192.168.1.100:8088
...
def delete_cmd(
    ...
    \b
    AGENT_ID  Configured agent ID, obtainable via `aiarb agents list`.

    \b
    Examples:
      aiarb agents delete research
      aiarb agents delete research --remove-workspace
      aiarb agents delete research --yes
    """
...
def chat_cmd(
    ...
    \b
    Prerequisites:
      1. Use 'aiarb agents list' to discover available agents
      2. Ensure target agent (--to-agent) is configured and running
      3. Use 'aiarb chats list' to find existing sessions (optional)
