# -*- coding: utf-8 -*-
"""Read-only diagnostics for `aiarb doctor` (no config or disk mutations)."""
...
# Log file opened on app startup (see ``aiarb.app._app`` lifespan).
APP_LOG_BASENAME = LOG_FILE_BASENAME

# Built-in local llama.cpp provider id; legacy configs may still use
# copaw-local.
_AIARB_LOCAL_PROVIDER_IDS = frozenset({"aiarb-local", "copaw-local"})
...
def check_app_log_writable() -> tuple[bool, str]:
    ...
    if log_path.exists():
        ...
        return (
            ...
            "(required when starting `aiarb app`)",
        )
...
def environment_summary_lines(
    ...
    *server_python_environment* describes the **HTTP API process** (running
    ``aiarb app``), when ``GET /api/doctor/runtime`` returned
    ...
    lines = [
        ...
        f"aiarb version: {__version__}",
        f"platform: {platform.system()} {platform.machine()}",
        f"doctor_python_environment: {doctor_env}",
    ]
    if server_python_environment is not None:
        lines.append(
            f"aiarb_python_environment: {server_python_environment}",
        )
    else:
        lines.append(
            "aiarb_python_environment: "
            + (server_python_note or "(unknown)"),
        )
    lines.append(f"working_dir: {WORKING_DIR}")
    wd_qp = os.getenv("AIARB_WORKING_DIR") or os.getenv("AIARB_WORKING_DIR")
    wd_legacy = os.getenv("COPAW_WORKING_DIR")
    if wd_qp:
        lines.append(f"AIARB_WORKING_DIR (env): {wd_qp}")
    elif wd_legacy:
...
def windows_environment_lines() -> list[str]:
    """Windows-specific read-only diagnostics for ``aiarb doctor``."""
...
def legacy_single_agent_workspace_note(cfg: Config) -> str | None:
    ...
    When this applies, ``aiarb app`` may run an automatic migration;
    ...
    return (
        ...
        "`aiarb app` uses to trigger legacy → multi-agent workspace "
        "migration. Start `aiarb app` once (or see docs / `aiarb init`). "
...
def browser_automation_notes(cfg: Config | None) -> list[str]:
    ...
    use_default = (
        EnvVarLoader.get_str("AIARB_BROWSER_USE_DEFAULT", "1")
        .strip()
        .lower()
    )
    if use_default in ("0", "false", "no", "off"):
        notes.append(
            "AIARB_BROWSER_USE_DEFAULT is off — browser_use will not "
...
# --- AIArb checks (agent.json, channels, MCP, skills, providers) ---
...
def check_enabled_agents_load_agent_config(cfg: Config) -> tuple[bool, str]:
    """Dry-run :func:`~aiarb.config.config.load_agent_config` for enabled.
    ...
    for agent_id, ref in cfg.agents.profiles.items():
        ...
        if not path.is_file():
            problems.append(
                ...
                "disk; ensure the file exists or use `aiarb doctor fix` "
                "where applicable.",
            )
            continue
        try:
...
def aiarb_local_llm_deep_notes() -> list[str]:
...
def console_static_diagnostic_notes() -> list[str]:
    ...
    from ..utils.console_static import (
        CONSOLE_STATIC_ENV,
        find_aiarb_source_repo_root,
        resolve_console_static_dir,
    )

    notes: list[str] = []
    env_dir = EnvVarLoader.get_str("AIARB_CONSOLE_STATIC_DIR", "").strip()
    if env_dir:
        notes.append(
