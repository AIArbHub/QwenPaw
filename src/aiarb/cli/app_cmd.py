...
def _warn_if_auth_off_non_loopback_bind(host: str, port: int) -> None:
    """Warn when AIArb is reachable beyond loopback without auth."""
    ...
    warning = f"""
============================================================
SECURITY NOTICE: AIArb is bound to {bind_address} without authentication.

Anyone who can reach this address may access AIArb APIs without login.
...
def app_cmd(
    ...
    if workers is not None:
        ...
        click.echo(
            "   AIArb always uses 1 worker for stability. "
    ...
    if reload:
        os.environ["AIARB_RELOAD_MODE"] = "1"
    else:
