...
def hub_cmd(
    ...
    """Run the multi-user AIArb Hub control plane."""
    ...
    try:
        ...
        if exc.name != "docker":
            raise
        raise click.ClickException(
            f"AIArb Hub dependency {exc.name!r} is not installed. "
            f"Install aiarb[hub] to provide {exc.name!r} and try again.",
        ) from exc
