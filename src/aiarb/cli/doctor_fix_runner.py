# -*- coding: utf-8 -*-
"""Conservative filesystem repairs for ``aiarb doctor fix``.
...
``src/aiarb/console/`` (needs network for npm).

``validate-all-jobs-json`` reuses
:func:`~aiarb.cli.doctor_checks.check_cron_jobs_files` (read-only); exits
...
from ..utils.console_static import find_aiarb_source_repo_root
    ...
    *,
        ...
        for fid in fix_ids:
            if fid in RISKY_FIX_IDS and not yes and not dry_run:
                raise ValueError(
                    ...
                    "Use `aiarb doctor fix --dry-run --only ...` to preview "
                    "the plan without -y.",
                )
    ...
    if (
        ...
        raise ValueError(
            ...
            "ensure-working-dir in --only or run `aiarb doctor fix` without "
    ...
    if "rebuild-console-npm" in fix_ids:
        ...
        if repo is None:
            skip_msgs.append(
                "rebuild-console-npm: only in a AIArb source checkout "
