...
class PawApp(App):
    ...
    def __init__(
        ...
        *,
        ...
        # Whether the "aiarb" label has been shown for the current turn.
        self._labeled = False
        ...
        # The model AIArb reports it is using (read-only, for the status bar
    ...
    async def _ensure_turn_label(self) -> None:
        """Mount the ``aiarb`` label once per turn, above the first piece
    ...
    async def _submit(self, text: str) -> None:
        ...
        # new "aiarb" label) is created for the reply.
    ...
    async def _handle_local_command(self, raw: str) -> None:
        ...
        match command:
            ...
            case _:
                # Everything else (including AIArb's own slash commands such
    ...
    async def _resume_session(self, session_id: str) -> None:
        ...
        self._set_terminal_title(f"AIArb {session_id[:8]}")
    ...
    def _on_connected(self, ev: Connected) -> None:
        ...
        self._status().set(
            ...
            model=ev.model or "—",
            aiarb_version=ev.aiarb_version or "—",
        ...
        self._set_terminal_title(f"AIArb {str(ev.session_id)[:8]}")
    ...
    async def _dispatch(self, event) -> None:
        if isinstance(event, TextDelta):
            ...
            self._set_terminal_title(f"AIArb {event.title}")
    ...
    def _no_response_text(self, stop_reason: str | None) -> str:
        ...
        return (
            ...
            "may be misconfigured in AIArb — check it with `aiarb "
            "doctor` or `aiarb models config-key`."
        )
...
def _local_commands() -> list[SlashCommand]:
    ...
    AIArb's and are forwarded to the agent, not listed here."""
    commands = [
        SlashCommand("help", "show AIArb TUI shortcuts"),
