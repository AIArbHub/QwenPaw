# -*- coding: utf-8 -*-
"""Launch the AIArb TUI.

``aiarb``                    open an interactive chat with the active agent
``aiarb tui``                same, with explicit options
``aiarb tui --agent NAME``   chat with a specific agent
``aiarb tui --resume ID``    resume a previous session and continue it

The TUI spawns ``aiarb acp`` using the *current* interpreter
(``python -m aiarb acp --local-diagnostics``), so it always drives the same
install/venv it ships in -- no reliance on ``aiarb`` being on ``PATH``.

Textual and the transport are imported lazily so ``aiarb --help`` and other
...
def _build_transport(
    *,
    ...
    ``[sys.executable, "-m", "aiarb", "acp", "--local-diagnostics"]`` --
    ...
    *not* appended here: ``AcpTransport`` appends ``--agent <id>`` itself when
    ...
    description = (
        f"aiarb acp ({sys.executable} -m aiarb acp --local-diagnostics)"
    )
    if project_dir:
...
def tui_cmd(
    ...
    """Open the AIArb terminal chat UI."""
