# -*- coding: utf-8 -*-
"""Default TUI transport: drive ``aiarb acp`` over ACP/stdio.

The transport spawns ``aiarb acp`` as a subprocess and connects an ACP
client to it (the same ``spawn_agent_process`` plumbing the in-process ACP
service uses, see ``aiarb/agents/acp/service.py``). All agent capabilities —
...
hooks for the AIArb agent, so we implement exactly those.
"""
...
_WARMUP_PROMPT = (
    "Warm up the AIArb backend for an interactive terminal session. "
    "Reply with exactly: ready. Do not call tools."
)

# ``_meta`` flag marking the warmup session ephemeral so the AIArb ACP server
...
def _kill_process_tree(pid: int) -> None:
    ...
    try:
        import psutil  # local import; psutil is already a AIArb dep
...
class _TuiClient:
    ...
    async def ext_notification(
        ...
        if method in ("aiarb/push_message", "session/push_message"):
...
class AcpTransport:
    """Spawns ``aiarb acp`` and drives one session over ACP/stdio."""
    ...
    def __init__(
        ...
        *,
        ...
        # Default: re-invoke this very interpreter as `python -m aiarb acp`.
    ...
    async def start(self) -> Connected:
        ...
        return Connected(
            ...
            model=model,
            aiarb_version=_agent_version(initialized),
    ...
    async def list_sessions(self) -> list[SessionSummary]:
        if self._conn is None:
            raise RuntimeError("transport not started")
        # No cwd filter: AIArb runs every session in its single workspace
...
# ``_meta`` key AIArb sets on the session response with the resolved agent
# id; mirrors the ACP server's ``ACP_AGENT_META_KEY``.
_AGENT_META_KEY = "aiarb.agent"
