    ...
    try:
        ...
        click.echo(
            click.style(
                ...
                "Install with: pip install 'aiarb[local]'",
                fg="red",
            ),
        )
...
def configure_llm_slot_interactive(*, use_defaults: bool = False) -> None:
    ...
    if not eligible:
        if use_defaults:
            click.echo(
                "No LLM provider configured. Run 'aiarb models config' "
                "to configure later.",
            )
            return
        click.echo(
    ...
    if not model and use_defaults:
        click.echo(
            ...
            "Run 'aiarb models config' to set one.",
        )
        return
    try:
    ...
    for defn in _all_provider_objects(manager):
        ...
        if defn.is_local:
            ...
            if all_models:
                ...
                click.echo("  Use 'aiarb models download' to add models.")
        else:
...
def add_provider_cmd(
    ...
    click.echo(
        "  Run 'aiarb models add-model' to add models, "
        "then 'aiarb models config-key' to set the API key.",
    )
...
def download_cmd(
    ...
    \b
    Examples:
      aiarb models download TheBloke/Mistral-7B-Instruct-v0.2-GGUF
    ...
    click.echo(
        ...
        "  aiarb models set-llm  (select 'aiarb-local' provider)",
    )
