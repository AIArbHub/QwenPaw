...
from .process_utils import (
    ...
    _is_aiarb_service_command,
    _process_table,
)

_PYPI_JSON_URL = "https://pypi.org/pypi/aiarb/json"
...
class InstallInfo:
    """Information about the current AIArb installation."""
...
class RunningServiceInfo:
    """Detected AIArb service endpoint state."""
...
def _fetch_pypi_release_data() -> dict[str, Any]:
    """Fetch aiarb release metadata from PyPI."""
    try:
        resp = httpx.get(
            _PYPI_JSON_URL,
            timeout=10.0,
        ...
        raise click.ClickException(
            f"Failed to fetch the latest AIArb version from PyPI: {exc}",
        ) from exc
    except json.JSONDecodeError as exc:
        raise click.ClickException(
            "Received an invalid response from PyPI when checking for the "
            f"latest AIArb version: {exc}",
        ) from exc
    if not isinstance(data, dict):
        raise click.ClickException(
            "Received an invalid response from PyPI when checking for the "
            "latest AIArb version.",
        )
    return data
...
def _select_latest_version(
    ...
    *,
    ...
    if not candidates:
        raise click.ClickException(
            "Unable to determine the latest stable AIArb version.",
        )
    return str(max(candidates))


def _fetch_latest_version(*, include_prerelease: bool = False) -> str:
    """Fetch the latest published AIArb version from PyPI."""
...
def _detect_installation() -> InstallInfo:
    ...
    dist = metadata.distribution("aiarb")
...
def _probe_service(base_url: str) -> RunningServiceInfo:
    """Probe a possible running AIArb HTTP service."""
    try:
        resp = httpx.get(
...
def _process_candidate_ports() -> list[int]:
    """Infer candidate local AIArb service ports from running processes."""
    ports: list[int] = []
    for _pid, command in _process_table():
        if not _is_aiarb_service_command(command):
            continue
...
def _detect_running_service(
    ...
    """Detect whether a AIArb HTTP service is currently running."""
    candidates: list[str] = []
    seen: set[str] = set()
    preferred_hosts: list[str] = []
...
def _running_service_display(running: RunningServiceInfo) -> str:
    ...
    if not running.base_url:
        return "a running AIArb service"
    version_suffix = f" (version {running.version})" if running.version else ""
    return f"AIArb service at {running.base_url}{version_suffix}"


def _confirm_force_shutdown(running: RunningServiceInfo) -> bool:
    """Ask whether `aiarb shutdown` should be used before updating."""
    click.echo("")
    click.secho("!" * 72, fg="yellow", bold=True)
    click.secho(
        "WARNING: RUNNING AIARB SERVICE DETECTED",
        fg="yellow",
        bold=True,
    )
    click.secho("!" * 72, fg="yellow", bold=True)
    ...
    click.secho(
        "Running `aiarb shutdown` will forcibly terminate the current "
        "AIArb backend/frontend processes.",
        fg="red",
        bold=True,
    )
    click.secho(
    ...
    return click.confirm(
        "Run `aiarb shutdown` now and continue with the update?",
        default=False,
    )
...
def _run_shutdown_for_update(
    ...
    """Run `aiarb shutdown` in the current environment before updating."""
    command = [info.python_executable, "-m", "aiarb"]
    ...
    try:
        ...
        raise click.ClickException(
            "Failed to run `aiarb shutdown`: " f"{exc}",
        ) from exc
    ...
    if result.returncode != 0:
        raise click.ClickException(
            "`aiarb shutdown` failed. Please stop the running AIArb "
            "service manually before running `aiarb update`.",
        )
...
def _build_upgrade_command(
    ...
    *,
    ...
    package_spec = f"aiarb=={latest_version}"
...
def _spawn_update_worker(
    ...
    *,
    ...
    worker_code = (
        "from aiarb.cli.update_cmd import run_update_worker; "
        "import sys; "
...
def _run_update_worker_foreground(plan_path: Path) -> int:
    ...
    try:
        ...
        click.echo("")
        click.echo("[aiarb] Update interrupted. Stopping installer...")
...
def run_update_worker(plan_path: str | Path) -> int:
    ...
    click.echo("")
    click.echo(
        "[aiarb] Updating AIArb "
        f"{plan['current_version']} -> {plan['latest_version']}...",
    )
    click.echo(f"[aiarb] Using installer: {plan['installer_label']}")
    ...
    try:
        ...
        click.echo(f"[aiarb] Update failed: {exc}")
        return_code = 1
    finally:
        try:
    ...
    if return_code == 0:
        click.echo("[aiarb] Update completed successfully.")
        click.echo(
            "[aiarb] Please restart any running AIArb service "
            "to use the new version.",
        )
    else:
        click.echo(f"[aiarb] Update failed with exit code {return_code}.")
        click.echo(
            "[aiarb] Please fix the error above and run "
            "`aiarb update` again.",
        )
...
def update_cmd(ctx: click.Context, yes: bool, prerelease: bool) -> None:
    """Upgrade AIArb in the current Python environment."""
    ...
    if version_check is False:
        click.echo("AIArb is already up to date.")
        return
    ...
    if running.is_running:
        if yes:
            raise click.ClickException(
                ...
                "Please stop it before running `aiarb update`, or rerun "
                "without `--yes` to confirm a forced `aiarb shutdown`.",
            )
        ...
        if running.is_running:
            raise click.ClickException(
                f"Detected {_running_service_display(running)} "
                "after `aiarb shutdown`. "
                "Please stop it manually before running `aiarb update`.",
            )

    if not yes and not click.confirm(
        f"Update AIArb to {latest_version} in the current environment?",
        default=True,
    ):
        click.echo("Cancelled.")
        return
