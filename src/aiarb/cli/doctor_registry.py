# -*- coding: utf-8 -*-
"""Extensible hooks for `aiarb doctor`
(entry points + programmatic registration).

Plugins can expose a setuptools entry point in group ``aiarb.doctor``::

    [project.entry-points."aiarb.doctor"]
...
def _load_entry_point_functions() -> list[tuple[str, DoctorNotesFn]]:
    ...
    seen: set[str] = set()
    for group in ("aiarb.doctor", "copaw.doctor"):
