# -*- coding: utf-8 -*-
"""aiarb uninstall — remove the AIArb environment and CLI wrapper."""
...
def _remove_path_entry(profile: Path) -> bool:
    """
    Remove AIArb PATH lines from a shell profile. Returns True if changed.
    """
    if not profile.is_file():
        return False

    text = profile.read_text()
    # Remove the "# AIArb" comment line and the export PATH line
    cleaned = re.sub(
        r"\n?# AIArb\nexport PATH=\"\$HOME/\.aiarb/bin:\$PATH\"\n?",
        "\n",
        text,
    )
    if cleaned == text:
...
def uninstall_cmd(purge: bool, yes: bool) -> None:
    """Remove AIArb environment, CLI wrapper, and shell PATH entries."""
    wd = WORKING_DIR

    if purge:
        click.echo(f"This will remove ALL AIArb data in {wd}")
    else:
        click.echo(
            "This will remove the AIArb Python environment and CLI wrapper.",
        )
    ...
    click.echo("")
    click.echo("AIArb uninstalled. Please restart your terminal.")
