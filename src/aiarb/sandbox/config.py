...
This is the natural entry point into the :mod:`aiarb.sandbox` package:
...
Backend layout (``create_sandbox`` dispatches to these by ``SandboxMode``):
  - SEATBELT     → mod:`aiarb.sandbox.macos_sandbox`      (MacOSSandbox)
  - BUBBLEWRAP   → mod:`aiarb.sandbox.bubblewrap_sandbox` (BubblewrapSandbox)
  - LANDLOCK     → mod:`aiarb.sandbox.linux_sandbox`      (LinuxSandbox)
  - APPCONTAINER → Windows sandbox, dispatched on ``allow_read_all``:
      True  → windows_restricted_sandbox (WindowsRestrictedSandbox)
      False → mod:`aiarb.sandbox.windows_sandbox`            (WindowsSandbox)
  - NONE         → mod:`aiarb.sandbox.local_sandbox`      (NoneSandbox)

Shared base class for all backends:
  - :class:`aiarb.sandbox.local_sandbox.LocalSandbox`
