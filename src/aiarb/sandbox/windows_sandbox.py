...
def _save_container_metadata(
    ...
    Args:
        state_dir: AIArb state directory (``~/.aiarb``).
...
class WindowsSandbox:
    ...
    async def __aenter__(self):
        ...
        if not _is_admin():
            print(
                "[AIArb Sandbox] WARNING: Not running as administrator. "
                "Sandbox ACL setup may fail.",
            )

        fingerprint = _compute_acl_fingerprint(self._config)
        self._container_name = f"aiarb_{fingerprint[:12]}"
        ...
        if existing_sid:
            ...
            print(
                f"[AIArb Sandbox] Reusing existing sandbox "
            ...
            print(
                f"[AIArb Sandbox] Sandbox '{self._container_name}' "
...
def shutdown_cleanup() -> None:
    ...
    For each container metadata file in ~/.aiarb/containers/:
