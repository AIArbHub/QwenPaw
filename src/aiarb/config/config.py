...
class LightContextConfig(BaseModel):
    ...
    strategy: Literal["native", "scroll"] = Field(
        default="scroll",
        description=(
            "Context management strategy. 'native' = the framework compression; "
    ...
    with _BUILTIN_TOOLS_LOCK:
        if _BUILTIN_TOOLS_CACHE is None:
            ...
            try:
                ...
                importlib.import_module("aiarb.agents.tools")
...
class ToolGuardConfig(BaseModel):
    ...
    auto_denied_rules: List[str] = Field(
        ...
        description=(
            ...
            "set env AIARB_TOOL_GUARD_AUTO_DENIED_RULES=none."
        ),
    )
...
def build_fallback_agent_profile_config(
    ...
    Used by :func:`load_agent_config` and ``aiarb doctor fix``
...
def load_agent_config(  # pylint: disable=too-many-branches,too-many-statements
    ...
    with _agent_config_lock:
        ...
        # any in-memory normalization (e.g. ~/.aiarb / ~/.copaw path rewriting) so we
        ...
        # This keeps AIARB_WORKING_DIR effective even if existing agent.json
        # contains older hard-coded paths like "~/.aiarb/media" or "~/.copaw/media".
...
def migrate_legacy_config_to_multi_agent() -> bool:
    ...
    # When AIARB_WORKING_DIR is customized, historical data may still exist
    # under "~/.aiarb" or "~/.copaw".
    _legacy_workspace_dirs = [
        d
        for d in (
            Path("~/.aiarb").expanduser().resolve(),
            Path("~/.copaw").expanduser().resolve(),
        )
        if d.is_dir()
    ]

    for old_workspace in _legacy_workspace_dirs:
...
def _ensure_builtin_arbitrator(config: "Config"):
    ...
    This copies the bundled template from *aiarb.builtins.arbitrator* into
    ...
    try:
        pkg_res = importlib.resources.files("aiarb.builtins.arbitrator")
