...
def _normalize_working_dir_bound_paths(data: object) -> object:
    """Normalize legacy ~/.copaw / ~/.aiarb paths to current WORKING_DIR.

    This keeps AIARB_WORKING_DIR effective even if user config files contain
    older hard-coded paths like "~/.copaw/media" or
    "/Users/x/.aiarb/workspaces/...".
    Only rewrites known working-dir-bound keys.
    """
    legacy_roots = [
        ("~/.copaw", str(Path("~/.copaw").expanduser().resolve())),
        ("~/.aiarb", str(Path("~/.aiarb").expanduser().resolve())),
    ]
...
def get_available_channels() -> Tuple[str, ...]:
    ...
    aiarb.channels), filtered by AIARB_ENABLED_CHANNELS or
    AIARB_DISABLED_CHANNELS when set.

    * AIARB_ENABLED_CHANNELS — whitelist (only these channels are active).
    * AIARB_DISABLED_CHANNELS — blacklist (all channels *except* these).
    * If both are set, AIARB_ENABLED_CHANNELS takes precedence.
    * If neither is set, all discovered channels are returned.
    ...
    raw_enabled = EnvVarLoader.get_str("AIARB_ENABLED_CHANNELS", "").strip()
    if raw_enabled:
...
def is_running_in_container() -> bool:
    ...
    Prefer env AIARB_RUNNING_IN_CONTAINER (1/true/yes) at call time so
...
def get_builtin_plugins_dir() -> Path:
    """Return built-in plugins directory (shipped with the package)."""
    from ..constant import BUILTIN_PLUGINS_DIR

    return BUILTIN_PLUGINS_DIR


def get_all_plugin_dirs() -> list[Path]:
    """Return all plugin search directories in priority order.

    Built-in plugins are searched first so that a user-installed plugin
    with the same id takes precedence (user dir is checked by the loader
    after the built-in dir during discovery, and the loader dedupes by id).
    """
    return [get_builtin_plugins_dir(), get_plugins_dir()]
...
def is_aiarb_running() -> bool:
    """Check if AIArb is currently running by checking API availability.

    Returns:
        True if AIArb is running, False otherwise
    """
    try:
