# -*- coding: utf-8 -*-
"""Fail-closed isolated local process provisioner for AIArb Hub."""
...
class LocalProcessRuntimeProvisioner(RuntimeProvisioner):
    """Run a complete AIArb process tree inside an OS sandbox."""
    ...
    def __init__(
        ...
        *,
        ...
        self._launcher = ThreadPoolExecutor(
            max_workers=1,
            thread_name_prefix="aiarb-runtime-launcher",
        )
        ...
        try:
            with tempfile.TemporaryDirectory(
                ...
                environment = self.runtime_environment(
                    record,
                    {
                        "AIARB_RUNTIME_INTERNAL_TOKEN": "preflight",
                    },
                )
                try:
    ...
    def start(
        ...
        """Start an isolated local AIArb process and wait for readiness."""
        if not credentials.get("AIARB_RUNTIME_INTERNAL_TOKEN"):
            raise RuntimeError(
    ...
    def runtime_environment(
        ...
        environment["AIARB_WORKING_DIR"] = str(record.working_dir)
        environment["AIARB_SECRET_DIR"] = str(record.secret_dir)
        environment["AIARB_BACKUP_DIR"] = str(record.backup_dir)
        environment[
            "AIARB_KEYRING_ACCOUNT"
        ] = f"aiarb-hub-{record.runtime_id}"
        environment["AIARB_RUNTIME_ID"] = record.runtime_id
        environment["AIARB_TENANT_ID"] = record.tenant_id
        runtime_token = credentials.get("AIARB_RUNTIME_INTERNAL_TOKEN")
        if runtime_token:
            environment["AIARB_RUNTIME_INTERNAL_TOKEN"] = runtime_token
