# -*- coding: utf-8 -*-
"""FastAPI control plane and server entry point for AIArb Hub."""
...
def get_hub_root() -> Path:
    ...
    configured = os.environ.get("AIARB_HUB_DIR", "").strip()
    if configured:
...
def create_hub_app(  # pylint: disable=too-many-statements
    ...
    app = FastAPI(title="AIArb Hub", lifespan=lifespan)
    ...
    async def ensure_personal_runtime(user: HubUser) -> RuntimeRecord:
        ...
        if record.state is not RuntimeState.RUNNING:
            try:
                ...
                raise HTTPException(
                    status_code=503,
                    detail=f"Personal AIArb failed to start: {exc}",
                ) from exc
        return record
    ...
    async def restart_own_runtime(
        ...
        try:
            ...
            raise HTTPException(
                status_code=503,
                detail=f"Personal AIArb failed to restart: {exc}",
            ) from exc
        await record_audit(
            user,
            "runtime.restart",
        ...
        try:
            async with httpx.AsyncClient(
                ...
                upstream = await client.get(
                    target,
                    headers={
                        "X-AIArb-Runtime-Token": internal_token,
                    },
                )
        except httpx.HTTPError as exc:
            raise HTTPException(
                status_code=502,
                detail=f"Personal AIArb is unavailable: {exc}",
            ) from exc
        excluded_headers = {
            "connection",
            "content-length",
    ...
    async def personal_runtime_proxy(
        ...
        headers["X-AIArb-Runtime-Token"] = internal_token
    ...
    async def hub_console(path: str) -> Response:
        ...
        return JSONResponse(
            {
                "message": (
                    "AIArb Hub is running, but Console assets are "
                    "unavailable. "
...
def run_hub_app(
    *,
    ...
    """Run the AIArb Hub control plane with safe public-bind defaults."""
    public_bind = not is_loopback_host(host)
    if public_bind and not force_public:
        raise ValueError(
            "AIArb Hub refuses a non-loopback host by default. "
    ...
    if public_bind:
        ...
        warning = (
            "AIArb Hub is accepting network connections at "
