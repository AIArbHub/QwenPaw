...
logger = logging.getLogger("qwenpaw.routers.text_selection")
...
def build_router() -> APIRouter:
    ...
    def status() -> dict[str, Any]:
        ...
        try:
            ...
            return {
                "ok": True,
                "service": "qwenpaw-text-selection",
    ...
    def _notify_desktop_config_change(config: dict[str, Any]) -> None:
        ...
        try:
            ...
            headers = {}
            if token:
                headers["X-QwenPaw-TS-Token"] = token
    ...
    async def _process_with_ai(prompt: str) -> str:
        """Process text using the QwenPaw AI agent infrastructure."""
        # Try to use the agent context if available
        try:
            from qwenpaw.app.chats.runner import (
                _build_chat_request,
            )  # type: ignore[import-untyped]
            from qwenpaw.app.chats.workflow import (
                execute_chat_workflow,
            )  # type: ignore[import-untyped]
            from qwenpaw.app.providers.manager import (
                ProviderManager,
            )  # type: ignore[import-untyped]
