...
def _apply_color_tone(spritesheet_path: Path, tone: str) -> Path:
    """Apply a color tone adjustment to a spritesheet copy. Returns new path."""
    try:
        from PIL import Image, ImageEnhance
    except ImportError:
        return spritesheet_path  # Pillow not available — return original
    ...
    for y in range(h):
        for x in range(w):
            r, g, b, a = pixels[x, y]
            if a == 0 or (r < 30 and g < 30 and b < 30):
                continue
            avg = (r + g + b) // 3
            if avg > 100:
                new_r = min(255, int(r * p["r_mul"] + p["r_add"]))
                new_g = min(255, int(g * p["g_mul"] + p["g_add"]))
                new_b = max(0, int(b * p["b_mul"] + p["b_add"]))
                pixels[x, y] = (new_r, new_g, new_b, a)
            elif avg > 50:
                new_r = min(255, int(r * p["r_mul"] * 0.95 + p["r_add"] * 0.5))
                new_g = min(255, int(g * p["g_mul"] * 0.95 + p["g_add"] * 0.5))
                new_b = max(0, int(b * p["b_mul"] * 0.95 + p["b_add"] * 0.5))
                pixels[x, y] = (new_r, new_g, new_b, a)

    if tone != "natural":
        img = ImageEnhance.Color(img).enhance(1.3)

    out_path = spritesheet_path.parent / f"spritesheet_{tone}.webp"
    img.save(out_path, "WEBP", quality=90)
    return out_path


def build_router() -> APIRouter:
    router = APIRouter(prefix="/qwenpaw-pet", tags=["pet"])
    ...
    def create_pet(payload: CreatePetRequest):
        ...
        import tempfile
        tmp_dir = Path(tempfile.mkdtemp(prefix="qwenpaw-pet-create-"))
        try:
