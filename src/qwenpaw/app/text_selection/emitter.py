...
def _headers() -> dict[str, str]:
    ...
    if not token:
        return {}
    return {"X-QwenPaw-TS-Token": token}
...
def _spawn_host_port_from_env() -> tuple[str, int]:
    url = (os.environ.get("QWENPAW_TS_DESKTOP_URL") or "").strip()
    if url:
        u = urlparse(url)
    ...
    host = (os.environ.get("QWENPAW_TS_DESKTOP_HOST") or _DEFAULT_HOST).strip() or _DEFAULT_HOST
    port = int(os.environ.get("QWENPAW_TS_DESKTOP_PORT", str(_DEFAULT_PORT)))
    return host, port
...
def _pick_listen_port(host: str, preferred: int) -> int:
    if os.environ.get("QWENPAW_TS_DESKTOP_STRICT_PORT", "0") == "1":
        return preferred
    if (os.environ.get("QWENPAW_TS_DESKTOP_URL") or "").strip():
        return preferred
...
def _desktop_url_candidates() -> list[str]:
    ...
    explicit = (os.environ.get("QWENPAW_TS_DESKTOP_URL") or "").strip()
    if explicit:
...
def _spawn_desktop_background_impl() -> tuple[bool, str | None]:
    ...
    try:
        ...
        # Get the main QwenPaw backend URL to pass to the desktop process
        qwenpaw_backend_url = os.environ.get("QWENPAW_TS_BACKEND_URL", "http://127.0.0.1:8023")

        cmd: list[str] = [
            sys.executable,
            "-m",
            "qwenpaw.app.text_selection.desktop",
            "--host",
            host,
            "--port",
            str(port),
...
def _stop_desktop_skip_reason(*, force: bool) -> str | None:
    if os.environ.get("QWENPAW_TS_STOP_ON_SHUTDOWN", "1") == "0":
        return "opted out"
...
def ensure_desktop_available() -> None:
    ...
    if desktop_health():
        ...
        _reset_desktop_reachability_probe()
        return
    if os.environ.get("QWENPAW_TS_AUTOSTART", "0") == "0":
        return
