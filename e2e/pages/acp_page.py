# -*- coding: utf-8 -*-
"""
AIArb ACP (Agent Communication Protocol) page object.
...
class ACPPage(BasePage):
    ...
    # Filter tabs
    FILTER_TABS = '.aiarb-tabs, .aiarb-segmented, [class*="filterTabs"]'
    TAB_ALL = '[class*="tab"]:has-text("All"), [class*="tab"]:has-text("全部"), .aiarb-segmented-item:has-text("All")'
    TAB_BUILTIN = '[class*="tab"]:has-text("Builtin"), [class*="tab"]:has-text("内置"), .aiarb-segmented-item:has-text("Builtin")'
    TAB_CUSTOM = '[class*="tab"]:has-text("Custom"), [class*="tab"]:has-text("自定义"), .aiarb-segmented-item:has-text("Custom")'

    # Create button
    CREATE_BUTTON = 'button:has-text("Create"), button:has-text("创建"), button:has-text("Add"), button:has-text("添加")'

    # ACP card list
    ACP_CARD = '[class*="acpCard"], [class*="ACPCard"], .aiarb-card'
    ACP_CARD_TITLE = '[class*="agentKey"], [class*="title"], .aiarb-card-meta-title'
    ACP_CARD_TAG = '.aiarb-tag'
    ACP_CARD_SWITCH = '.aiarb-switch'

    # ACP drawer (create/edit)
    DRAWER = '.aiarb-drawer'
    DRAWER_TITLE = '.aiarb-drawer-title'
    DRAWER_CLOSE = '.aiarb-drawer-close'
    ...
    FORM_ENABLED_SWITCH = '[class*="enabled"] .aiarb-switch, #enabled'
    FORM_TRUSTED_SWITCH = '[class*="trusted"] .aiarb-switch, #trusted'
    FORM_TOOL_PARSE_MODE = '.aiarb-select, select[id*="tool_parse_mode"]'
    FORM_BUFFER_LIMIT = 'input[id*="buffer"], input[name*="buffer"], input[type="number"]'

    # Drawer action buttons
    SAVE_BUTTON = '.aiarb-drawer button:has-text("Save"), .aiarb-drawer button:has-text("保存"), .aiarb-drawer button.aiarb-btn-primary'
    CANCEL_BUTTON = '.aiarb-drawer button:has-text("Cancel"), .aiarb-drawer button:has-text("取消")'
    DELETE_BUTTON_DRAWER = '.aiarb-drawer button:has-text("Delete"), .aiarb-drawer button:has-text("删除")'
    DOC_LINK = '.aiarb-drawer a[href*="doc"], .aiarb-drawer a[href*="integration"]'

    # Confirmation popups
    POPCONFIRM = '.aiarb-popconfirm, .aiarb-modal-confirm'
    POPCONFIRM_OK = '.aiarb-popconfirm button:has-text("OK"), .aiarb-popconfirm button:has-text("确定"), .aiarb-popconfirm .aiarb-btn-primary'
    POPCONFIRM_CANCEL = '.aiarb-popconfirm button:has-text("Cancel"), .aiarb-popconfirm button:has-text("取消")'

    # Toast messages
    SUCCESS_TOAST = '.aiarb-message-success, .aiarb-notification-success'
    ERROR_TOAST = '.aiarb-message-error, .aiarb-notification-error'
    ...
    def is_card_enabled(self, card: Locator) -> bool:
        ...
        if switch.count() > 0:
            return switch.evaluate(
                "el => el.classList.contains('aiarb-switch-checked') || "
