...
- First-time activation triggers an "Experimental Feature" confirmation
  modal gated by ``localStorage["aiarb-coding-mode-confirmed"]``. To
...
class CodingPage(BasePage):
    """Page object for AIArb Coding Mode."""
    ...
    # forced to ``en-US`` in fixtures, but the AIArb frontend keeps a
    ...
    LS_KEY_CONFIRMED = "aiarb-coding-mode-confirmed"
    ...
    # via ``aiarb-last-used-agent`` (localStorage) and the persisted
    # zustand state under ``aiarb-agent-storage`` (both local and
    ...
    def _install_default_agent_init_script(self) -> None:
        ...
        rewrites both the legacy ``aiarb-last-used-agent`` key and the
        persisted ``aiarb-agent-storage`` blob in localStorage and
