...
def pytest_sessionstart(session):
    """Hard guard against running e2e against a real AIArb instance.

    Triggers ``config.working_dir`` validation before any fixture
    chain runs. Fails the entire session in <1s if
    ``AIARB_WORKING_DIR`` is unset or points inside the user's home
...
_DEFAULT_PROVIDER = os.getenv("AIARB_MODEL_PROVIDER", "dashscope")
_DEFAULT_MODEL = os.getenv("AIARB_DEFAULT_MODEL", "qwen3.6-plus")
...
def setup_default_model(api_context):
    """Configure provider API key and set a global default model when AIARB_DASHSCOPE_API_KEY is set."""
    model_key = os.getenv("AIARB_DASHSCOPE_API_KEY")
    if not model_key:
        logger.info("AIARB_DASHSCOPE_API_KEY not set, skipping model setup")
        yield
        return
