# -*- coding: utf-8 -*-
"""
AIArb Local Models module P0 end-to-end test cases.
...
class TestModelListDisplay:
    ...
    def test_model_list_display(self, page: Page, request: pytest.FixtureRequest):
        ...
        server_status = page.locator('[class*="serverStatus"], .aiarb-card:has-text("llama.cpp"), .aiarb-card:has-text("Server")').first
        ...
        provider_tiles = page.locator(
            ...
            '[class*="modelList"], .aiarb-list, .aiarb-card'
        ).all()
        ...
        provider_cards = page.locator('[class*="providerCard"], .aiarb-card').all()
        ...
        data_items = page.locator('[class*="modelItem"], .aiarb-list-item, .aiarb-table-row').all()
...
class TestModelServe:
    ...
    def test_model_serve_flow(self, page: Page, request: pytest.FixtureRequest):
        ...
        model_items = page.locator('[class*=modelItem], .aiarb-list-item, .aiarb-card').all()
        ...
        serve_btns = page.locator('button:has-text("启动"), button:has-text("Serve"), button:has-text("服务"), .aiarb-btn:has-text("启动")').or_(page.get_by_text("启动")).or_(page.get_by_text("Serve")).or_(page.get_by_text("服务")).all()
        ...
        if len(serve_btns) > 0:
            ...
            response_indicators = page.locator(
                '.aiarb-modal, .aiarb-drawer, '
                '.aiarb-message, .aiarb-notification, '
        ...
        status_display = page.locator('[class*="status"], [class*="serving"], .aiarb-tag, .aiarb-badge').first
...
class TestModelManagement:
    ...
    def test_model_management_operations(self, page: Page, request: pytest.FixtureRequest):
        ...
        more_btns = page.locator('button:has-text("⋮"), button:has-text("⋯"), .aiarb-btn-icon:has(.spark-icon-spark-more-line)').all()
        ...
        if len(more_btns) > 0:
            ...
            delete_option = page.locator('.aiarb-dropdown-menu-item:has-text("删除"), .aiarb-dropdown-menu-item:has-text("Delete")').or_(page.get_by_text("删除")).or_(page.get_by_text("Delete")).first
        ...
        running_status = page.locator('[class*=running], .aiarb-tag:has-text("运行中")').or_(page.get_by_text("运行中")).or_(page.get_by_text("Running")).first
...
class TestCustomProviderCreateAndDelete:
    ...
    def test_custom_provider_create_and_delete(self, page: Page):
        ...
        ok_button = modal.locator("button.aiarb-btn-primary").first
        ...
        delete_btn = provider_card.locator("button.aiarb-btn-dangerous, button[class*='dangerous']").first
        ...
        confirm_modal = page.locator(".aiarb-modal-confirm, .aiarb-modal, .aiarb-popconfirm").first
        if confirm_modal.count() > 0:
            ...
            if confirm_btn.count() > 0:
                ...
                footer_btn = confirm_modal.locator(".aiarb-modal-confirm-btns button, .aiarb-modal-footer button").last
...
class TestProviderConfigAndConnection:
    ...
    def test_provider_config_and_connection_test(self, page: Page):
        ...
        try:
            ...
            log_test_step("Submit the create form")
            create_button = modal.locator("button.aiarb-btn-primary").first
            ...
            try:
                ...
                provider_card = page.locator(
                    f".aiarb-card:has-text('{provider_id}'), "
                ...
                if provider_card.count() > 0:
                    ...
                    if delete_btn.count() > 0:
                        ...
                        confirm_modal = page.locator(".aiarb-modal-confirm, .aiarb-modal, .aiarb-popconfirm").first
                        if confirm_modal.count() > 0:
                            confirm_btn = confirm_modal.locator("button.aiarb-btn-primary, button:has-text('确 定'), button:has-text('OK')").first
                            if confirm_btn.count() > 0:
                                confirm_btn.click()
                                page.wait_for_timeout(2000)
...
class TestProviderSearchFilter:
    ...
    def test_provider_search_filter(self, page: Page, request: pytest.FixtureRequest):
        ...
        search_input = page.locator('input[type="search"], input[placeholder*="search"], input[placeholder*="Search"], input[placeholder*="搜索"], .aiarb-input-search input').first
        ...
        # Search box is a aiarb-select component (readonly input); need to click the parent container to open dropdown
        ...
        if is_readonly:
            ...
            select_container = page.locator('.aiarb-select').first
        ...
        if is_readonly:
            ...
            clear_btn = page.locator('.aiarb-select-clear').first
    ...
    def test_model_activation(self, page: Page, request: pytest.FixtureRequest):
        ...
        if models_btn.count() > 0:
            ...
            if modal.count() > 0:
                ...
                # Close modal
                close_btn = modal.locator('.aiarb-modal-close, button:has-text("Cancel"), button:has-text("取消")').first
                if close_btn.count() > 0:
                    close_btn.click()
                    page.wait_for_timeout(1000)
            else:
                logger.info("No model management modal, may be a Drawer instead")
                drawer = page.locator('.aiarb-drawer').first
                if drawer.count() > 0:
            ...
            has_response = page.locator('.aiarb-modal, .aiarb-drawer').first.count() > 0
...
class TestOpenRouterFilter:
    ...
    def test_openrouter_filter(self, page: Page, request: pytest.FixtureRequest):
        ...
        config_modal = page.locator(
            '.aiarb-modal:has-text("OpenRouter"), '
            '.ant-modal:has-text("OpenRouter"), '
            '.aiarb-modal:has-text("Base URL"), '
...
class TestModelJsonEditor:
    ...
    def test_model_json_editor(self, page: Page, request: pytest.FixtureRequest):
        ...
        provider_cards = page.locator('.aiarb-card').all()
        ...
        page.wait_for_timeout(500)
        modal_or_drawer = page.locator('.aiarb-modal, .ant-modal, .aiarb-drawer, .ant-drawer').first
