# -*- coding: utf-8 -*-
"""
AIArb cross-module end-to-end test cases.
...
"""
            ...
            agent_table = page.locator('.aiarb-table').first
            expect(agent_table).to_be_visible(timeout=5000)
            agent_rows = page.locator('.aiarb-table-tbody tr.aiarb-table-row').all()
            ...
            if editable_agent_found:
                ...
                modal = page.locator('.aiarb-modal, [role="dialog"]').first
                expect(modal).to_be_visible(timeout=5000)

                skills_section = modal.locator(
                    '.aiarb-form-item:has-text("Skills"), '
                    '[class*=skill]'
                ).first
                ...
                cancel_btn = modal.locator(
                    ...
                    '.aiarb-modal-footer button.aiarb-btn-default'
                ).first
...
class TestSecurityInterceptionInChat:
    ...
    def test_security_config_affects_chat(self, page: Page, request: pytest.FixtureRequest):
        ...
        try:
            ...
            guard_switch = tool_guard_panel.locator('button.aiarb-switch[role="switch"]').first
            ...
            if file_guard_tab.is_visible(timeout=3000):
                ...
                file_switch = file_guard_panel.locator('button.aiarb-switch[role="switch"]').first
...
class TestWorkspaceFileChatFlow:
    ...
    def test_workspace_file_and_chat_qa(self, page: Page, test_file: str, request: pytest.FixtureRequest):
        ...
        try:
            ...
            file_keywords = ["AIArb", "智能", "对话", "功能", "平台"]
...
class TestEnvAndRuntimeConfigFlow:
    ...
    def test_env_and_runtime_config_consistency(self, page: Page, request: pytest.FixtureRequest):
        ...
        env_rows = page.locator(
            '.aiarb-table-tbody tr.aiarb-table-row, '
            '[class*=envRow], '
            '.aiarb-form-item'
        ).all()
