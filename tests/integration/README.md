...
HTTP smoke tests that exercise the AIArb FastAPI app end-to-end via a
real subprocess. Each test file owns its own AIArb app subprocess on a
...
test file gets its own AIArb app subprocess on a random port, sharing
...
- Forces `AIARB_AUTH_ENABLED=false` and `NO_PROXY=*`
...
```bash
AIARB_INTEGRATION_COVERAGE=1 pytest tests/integration/ --no-cov
```
