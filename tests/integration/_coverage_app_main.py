...
Launches the same server as ``python -m aiarb app`` (same CLI entry,
...
SIGINT/SIGTERM only) nor AIArb registers a SIGBREAK handler.  The
...
(per-app debug logs with ``Tracing '...aiarb...'`` dispositions), yet
...
This file is only used when ``AIARB_INTEGRATION_COVERAGE`` is enabled
...
# Emulate ``python -m aiarb app <args...>``.  The fixture invokes this
...
sys.argv = ["aiarb", *sys.argv[1:]]
runpy.run_module("aiarb", run_name="__main__")
