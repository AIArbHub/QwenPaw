...
from aiarb.cli.channels_cmd import channels_group
from aiarb.cli.cron_cmd import cron_group
from aiarb.cli.daemon_cmd import daemon_group
from aiarb.cli.chats_cmd import chats_group
from aiarb.cli.skills_cmd import skills_group
...
def test_channels_list_default_agent(
    ...
    """Test aiarb channels list uses default agent by default."""
    ...
    with patch("aiarb.cli.channels_cmd.load_agent_config") as mock_load:
...
def test_channels_list_custom_agent(
    ...
    """Test aiarb channels list with custom agent_id."""
...
def test_cron_list_with_agent_id():
    """Test aiarb cron list with --agent-id."""
    runner = CliRunner()

    with patch("aiarb.cli.cron_cmd.client") as mock_client:
...
def test_daemon_status_default_agent():
    """Test aiarb daemon status defaults to 'default' agent."""
    runner = CliRunner()

    with patch("aiarb.cli.daemon_cmd.run_daemon_status") as mock_status:
        with patch("aiarb.cli.daemon_cmd._get_agent_workspace") as mock_ws:
...
def test_daemon_status_custom_agent():
    """Test aiarb daemon status with custom agent."""
    runner = CliRunner()
...
def test_skills_list_default_agent():
    """Test aiarb skills list defaults to 'default' agent."""
    runner = CliRunner()

    with patch(
        "aiarb.cli.skills_cmd._get_agent_workspace",
    ) as mock_ws:
        with patch("aiarb.cli.skills_cmd.SkillService") as mock_service:
            mock_ws.return_value = "/tmp/default"
...
def test_skills_list_custom_agent():
    """Test aiarb skills list with custom agent."""
    runner = CliRunner()
...
def test_chats_list_with_agent_id():
    """Test aiarb chats list with --agent-id."""
    runner = CliRunner()

    with patch("aiarb.cli.chats_cmd.client") as mock_client:
        mock_response = MagicMock()
