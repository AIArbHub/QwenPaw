...
from aiarb.agents.acp.meta import ACP_APPROVAL_EXPIRES_AT_META_KEY
from aiarb.agents.acp.server import (
    _EnvelopeTracker,
    ACP_AGENT_META_KEY,
    ACP_ERROR_META_KEY,
    AIArbACPAgent,
...
def test_build_available_commands_set():
    agent = object.__new__(AIArbACPAgent)
...
async def test_registered_help_text_command_is_executed_and_advertised():
    ...
    from aiarb.runtime.slash_command_registry import (
        CommandSpec,
...
async def test_new_session_reports_agent_id_in_meta():
    agent = AIArbACPAgent(agent_id="my-agent")
    conn = _FakeConn()
    agent.on_connect(conn)
...
async def test_report_prompt_error_hides_unexpected_exception_details():
    ...
    assert update.content.text == (
        "Error: AIArb failed to process the request. "
...
async def test_report_prompt_error_shows_details_for_local_diagnostics():
    agent = AIArbACPAgent(agent_id="default", local_diagnostics=True)
    conn = _FakeConn()
    agent.on_connect(conn)
...
async def test_approval_bridge_resolves_pending_approval(monkeypatch):
    ...
    monkeypatch.setattr(
        "aiarb.app.approvals.service._approval_service",
        approval_svc,
    )
...
def test_acp_bootstrap_includes_runtime_slash_commands():
    from aiarb.app.app_services import AppServiceManager

    kwargs = AIArbACPAgent._build_bootstrap_kwargs(AppServiceManager())
    command_names = {
...
def test_usage_meta_includes_model(monkeypatch):
    ...
    assert AIArbACPAgent._pop_session_usage("sess-usage") == {
        "usage": {
            "inputTokens": 12,
            "outputTokens": 34,
            "totalTokens": 46,
...
def test_usage_meta_includes_context_size(monkeypatch):
    ...
    meta = AIArbACPAgent._pop_session_usage("sess-usage")
