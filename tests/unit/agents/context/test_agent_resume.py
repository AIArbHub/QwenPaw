...
These drive the REAL ``AIArbAgent.state_dict`` / ``load_state_dict`` wiring
...
from aiarb.agents.context.scroll.continuation_summary import (
    ContinuationSummary,
)
...
class AgentShim:
    """Minimal stand-in for AIArbAgent's state (de)serialization.
    ...
    def _sanitize_loaded_context(self) -> None:
        ...
        AIArbAgent._sanitize_loaded_context(self)
...
def test_state_dict_carries_the_scroll_bookkeeping(store):
    ...
    dumped = AIArbAgent.state_dict(agent)
...
def test_resume_after_crash_does_not_reappend(store):
    ...
    snapshot = json.loads(json.dumps(AIArbAgent.state_dict(agent1)))
    ...
    AIArbAgent.load_state_dict(agent2, snapshot, strict=True)
