# -*- coding: utf-8 -*-
"""Tests for the API health probe used by ``aiarb doctor``."""
...
from aiarb.cli import doctor_cmd
...
def test_api_health_reports_background_startup(monkeypatch, capsys) -> None:
    ...
    assert "rerun `aiarb doctor`" in captured.err
...
def test_api_health_reports_connection_failure(monkeypatch, capsys) -> None:
    ...
    assert "start the server with `aiarb app`" in captured.err
