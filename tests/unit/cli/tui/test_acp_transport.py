...
from aiarb.agents.acp.meta import (
...
from aiarb.cli.tui.transport.acp import AcpTransport, _TuiClient
...
async def test_start_and_basic_turn():
    ...
    try:
        ...
        assert connected.aiarb_version == "0.0.1"
...
async def test_permission_expires_locally_at_deadline(monkeypatch):
    ...
    """
    monkeypatch.setattr(
        "aiarb.cli.tui.transport.acp._PERMISSION_EXPIRY_GRACE_SECONDS",
        10.0,
    )
    queue = asyncio.Queue()
    client = _TuiClient(queue)
...
def test_session_agent_reads_meta():
    from aiarb.cli.tui.transport.acp import _session_agent

    # Agent id reported via the session response _meta.
    sess = SimpleNamespace(field_meta={"aiarb.agent": "writer"})
