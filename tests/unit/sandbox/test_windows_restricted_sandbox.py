...
from aiarb.sandbox.windows_restricted_sandbox import (
...
from aiarb.sandbox.windows_sandbox import (
    _is_cmd_exe,
    _is_powershell_exe,
)
...
class TestFactoryRouting:
    ...
    def test_allow_read_all_false_does_not_route_here(self):
        ...
        from aiarb.sandbox import create_sandbox
        from aiarb.sandbox.windows_sandbox import WindowsSandbox
...
class TestUserProvisioning:
    ...
    @patch("aiarb.sandbox.windows_restricted_sandbox._get_netapi32")
...
class TestACLApplication:
    ...
    @patch("aiarb.sandbox.windows_restricted_sandbox._allow_null_device")
    @patch("aiarb.sandbox.windows_restricted_sandbox._add_deny_all_ace")
    @patch("aiarb.sandbox.windows_restricted_sandbox._add_allow_read_ace")
    @patch("aiarb.sandbox.windows_restricted_sandbox._add_allow_ace")
    @patch(
        "aiarb.sandbox.windows_restricted_sandbox"
        "._ensure_python_dir_group_acl",
    )
    @patch(
        "aiarb.sandbox.windows_restricted_sandbox._get_python_install_dir",
    )
    @patch("aiarb.sandbox.windows_restricted_sandbox._string_to_sid")
    @patch("aiarb.sandbox.windows_restricted_sandbox._get_kernel32")
    ...
    def test_workspace_gets_full_access(
        ...
        from aiarb.sandbox.windows_restricted_sandbox import _apply_all_acls
...
class TestWindowsRestrictedSandboxExecute:
    ...
    @patch("aiarb.sandbox.windows_restricted_sandbox._wait_and_read_process")
    @patch(
        "aiarb.sandbox.windows_restricted_sandbox"
...
class TestWindowsRestrictedSandboxStop:
    ...
    @patch("aiarb.sandbox.windows_restricted_sandbox._release_sandbox")
    @patch("aiarb.sandbox.windows_restricted_sandbox._get_kernel32")
...
class TestShutdownBudget:
    ...
    def test_remaining_budget_no_deadline_returns_large_value(self):
        ...
        import aiarb.sandbox.windows_restricted_sandbox as wrs
    ...
    def test_run_icacls_sync_uses_min_of_180_and_budget(self):
        ...
        try:
            ...
            with patch(
                "aiarb.sandbox.windows_restricted_sandbox._run_cmd_sync",
