...
class TestProbeSandboxSupport:
    ...
    @patch("aiarb.sandbox.config._probe_windows_appcontainer")
...
class TestProbeAppContainer:
    ...
    def test_non_windows_returns_unsupported(self):
        from aiarb.sandbox.config import _probe_windows_appcontainer
...
class TestACLCommandGeneration:
    ...
    @patch("aiarb.sandbox.windows_sandbox._run_icacls")
    ...
    def test_workspace_gets_full_access(
        ...
        from aiarb.sandbox.windows_sandbox import _apply_all_acls
...
class TestSandboxReuse:
    ...
    @patch("aiarb.sandbox.windows_sandbox._get_appcontainer_sid")
    @patch("aiarb.sandbox.windows_sandbox._get_userenv")
...
class TestAppContainerProfileLifecycle:
    ...
    @patch("aiarb.sandbox.windows_sandbox._sid_to_string")
    @patch("ctypes.windll", create=True)
    @patch("aiarb.sandbox.windows_sandbox._get_advapi32")
    @patch("aiarb.sandbox.windows_sandbox._get_userenv")
...
class TestWindowsSandboxExecute:
    ...
    @patch("aiarb.sandbox.windows_sandbox._wait_and_read_process")
    @patch("aiarb.sandbox.windows_sandbox._create_process_in_appcontainer")
