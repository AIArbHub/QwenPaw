...
class TestAddFileHandler:
    ...
    def test_creates_log_directory(self, tmp_path):
        ...
        log_path = tmp_path / "logs" / "aiarb.log"
    ...
    def test_uses_environment_rotation_limits(self, tmp_path, monkeypatch):
        log_path = (tmp_path / "aiarb.log").resolve()
        monkeypatch.setenv("AIARB_LOG_MAX_SIZE", "10MB")
        monkeypatch.setenv("AIARB_LOG_MAX_BACKUPS", "5")
        logger = logging.getLogger(LOG_NAMESPACE)
        original_handlers = list(logger.handlers)
        logger.handlers = []
...
def test_invalid_rotation_environment_uses_defaults(monkeypatch):
    monkeypatch.setenv("AIARB_LOG_MAX_SIZE", "unbounded")
    monkeypatch.setenv("AIARB_LOG_MAX_BACKUPS", "-2")
...
def test_zero_log_backups_is_supported(monkeypatch):
    monkeypatch.delenv("AIARB_LOG_MAX_SIZE", raising=False)
    monkeypatch.setenv("AIARB_LOG_MAX_BACKUPS", "0")
...
class TestLogConstants:
    ...
    def test_log_namespace_is_aiarb(self):
        """S级: LOG_NAMESPACE is 'aiarb'."""
        assert LOG_NAMESPACE == "aiarb"
