...
- **Check [Open Issues](https://github.com/agentscope-ai/AIArb/issues)** and any [Projects](https://github.com/agentscope-ai/AIArb/projects) or roadmap labels.
...
AIArb is designed to be **extensible**: you can add models, channels, Skills, and more. Below are the main contribution areas we care about.

---

### Adding New Models / Model Providers

AIArb supports multiple providers, including cloud providers (such as DashScope, ModelScope) and local providers (such as Ollama, LM Studio). We also welcome new model providers to enrich user choices.
...
If the above conditions are met, you can create a new Provider instance in `src/aiarb/providers/provider_manager.py` and register it in the `ProviderManager` class to make it a built-in provider in AIArb.
...
Channels are how AIArb talks to **DingTalk, Feishu, QQ, Discord, iMessage**, etc. You can add a new channel so AIArb can work with your favorite IM or bot platform.
...
- **Implementation:** Implement a **subclass of `BaseChannel`** (in `src/aiarb/app/channels/base.py`):
...
- **Discovery:** Built-in channels are registered in `src/aiarb/app/channels/registry.py`. **Custom channels** are registered via the plugin system — create a plugin with `type: "channel"` and call `api.register_channel(...)` in your plugin's `register()` method. See the [Plugin System docs](website/public/docs/plugins.en.md) for a full example.
- **CLI:** `aiarb channels config` — interactive config; `aiarb channels list` — view status.
...
**Skills** define what AIArb can do: cron, file reading, PDF/Office, news, browser, etc. We welcome **broadly useful** base skills (productivity, documents, communication, automation) that fit the majority of users.
...
- **Location:** Built-in skills live under `src/aiarb/agents/skills/<skill_name>/`. The app merges built-in and user **customized_skills** from the working dir into **active_skills**; no extra registration is needed beyond placing a valid `SKILL.md` in a directory.
...
- **Skills Hub:** AIArb supports importing skills from a community hub (e.g. ClawHub). If you want your skill to be installable via hub, follow the same `SKILL.md` + `references/`/`scripts/` layout and the hub’s packaging format.
...
AIArb aims to run on **Windows**, **Linux**, and **macOS**. Contributions that improve support on a specific platform are welcome.
...
- **Install and run:** One-line install (`install.sh`), `pip` install, and `aiarb init` / `aiarb app` should work (or be clearly documented) on each supported platform. Fixes to install or startup on a given OS are valuable.
...
- **Discussions:** [GitHub Discussions](https://github.com/agentscope-ai/AIArb/discussions)
- **Bugs and features:** [GitHub Issues](https://github.com/agentscope-ai/AIArb/issues)
- **Community:** DingTalk group (see [README](README.md)) and [Discord](https://discord.gg/eYMpfnkG8h)

Thank you for contributing to AIArb. Your work helps make it a better assistant for everyone. 🐾
