...
感谢你对 AIArb 的关注！AIArb 是一个开源的**个人 AI 助手**，可以在你自己的环境中运行——无论是你的机器还是云端。它可以连接钉钉、飞书、QQ、Discord、iMessage 等聊天应用，支持定时任务和心跳机制，并通过 **Skills** 扩展其能力。我们热烈欢迎能让 AIArb 对所有人更有用的贡献：无论是添加新的频道、新的模型提供商、Skill，改进文档，还是修复 bug。

**快速链接：** [GitHub](https://github.com/agentscope-ai/AIArb) · [文档](https://aiarb.agentscope.io/) · [许可证：Apache 2.0](LICENSE)
...
- **检查 [Open Issues](https://github.com/agentscope-ai/AIArb/issues)** 以及任何 [Projects](https://github.com/agentscope-ai/AIArb/projects) 或路线图标签。
...
AIArb 设计为**可扩展的**：你可以添加模型、频道、Skills 等。以下是我们关心的主要贡献领域。

---

### 添加新模型 / 模型提供商

AIArb 支持多种提供商：包括云提供商（如 DashScope、ModelScope）以及本地提供商（如 Ollama、LM Studio），但我们也欢迎新的模型供应商以丰富用户选择。
...
确定满足上述条件后，可以在 `src/aiarb/providers/provider_manager.py` 中创建新的 Provider 实例并在 `ProviderManager` 类中注册，使其成为 AIArb 内置的提供商。
...
频道是 AIArb 与**钉钉、飞书、QQ、Discord、iMessage** 等通信的方式。你可以添加新频道，以便 AIArb 可以与你喜欢的 IM 或机器人平台配合使用。
...
- **实现：** 实现 **`BaseChannel` 的子类**（在 `src/aiarb/app/channels/base.py` 中）：
...
- **发现：** 内置频道在 `src/aiarb/app/channels/registry.py` 中注册。**自定义频道**通过插件系统注册 — 创建 `type: "channel"` 的插件，在 `register()` 方法中调用 `api.register_channel(...)`。完整示例请参阅[插件系统文档](website/public/docs/plugins.zh.md)。
- **CLI：** `aiarb channels config` — 交互式配置；`aiarb channels list` — 查看状态。
...
**Skills** 定义了 AIArb 可以做什么：cron、文件读取、PDF/Office、新闻、浏览器等。我们欢迎**广泛有用的**基础 skills（生产力、文档、通信、自动化），适合大多数用户。
...
- **位置：** 内置 skills 位于 `src/aiarb/agents/skills/<skill_name>/` 下。应用程序将内置和用户的 **customized_skills**（来自工作目录）合并到 **active_skills** 中；除了在目录中放置有效的 `SKILL.md` 外，不需要额外的注册。
...
- **Skills Hub：** AIArb 支持从社区 hub（如 ClawHub）导入 skills。如果你希望你的 skill 可以通过 hub 安装，请遵循相同的 `SKILL.md` + `references/`/`scripts/` 布局和 hub 的打包格式。
...
AIArb 旨在在 **Windows**、**Linux** 和 **macOS** 上运行。欢迎改进特定平台支持的贡献。

- **兼容性修复：** 路径处理、行尾、shell 命令或在不同操作系统上行为不同的依赖项。例如：内存/向量栈的 Windows 兼容性，或在 Linux 和 macOS 上都能工作的安装脚本。
- **安装和运行：** 一行安装（`install.sh`）、`pip` 安装，以及 `aiarb init` / `aiarb app` 应该在每个支持的平台上工作（或有清晰的文档说明）。对给定操作系统上的安装或启动的修复很有价值。
...
- **MCP（模型上下文协议）：** AIArb 支持运行时 **MCP 工具**发现和热插拔。贡献新的 MCP 服务器或工具（或关于如何附加它们的文档）可以帮助用户扩展 agent 而无需更改核心代码。
- **文档：** 对 [文档](https://aiarb.agentscope.io/)（位于 `website/public/docs/` 下）和 README 的修复和改进始终受欢迎。
...
- **讨论：** [GitHub Discussions](https://github.com/agentscope-ai/AIArb/discussions)
- **Bug 和功能：** [GitHub Issues](https://github.com/agentscope-ai/AIArb/issues)
- **社区：** 钉钉群（见 [README](README_zh.md)）和 [Discord](https://discord.gg/eYMpfnkG8h)

感谢你为 AIArb 贡献代码。你的工作帮助它成为每个人更好的助手。🐾
