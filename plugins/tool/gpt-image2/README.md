...
A AIArb tool plugin that enables image generation and editing using OpenAI's GPT Image 2 model.
...
```bash
aiarb plugin install /path/to/gpt-image2
```

Or from ZIP:

```bash
aiarb plugin install gpt-image2-tool.zip
```

## Configuration

1. Start AIArb application
...
- Ensure the plugin is installed: `aiarb plugin list`
- Check AIArb logs: `~/.aiarb/logs/aiarb.log`
- Restart AIArb after installation
...
- Check file permissions in `~/.aiarb/plugins/`
...
For issues and feature requests, please use the AIArb issue tracker.
