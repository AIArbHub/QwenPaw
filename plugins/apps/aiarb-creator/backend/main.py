...
"""AIArb Creator — PawApp backend entry point.
...
REST routes are mounted under ``/api/aiarb-creator`` via the PawApp
...
logger = logging.getLogger("aiarb").getChild("plugin.aiarb_creator")
...
def configure_creator_runtime_environment(
    *,
    ...
    """Provision Creator paths below AIArb's working directory by default.
...
app = PawApp("AIArb Creator", app_id="aiarb-creator")
...
async def _startup() -> None:
    ...
    try:
        ...
        logger.exception(
            "AIArb Creator runtime dependency preparation failed; startup aborted",
        )
        raise
    try:
    ...
    logger.info("AIArb Creator file runtime ready at %s", data_root)
    ...
    logger.info(
        "AIArb Creator native tool status: %s",
        {
            name: {
                "status": item.status,
                "path": item.path,
    ...
    if configured:
        logger.info(
            "AIArb Creator env-backed config available: %s",
            configured,
        )
    else:
        logger.warning(
            "AIArb Creator is installed, but no env-backed generation "
            ...
            "in AIArb Tools, or set the matching TEXT/IMAGE/VIDEO/OSS "
