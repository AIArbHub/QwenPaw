...
def _grounding_stable_id(prefix: str, project_id: str, identity: str) -> str:
    value = uuid5(
        NAMESPACE_URL,
        f"aiarb-creator:grounding:{prefix}:{project_id}:{identity}",
    ).hex
...
class FileCreatorAgentRuntime:
    ...
    async def _reconcile_project(self, project_id: str) -> None:
        ...
        # cross-process run lease. A second AIArb process may observe the same
        ...
        *,
        ...
        while True:
            authorization_id = (
                ...
                + uuid5(
                    NAMESPACE_URL,
                    "aiarb-creator:file-tool-authorization:"
    ...
    async def _cancel_run(
        ...
        if superseded:
            ...
            # AIArb processes can then observe the same stop and cannot
