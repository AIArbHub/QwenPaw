...
AIArb installs Python requirements for plugins.  Creator additionally needs
...
logger = logging.getLogger("aiarb.creator.runtime_dependencies")
    ...
    def _download_jq(self) -> Path:
        ...
        try:
            ...
            with temporary_path.open("wb") as temporary:
                request = Request(
                    url,
                    headers={"User-Agent": "AIArb-Creator/0.1"},
                )
