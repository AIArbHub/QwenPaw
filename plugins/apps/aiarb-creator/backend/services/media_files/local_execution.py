...
class FfmpegLocalMediaRunner:
    ...
    def _segment_concurrency(self, segment_count: int) -> int:
        ...
        developer machine.  ``AIARB_SEGMENT_CONCURRENCY=1`` restores
        strictly serial rendering.
        """

        raw = os.environ.get("AIARB_SEGMENT_CONCURRENCY", "")
        try:
    ...
    def _segment_cache_root() -> Path:
        root = Path(tempfile.gettempdir()) / "aiarb-segment-cache-v1"
...
def _stable_id(prefix: str, project_id: str, key: str) -> str:
    digest = uuid5(
        NAMESPACE_URL,
        f"aiarb-creator:file-local-media:{prefix}:{project_id}:{key}",
    ).hex
