...
def serve():
    ...
    with sync_playwright() as playwright:
        ...
        # AIARB_CREATOR_MOTION_BROWSER points at a known-good browser.
        BROWSER = playwright.chromium.launch(
            headless=True,
            executable_path=(
                os.environ.get("AIARB_CREATOR_MOTION_BROWSER") or None
            ),
        )
...
def render_motion_poster(
    ...
    *,
    ...
    cache_root = Path(tempfile.gettempdir()) / "aiarb-motion-poster-cache-v1"
    try:
...
def prepare_motion_layer(
    *,
    ...
    if viewport_inset > 0 and doc_format != "html_js":
        ...
        safety_css = (
            "<style data-aiarb-viewport-safety>"
            ...
            "[data-motion-motif=paw_trail] .p3{opacity:0;animation:aiarb-paw-appear .36s "
            ...
            "@keyframes aiarb-paw-appear{0%{opacity:0}100%{opacity:1}}"
            "</style>"
        )
        html = (
            re.sub(
    ...
    cache_root = Path(tempfile.gettempdir()) / "aiarb-motion-frame-cache-v2"
    cache_root.mkdir(mode=0o700, parents=True, exist_ok=True)
