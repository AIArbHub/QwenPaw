...
def test_seed_uses_project_asset_and_message_http_boundaries() -> None:
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        if request.url.path == "/api/aiarb-creator/projects":
            return httpx.Response(201, json=_project_response())
        if (
            request.url.path
            == "/api/aiarb-creator/projects/project-demo/assets"
        ):
        ...
        if (
            request.url.path
            == "/api/aiarb-creator/projects/project-demo/messages"
        ):
            return httpx.Response(
                202,
                json={
    ...
    with httpx.Client(
        base_url="http://creator.test/api/aiarb-creator",
