...
# AIArb secret store for reading encrypted provider API keys
try:
    from aiarb.security.secret_store import (
        decrypt as aiarb_decrypt,
        encrypt as aiarb_encrypt,
        is_encrypted as aiarb_is_encrypted,
    )
    from aiarb.constant import SECRET_DIR as AIARB_SECRET_DIR

    AIARB_SECRET_AVAILABLE = True
except ImportError:
    AIARB_SECRET_AVAILABLE = False
    aiarb_decrypt = None
    aiarb_encrypt = None
    aiarb_is_encrypted = None
    AIARB_SECRET_DIR = None
...
def _assemble_model_config(
    ...
    *,
    ...
    # Decrypt secret fields when the AIArb secret store is available.
...
def get_host_provider_api_key(provider_id: str) -> str | None:
    """Read a provider's API key from the AIArb encrypted store.

    Lookup order:
    1. builtin providers: ~/.aiarb.secret/providers/builtin/{provider_id}.json
    2. custom providers: ~/.aiarb.secret/providers/custom/{provider_id}.json

    Returns:
        str: the decrypted API key, or None when missing or undecryptable
    """
    if not AIARB_SECRET_AVAILABLE or AIARB_SECRET_DIR is None:
        logger.debug("AIArb secret store not available")
        return None
    ...
    for subdir in ["builtin", "custom"]:
        provider_file = (
            AIARB_SECRET_DIR / "providers" / subdir / f"{provider_id}.json"
        )
            ...
            try:
                with open(provider_file, "r", encoding="utf-8") as f:
                    ...
                    if encrypted_key.startswith("ENC:"):
                        decrypted = aiarb_decrypt(encrypted_key)
...
def _decrypt_secret_fields(data: dict) -> dict:
    ...
    if not (AIARB_SECRET_AVAILABLE and aiarb_decrypt is not None):
        return data
    ...
    for section_data in data.values():
        ...
        for field in _SECRET_FIELDS:
            ...
            if not (
                ...
                and aiarb_is_encrypted(value)
            ):
                continue
            section_data[field] = aiarb_decrypt(value)
    return data


def _encrypt_secret_fields(data: dict) -> dict:
    """Encrypt the secret fields inside the config data."""
    if not (AIARB_SECRET_AVAILABLE and aiarb_encrypt is not None):
        return data
    ...
    for section_data in data.values():
        ...
        for field in _SECRET_FIELDS:
            ...
            if not (
                value
                and isinstance(value, str)
                and not aiarb_is_encrypted(value)
            ):
                continue
            section_data[field] = aiarb_encrypt(value)
    return data
...
def mutate_model_config(
    ...
    with CrossProcessFileLock(config_path.parent / ".model-config.lock"):
        ...
        # Encrypt secret fields when the AIArb secret store is available.
...
def _aiarb_tool_configs(request: Request) -> dict[str, dict[str, Any]]:
...
async def bind_creator_tool_config(request: Request):
    ...
    configs = _aiarb_tool_configs(request)
...
async def get_host_provider_key(provider_id: str) -> dict[str, str | None]:
    """Fetch the API key of the given provider from the AIArb host.
