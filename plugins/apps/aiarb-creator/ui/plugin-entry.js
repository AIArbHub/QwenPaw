(function () {
  const pluginId = "aiarb-creator";
  const AIArb = window.AIArb;
  const host = AIArb && AIArb.host;
  const React = host && host.React;
  if (!AIArb || !React || !host.getApiUrl || !AIArb.registerRoutes) {
    console.warn("[aiarb-creator] AIArb PawApp host API is unavailable.");
    return;
  }

  const CREATOR_BG_COLOR = "#f9f8f4";
  const CREATOR_BUILD_ID = "__CREATOR_BUILD_ID__";
  const NAVIGATION_MESSAGE = "aiarb-creator:navigation";
  const RESTORE_ROUTE_MESSAGE = "aiarb-creator:restore-route";
  ...
  function normalizeCreatorRoute(path) {
    ...
    const parsed = new URL(path, "http://aiarb-creator.local");
  ...
  function CreatorFrame() {
    ...
    if (!initialSrcRef.current) {
      const appUrl = host.getApiUrl(
        "/frontend_plugin/aiarb-creator/files/ui/dist/app/index.html",
      );
    ...
    return React.createElement(
      "div",
      {
        className: "aiarb-creator-app-content",
        style: {
          width: "100%",
          height: "100%",
          overflow: "hidden",
  ...
  AIArb.registerRoutes(pluginId, [
    {
      path: "/apps/aiarb-creator",
  ...
  console.info("[aiarb-creator] registered route /apps/aiarb-creator");
})();
