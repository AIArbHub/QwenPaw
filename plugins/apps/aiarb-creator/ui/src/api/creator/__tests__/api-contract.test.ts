...
describe("new Creator API contract", () => {
  it("uses Project Patch and file Review routes with stable idempotency ids", async () => {
    ...
    expect(calls.map((call) => [call.method, call.url])).toEqual([
      ["POST", "/api/aiarb-creator/projects"],
      ["POST", "/api/aiarb-creator/projects/p1/messages"],
      ["PATCH", "/api/aiarb-creator/projects/p1/project"],
      [
        "POST",
        "/api/aiarb-creator/projects/p1/runtime/reviews/review-1/decisions",
      ],
    ]);
    ...
    expect(
      calls.every((call) =>
        call.url.startsWith("/api/aiarb-creator/projects"),
      ),
    ).toBe(true);
  });
  ...
  it("preserves browser folder paths and uses the canonical model probe", async () => {
    ...
    expect(calls[1]).toMatchObject({
      method: "POST",
      url: "/api/aiarb-creator/models/test",
      body: {
        type: "vlm",
        base_url: "https://example.test/v1",
        api_key: "secret",
  ...
  it("saves the complete single-file model configuration", async () => {
    ...
    expect(calls[0]).toMatchObject({
      method: "POST",
      url: "/api/aiarb-creator/models/config",
      body: {
