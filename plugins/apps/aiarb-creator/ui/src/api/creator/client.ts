...
export const CREATOR_API_BASE = "/api/aiarb-creator";

type HostWindow = Window & {
  AIArb?: { host?: { getApiToken?: () => string } };
};
 ...
 * query parameter, matching the AIArb AuthMiddleware contract.
 */
...
export function hostToken(): string | null {
  try {
    ...
    return (
      parent?.AIArb?.host?.getApiToken?.() ||
      current.AIArb?.host?.getApiToken?.() ||
      null
    );
  } catch {
    return null;
