...
for (const required of ['registerRoutes', '/apps/aiarb-creator']) {
...
if (routeRegistration?.pluginId !== 'aiarb-creator') {
  throw new Error('plugin entry did not self-register under the aiarb-creator id');
}
const appRoute = routeRegistration.routes?.find(
  (route) => route.path === '/apps/aiarb-creator',
);
if (!appRoute || typeof appRoute.component !== 'function') {
  throw new Error('plugin entry did not register the /apps/aiarb-creator route');
}
...
listeners.get('message')({
  source: frameWindow,
  data: {
    type: 'aiarb-creator:navigation',
    path: '/project/project-two/assets?selected=asset-one',
  },
});
if (replacedUrl !== '/apps/aiarb-creator#/project/project-two/assets?selected=asset-one') {
...
if (
  restored?.type !== 'aiarb-creator:restore-route' ||
