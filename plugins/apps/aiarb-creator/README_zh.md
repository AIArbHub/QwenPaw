...
![AIArb Creator：Agentic 视频创作平台](https://img.alicdn.com/imgextra/i1/O1CN01mdypL9tClfC3FPZ2_!!6000000002252-2-tps-1600-600.png)

<p class="creator-lead">AIArb Creator 是一个 <strong>Agentic 视频创作平台</strong>：你负责提出目标、提供素材和把握方向，Agent 团队负责策划、生成、剪辑与合成，并在关键节点把决定权交还给你。</p>
...
Creator 通过 AIArb 的 **Apps（应用中心）** 安装和打开。启动 AIArb 并进入控制台（默认 `http://127.0.0.1:8088/`），在左侧选择 **Apps**；找到 **AIArb Creator** 并点击安装，安装完成后从同一位置打开。

<figure class="creator-figure">
  <img class="creator-shot" src="https://img.alicdn.com/imgextra/i1/O1CN01C3nur6L5b7C4fX2q_!!6000000006822-2-tps-2300-1520.png" alt="AIArb Apps 页面中的应用导航和 AIArb Creator 应用卡片" />
  <figcaption>在 AIArb 的 Apps 页面找到 AIArb Creator，完成安装后即可从同一位置打开。</figcaption>
</figure>
...
请打开 AIArb 控制台的 **Apps（应用中心）**，找到 **AIArb Creator** 并点击安装；安装完成后，直接从 Apps 打开 Creator。

Creator 会使用若干本地工具，但不会改动系统安装：`ffmpeg` 负责媒体处理与合成（可用 `CREATOR_FFMPEG_PATH` 指定，否则回退系统 `ffmpeg` 或 `imageio-ffmpeg`）；`jq` 支撑 Agent 对项目文件的结构化编辑（`CREATOR_JQ_PATH` 或 `PATH`）。依赖缺失时 Creator 以降级模式启动，可通过 `GET /api/aiarb-creator/health` 查看缺失项。
