...
DATAPAW_SOURCE_DIR="${DATAPAW_SOURCE_DIR:-$HOME/dev/AIArb-Data}"
UV_BIN="${UV_BIN:-uv}"

if [[ ! -f "$DATAPAW_SOURCE_DIR/pyproject.toml" ]]; then
  echo "AIArb-Data source workspace not found: $DATAPAW_SOURCE_DIR" >&2
  echo "Set DATAPAW_SOURCE_DIR to the AIArb-Data checkout." >&2
  exit 1
fi

if ! command -v "$UV_BIN" >/dev/null 2>&1; then
  echo "uv is required to create the isolated AIArb-Data environment." >&2
  exit 1
fi

echo "==> Syncing editable AIArb-Data workspace packages"
(cd "$DATAPAW_SOURCE_DIR" && "$UV_BIN" sync --all-packages)

DATAPAW_PYTHON="$DATAPAW_SOURCE_DIR/.venv/bin/python"
if [[ ! -x "$DATAPAW_PYTHON" ]]; then
  echo "AIArb-Data Python was not created: $DATAPAW_PYTHON" >&2
  exit 1
fi
...
echo "==> AIArb-Data development packages are ready"
