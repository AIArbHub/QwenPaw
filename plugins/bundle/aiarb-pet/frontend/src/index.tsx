// React and antd are injected by the AIArb console host at runtime;
...
//
// ``aiarb-host.d.ts`` declares the ``window.AIArb`` contract so the
 ...
 * blob keyed ``aiarb-agent-storage`` (sessionStorage takes precedence
 * over localStorage so each tab can pin its own agent). The AIArb API
...
function getSelectedAgentId(): string | null {
  try {
    const raw =
      window.sessionStorage?.getItem("aiarb-agent-storage") ??
      window.localStorage?.getItem("aiarb-agent-storage");
    if (!raw) return null;
...
function PetThumb({ folder }: { folder: string }) {
  ...
  React.useEffect(() => {
    ...
    (async () => {
      try {
        const url = getApiUrl(
          `/aiarb-pet/pets/${encodeURIComponent(folder)}/spritesheet`,
        );
...
function PetControlPage() {
  ...
  const refresh = React.useCallback(async () => {
    ...
    try {
      const [petData, st] = await Promise.all([
        apiGet("/aiarb-pet/pets"),
        apiGet("/aiarb-pet/status"),
      ]);
  ...
  const startDesktop = async () => {
    ...
    try {
      const r = await apiPost("/aiarb-pet/desktop/start", {});
      const h = r?.desktop;
  ...
  const submitImport = async () => {
    ...
    try {
      ...
      const res = await fetch(getApiUrl("/aiarb-pet/import-pet-upload"), {
        method: "POST",
        headers: authHeaders(),
        body: form,
      });
  ...
  const switchTo = async (row: PetRow) => {
    ...
    try {
      const r = await apiPost("/aiarb-pet/switch-pet", { pet_id });
      if (r && r.ok === false) {
...
class AIArbPetPlugin {
  readonly id = "aiarb-pet";

  setup(): void {
    const locale = resolvePetLocale();
    window.AIArb.registerRoutes?.(this.id, [
      {
        path: "/plugin/aiarb-pet/pets",
