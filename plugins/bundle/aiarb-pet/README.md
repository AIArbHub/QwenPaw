# AIArb Pet (plugin + desktop runtime)

Full-stack AIArb plugin: backend hooks, console UI sidebar, **and** the
desktop pet runtime itself (`aiarb_pet_desktop/`). Installing this
plugin gives you everything: the Qt floating pet window, the local HTTP
bridge on `127.0.0.1:8765`, and the AIArb side hooks that emit
...
├── router.py              # plugin HTTP routes under /api/aiarb-pet/...
...
└── aiarb_pet_desktop/   # the Qt + FastAPI desktop runtime (embedded)
    ...
    ├── cli.py             # `python -m aiarb_pet_desktop` subcommands
...
time, so `aiarb_pet_desktop` is importable from AIArb's Python
process without any `pip install` step. See
[`aiarb_pet_desktop/README.md`](aiarb_pet_desktop/README.md) for
...
- **From the AIArb console** — open the plugin management page and
  install `aiarb-pet` like any other AIArb plugin. Three input
  shapes are accepted:
...
- **From the shell** — run:
  ...
  ```bash
  aiarb plugin install ./plugins/aiarb-pet
  ```

> [!IMPORTANT]
> **Restart AIArb after install** — the backend only picks up new
...
AIArb's interpreter needs these packages on its `sys.path` (declared
...
If your AIArb install does not auto-resolve plugin dependencies,
install them manually into the same environment:

```bash
pip install -r plugins/aiarb-pet/requirements.txt
```

PySide6-Essentials wheels exist only for **Python 3.10–3.13**. On 3.14
the pet window cannot start; AIArb itself will still run, the pet
...
```python
subprocess.Popen(
    [sys.executable, "-m", "aiarb_pet_desktop.app",
     "--port", "8765"],
...
python -m aiarb_pet_desktop.app --port 8765 --scale 0.58

# CLI subcommands (daemonized: PID file + log redirect)
python -m aiarb_pet_desktop start
python -m aiarb_pet_desktop status
python -m aiarb_pet_desktop stop
python -m aiarb_pet_desktop switch --pet-id snowpaw
...
Autostart can be disabled with `AIARB_PET_AUTOSTART=0`.
...
`~/.copaw/plugins/aiarb-pet/dist/index.js` and refresh the console.
...
host at runtime (`window.AIArb.host.React`, `host.antd`,
`host.getApiUrl`, `host.getApiToken`). The shape of this contract is
declared in `frontend/src/aiarb-host.d.ts`. If the console host
...
The UI adds a sidebar page **Pet** (`/plugin/aiarb-pet/pets`): lists
pets under `<WORKING_DIR>/pets`, **Start desktop pet** (calls `POST
/api/aiarb-pet/desktop/start`), **Switch** (hot-switch via `POST
...
`POST /api/aiarb-pet/import-pet-upload`, then validated and copied
...
```text
GET  /api/aiarb-pet/status
GET  /api/aiarb-pet/pets
GET  /api/aiarb-pet/pets/{folder}/spritesheet
POST /api/aiarb-pet/desktop/start
POST /api/aiarb-pet/switch-pet
POST /api/aiarb-pet/import-pet          # JSON body — server-side path
POST /api/aiarb-pet/import-pet-upload   # multipart/form-data — browser
POST /api/aiarb-pet/emit-test
```
...
`X-AIArb-Pet-Token: <runtime/update-token>` by default — the bundled
AIArb plugin reads the token file automatically; standalone clients
must do the same. Set `AIARB_PET_REQUIRE_TOKEN=0` to disable the check
...
Row layout (driven by `aiarb_pet_desktop/sprites.py`):
...
(`aiarb_pet_desktop/assets/default-pet/snowpaw/`).
...
installed and cached under `~/.aiarb-pet/cache/snowpaw-spritesheet.webp`
for subsequent runs. Override the source with `AIARB_PET_SNOWPAW_URL`
...
`aiarb_pet_desktop/assets/default-pet/snowpaw/` and the bundled copy
...
- `register_startup_hook` — patches `AgentRunner.query_handler` and the
  ...
  `aiarb.startup`. Patch failures (e.g. an upstream rename of
  `AgentRunner` / `ApprovalService`) are reported via
  `logger.exception` so a broken plugin install does not stay silently
  dead.
- `register_shutdown_hook` — emits `aiarb.shutdown`, terminates the
  pet desktop process that this AIArb process has adopted (either
  autostarted by the plugin or already healthy on startup /
  `desktop/start`; controlled by `AIARB_PET_STOP_ON_SHUTDOWN`), and
  restores the patched class methods. So when the user exits AIArb
  the floating pet exits with it, including the case where the pet was
  a leftover from a previous AIArb run.
- `register_http_router` — mounts `router.py` under `/aiarb-pet`.

## Environment variables

| Var | Purpose | Default |
| --- | --- | --- |
| `AIARB_PET_DESKTOP_URL` | Full base URL for the plugin → desktop HTTP bridge. If set, port auto-fallback is **disabled** (URL and listener must agree). | unset ⇒ derive from host + port below |
| `AIARB_PET_DESKTOP_HOST` | Bind address when spawning (`--host`) | `127.0.0.1` |
| `AIARB_PET_DESKTOP_PORT` | Preferred port when spawning; if busy, the plugin scans upward (unless `DESKTOP_URL` is set or `STRICT` is on) | `8765` |
| `AIARB_PET_DESKTOP_STRICT_PORT` | `1` ⇒ never auto-pick another port (fail with EADDRINUSE if taken) | `0` |
| `AIARB_PET_DESKTOP_SCALE` | Spawn-time scale (e.g. `0.58`) | unset |
| `AIARB_PET_DESKTOP_PET_DIR` | Spawn-time pet folder override | unset |
| `AIARB_PET_TOKEN_PATH` | Path to the local update token | `~/.aiarb-pet/runtime/update-token` |
| `AIARB_PET_REQUIRE_TOKEN` | `0` ⇒ desktop *skips* the token check on mutating endpoints (anything else, including unset, enforces it) | `1` |
| `AIARB_PET_AUTOSTART` | `0` ⇒ plugin will not spawn the desktop | `1` |
| `AIARB_PET_STOP_ON_SHUTDOWN` | `0` ⇒ leave the pet desktop running after AIArb exits. Default: terminate any pet desktop AIArb has adopted (either by autostarting it or by seeing it healthy at startup / explicit `desktop/start`). | `1` |
| `AIARB_PET_HOME` | Runtime dir (PID file, log, **cache**, token) | `~/.aiarb-pet/` |
| `AIARB_PET_SNOWPAW_URL` | CDN URL for snowpaw's `spritesheet.webp` (downloaded once on first install) | Alicdn-hosted default |
| `AIARB_WORKING_DIR` / `COPAW_WORKING_DIR` | Where `pets/` lives | falls back to `~/.copaw` then `~/.aiarb` |

### Pet desktop log / common errors

`~/.aiarb-pet/runtime/pet-desktop.log` captures stderr from the spawned
desktop process.

- **`ModuleNotFoundError: No module named 'PySide6'`** — install Qt into the
  **same** Python as AIArb:
...
- **`[Errno 48] address already in use` (uvicorn)** — something else is bound
  ...
  that process, set `AIARB_PET_DESKTOP_PORT` to a free port, or unset
  `AIARB_PET_DESKTOP_URL` and let the plugin auto-pick the next free port
  after the preferred one.

The effective listen URL is mirrored under
`~/.aiarb-pet/runtime/desktop-bridge.json` (`url` field) so the plugin can
