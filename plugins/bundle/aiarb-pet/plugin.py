# -*- coding: utf-8 -*-
"""AIArb Pet backend plugin entry point."""
...
# ``aiarb plugin install`` execs this file as a plain module (no
...
from aiarb.plugins.api import PluginApi  # noqa: E402
...
# Logger uses ``aiarb.*`` so messages appear in the project logger
# (``~/.aiarb/aiarb.log``).
...
class AIArbPetPlugin:
    ...
    def register(self, api: PluginApi):
        ...
        logger.info("AIArb Pet plugin registered")
    ...
    def _startup(self):
        ...
        the desktop autostart and ``aiarb.startup`` emit going so the
        ...
        try:
            ...
            logger.exception(
                "AIArb Pet: failed to patch AgentRunner; "
        ...
        try:
            ...
            logger.exception(
                "AIArb Pet: failed to patch ApprovalService; "
        ...
        try:
            ...
            logger.info("AIArb Pet startup hook complete")
        except Exception:
            logger.exception("AIArb Pet startup hook failed")

    def _shutdown(self):
        """Notify the desktop, terminate it, and restore the runner patch.

        The pet desktop is treated as a child of AIArb: when AIArb
        exits, the floating window goes with it. ``stop_desktop`` only
        kills a process that this plugin instance spawned (so a desktop
        started independently via ``python -m aiarb_pet_desktop start``
        ...
        try:
            ...
            logger.warning(
                "AIArb Pet: shutdown event emit failed",
                exc_info=True,
            )

        try:
            result = stop_desktop(force=True, aggressive=True, grace=5.0)
            logger.info("AIArb Pet: stop_desktop result=%s", result)
        except Exception:
            logger.exception("AIArb Pet: failed to stop desktop process")

        try:
            restore_approval_service()
            restore_agent_runner()
        except Exception:
            logger.exception("AIArb Pet: failed to restore class methods")

        logger.info("AIArb Pet shutdown hook complete")


plugin = AIArbPetPlugin()