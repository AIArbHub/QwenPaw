# `aiarb_pet_desktop` (package internals)

Importable Python package embedded inside the **AIArb Pet plugin**.
It ships the desktop runtime: a transparent always-on-top Qt pet
window + a local FastAPI bridge on `127.0.0.1:8765` that AIArb (via
...
the package is importable as `aiarb_pet_desktop` from AIArb's
...
| `__main__.py` | `python -m aiarb_pet_desktop` → delegates to `cli.main`. |
| `cli.py` | CLI subcommands (`python -m aiarb_pet_desktop ...`): `init`, `start`, `stop`, `status`, `install-pet`, `install-default-pet`, `token`, `switch`, `send`. |
...
| `server.py` | FastAPI app factory (`build_app`). Defines `/health`, `/event`, `/state`, `/bubble`, `/pet`. Optional bearer-style token guard via `AIARB_PET_REQUIRE_TOKEN=1` + `X-AIArb-Pet-Token`. |
...
| `assets/default-pet/snowpaw/` | The bundled default pet (`pet.json` + `spritesheet.webp`). Loaded via `importlib.resources.files("aiarb_pet_desktop")`, which resolves relative to this folder on disk (no setuptools install required). |

## Runtime layout

`runtime.home_dir()` defaults to `~/.aiarb-pet/` (override with
`AIARB_PET_HOME`) and contains:
...
~/.aiarb-pet/runtime/
  ...
  pet-desktop.log    # stdout/stderr of detached `aiarb-pet start`
```

Pet **packages** live in a separate location resolved by
`runtime.aiarb_working_dir()`, with this precedence:

1. `AIARB_WORKING_DIR`
2. `from aiarb.constant import WORKING_DIR` (if AIArb is installed)
3. `~/.aiarb`
...
send `X-AIArb-Pet-Token: <token from runtime/update-token>` on
mutating endpoints (the bundled AIArb plugin reads the token file
automatically). To disable the check (e.g. for local development), set
`AIARB_PET_REQUIRE_TOKEN=0`.
...
These two are equivalent and bypass the `aiarb-pet start` daemon
wrapper (no PID file, no log redirection — useful while iterating):

```bash
python -m aiarb_pet_desktop.app --port 8765 --scale 0.58
python -m aiarb_pet_desktop                 # → cli.main, takes subcommands
```
