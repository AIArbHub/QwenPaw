...
def run_desktop(pet_dir: Path, host: str, port: int, scale: float) -> int:
    ...
    # Local AIArb plugin always calls ``127.0.0.1`` even when the
    ...
    # SIGTERM is what the AIArb plugin sends on shutdown (see
    ...
    try:
        ...
        # immediately sees ``running=False`` and the next AIArb start
