# -*- coding: utf-8 -*-
"""Command-line management for AIArb Pet Desktop."""
...
def command_init(_args: argparse.Namespace) -> int:
    ...
    print(f"AIArb working dir: {runtime.aiarb_working_dir()}")
    ...
    print("")
    print("Install the AIArb backend plugin with:")
    print("  aiarb plugin install ./plugins/aiarb-pet")
    return 0
...
def command_start(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        if health.get("ok"):
            print(f"AIArb Pet already running on port {args.port}")
            return 0
    except Exception:
        pass
    ...
    print(f"Started AIArb Pet Desktop (pid {process.pid})")
...
def command_switch(args: argparse.Namespace) -> int:
    ...
    if token:
        headers["X-AIArb-Pet-Token"] = token
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="aiarb-pet")
    ...
    g.add_argument(
        "--pet-id",
        help="Folder name under AIArb pets/ (e.g. snowpaw)",
    )
    p.add_argument("--port", type=int, default=8765)
