...
def _build_acp_config(spec: dict[str, Any]) -> Any:
    ...
    """
    try:
        from aiarb.config.config import ACPAgentConfig, ACPConfig
    except ImportError:
...
def _inject_llm_env(env: dict[str, str]) -> None:
    ...
    For iac-code >= 0.1.2: write llm_source: aiarb to settings.yml
    so iac-code reads config directly from AIArb.
    ...
    _write_aiarb_mode_to_settings()
    return


def _write_aiarb_mode_to_settings() -> None:
    """Write llm_source: aiarb to ~/.iac-code/settings.yml."""
    import yaml
    ...
    # Write llm_source: aiarb
    if settings.get("llm_source") != "aiarb":
        settings["llm_source"] = "aiarb"
        try:
...
def _initialize_agent_workspace(
    ...
    from aiarb.agents.skill_system import get_workspace_skills_dir
...
def _seed_persona_md_files(
    ...
    try:
        from aiarb.agents.prompt import _get_agent_md_dir
...
def _uninstall_plugin_skills() -> None:
    ...
    try:
        from aiarb.agents.skill_system import (
            get_skill_pool_dir,
...
def _install_workspace_skills(
    ...
    try:
        ...
        from aiarb.agents.skill_system.store import (
