...
def resolve_state_dir(
    ...
    Uses the AIARBMAIL_STATE_DIR environment variable when set (aiarb
    injects it, pointing at the agent workspace mail_state/ directory);
    otherwise falls back to ``~/.aiarbmail-mcp/state/<email>/``.
    """
    ...
    return Path.home() / ".aiarbmail-mcp" / "state" / safe
...
class ThreadStore:
    ...
    def process_transaction(self) -> Iterator[None]:
        ...
        AIArb's build-before-swap driver reload briefly runs the old and new
