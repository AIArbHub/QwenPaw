...
def create_server(
    ...
    def _get_store() -> ThreadStore:
        ...
        The state directory comes from the AIARBMAIL_STATE_DIR environment
        variable (injected by aiarb); when missing it falls back to
        ~/.aiarbmail-mcp/state/<email>/. Directories are auto-created.
    ...
    async def set_credentials(
        ...
        def _sync() -> dict:
            env = {
                "AIARBMAIL_EMAIL": email,
                "AIARBMAIL_AUTH_CODE": auth_code,
            }
            if imap_host:
                env["AIARBMAIL_IMAP_HOST"] = imap_host
            if smtp_host:
                env["AIARBMAIL_SMTP_HOST"] = smtp_host
