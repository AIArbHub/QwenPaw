...
让 AI Agent 通过 IMAP/SMTP 读取、搜索、发送和整理邮件的 stdio [MCP](https://modelcontextprotocol.io/)（Model Context Protocol）server。作为 [AIArb](../../README_zh.md) 的邮箱子包开发，也可独立配合任何支持 MCP 的客户端（如 Claude Desktop）使用。
...
其他域名（如自定义公司域名的企业邮）可通过显式设置 `AIARBMAIL_IMAP_HOST` / `AIARBMAIL_SMTP_HOST` 接入——见[配置](#配置)。

## 工作原理

aiarbmail-mcp 是纯 Python（>= 3.10）实现的 MCP server，基于 FastMCP SDK（`mcp>=1.28`）+ `imap-tools`，通过 **stdio** 传输通信。

在 AIArb 中，主进程会以子进程方式自动拉起本 server（`sys.executable -m aiarbmail_mcp`），并通过环境变量注入凭据与状态目录——用户无需手动启动。在 AIArb 之外，任何支持 stdio 传输的 MCP 客户端（Claude Desktop、Cursor 等）都可以用同样的方式启动它。

## 安装

**方式一：随 AIArb 一起安装**（主项目 + 本子包）：
...
| 变量 | 是否必需 | 说明 |
| --- | --- | --- |
| `AIARBMAIL_EMAIL` | 邮件操作需要* | 完整邮箱地址，如 `someone@163.com` |
| `AIARBMAIL_AUTH_CODE` | 邮件操作需要* | 登录凭据——**语义因服务商而异**（见下） |
| `AIARBMAIL_IMAP_HOST` | 未知域名需要 | IMAP host 覆盖（域名不在内置表中时） |
| `AIARBMAIL_IMAP_PORT` | 否 | IMAP 端口覆盖（默认取服务商配置或 993） |
| `AIARBMAIL_SMTP_HOST` | 未知域名需要 | SMTP host 覆盖（域名不在内置表中时） |
| `AIARBMAIL_SMTP_PORT` | 否 | SMTP 端口覆盖（默认取服务商配置或 465） |
| `AIARBMAIL_STATE_DIR` | 否 | 线程/标签本地索引的状态目录；默认 `~/.aiarbmail-mcp/state/<email>/`（AIArb 会将其指向 agent workspace 的 `mail_state/` 目录） |
| `AIARBMAIL_WORKSPACE_DIR` | 否 | Agent workspace 根目录，用于解析附件保存的相对路径（由 AIArb 注入） |

\* server 无凭据也能启动；需要凭据的工具在凭据设置（env 或 `set_credentials`）之前会返回友好的错误提示。

> **关于 `AIARBMAIL_AUTH_CODE`**：它*不一定*是登录密码，含义因服务商而异：
...
域名不在内置服务商表中时，需在 `env` 中额外添加 `AIARBMAIL_IMAP_HOST` 与 `AIARBMAIL_SMTP_HOST`。
