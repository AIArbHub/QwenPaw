...
A stdio [MCP](https://modelcontextprotocol.io/) (Model Context Protocol) server that lets AI agents read, search, send, and organize email over IMAP/SMTP. Built as the mail subpackage of [AIArb](../../README.md), it also works standalone with any MCP-compatible client such as Claude Desktop.
...
Any other domain (e.g. enterprise mail on a custom company domain) is supported by setting `AIARBMAIL_IMAP_HOST` / `AIARBMAIL_SMTP_HOST` explicitly — see [Configuration](#configuration).

## How It Works

aiarbmail-mcp is a pure-Python (>= 3.10) MCP server built on the FastMCP SDK (`mcp>=1.28`) plus `imap-tools`, communicating over **stdio**.

Within AIArb, the main process spawns it automatically as a subprocess (`sys.executable -m aiarbmail_mcp`) and injects credentials and state directories through environment variables — users never start it by hand. Outside AIArb, any MCP client that supports stdio transport (Claude Desktop, Cursor, etc.) can launch it the same way.

## Installation

This package lives in the AIArb monorepo under `packages/aiarbmail-mcp/` and is not published to PyPI.

**Option 1: With AIArb** (installs the main project and this subpackage together):
...
# or equivalently:
pip install -e packages/aiarbmail-mcp
```
...
| --- | --- | --- |
| `AIARBMAIL_EMAIL` | For mail ops* | Full email address, e.g. `someone@163.com` |
| `AIARBMAIL_AUTH_CODE` | For mail ops* | Login credential — **semantics vary by provider** (see below) |
| `AIARBMAIL_IMAP_HOST` | Unknown domains | IMAP host override for domains not in the built-in table |
| `AIARBMAIL_IMAP_PORT` | No | IMAP port override (default: provider value or 993) |
| `AIARBMAIL_SMTP_HOST` | Unknown domains | SMTP host override for domains not in the built-in table |
| `AIARBMAIL_SMTP_PORT` | No | SMTP port override (default: provider value or 465) |
| `AIARBMAIL_STATE_DIR` | No | State directory for the local thread/label index; defaults to `~/.aiarbmail-mcp/state/<email>/` (AIArb points it at the agent workspace's `mail_state/`) |
| `AIARBMAIL_WORKSPACE_DIR` | No | Agent workspace root, used to resolve relative attachment save paths (injected by AIArb) |
...
> **About `AIARBMAIL_AUTH_CODE`**: this is *not* necessarily your login password. Its meaning depends on the provider:
...
```bash
export AIARBMAIL_EMAIL="someone@mycompany.com"
export AIARBMAIL_AUTH_CODE="your-credential"
export AIARBMAIL_IMAP_HOST="imap.mycompany.com"
export AIARBMAIL_SMTP_HOST="smtp.mycompany.com"
```
...
For domains outside the built-in provider table, add `AIARBMAIL_IMAP_HOST` and `AIARBMAIL_SMTP_HOST` to `env`.
