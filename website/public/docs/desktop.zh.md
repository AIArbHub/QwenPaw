...
- 应用会在后台运行，打开应用窗口
- 应用日志输出到：`~/Library/Logs/io.aiarb.desktop/aiarb-desktop.log`
...
```bash
# 跟踪应用日志
tail -f ~/Library/Logs/io.aiarb.desktop/aiarb-desktop.log
...
- **日志位置**:
  - Windows: Debug 快捷方式终端查看；应用日志 `%LOCALAPPDATA%\io.aiarb.desktop\logs\aiarb-desktop.log`；后端 `%USERPROFILE%\.aiarb\desktop.log`
  - macOS: 应用日志 `~/Library/Logs/io.aiarb.desktop/aiarb-desktop.log`；后端 `~/.aiarb/desktop.log`
