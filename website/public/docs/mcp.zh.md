...
AIArb 通过 **MCP（模型上下文协议）** 连接外部服务，并提供一组 **内置工具**，让智能体能够访问文件系统、执行命令、浏览网页等。
...
1. **内置工具**：开箱即用，由 AIArb 核心提供，如文件操作、命令执行、浏览器自动化等
...
**MCP（模型上下文协议，Model Context Protocol）** 允许 AIArb 连接到外部 MCP 服务器，扩展智能体访问文件系统、数据库、API 等外部资源的能力。
...
为 QwenPaw 原生智能体配置邮箱后，系统会在该智能体工作区自动创建并启用
...
AIArb 提供了一组开箱即用的内置工具，智能体可以直接调用这些工具完成各种任务。
...
- AIArb 会在合适的时候调用 `delegate_external_agent`，与外部智能体建立连续对话，并将中间进度和结果回传到当前会话中
...
- `browser(code)`：用异步 Python 调用 AIArb 内置 Browser SDK 驱动真实浏览器——导航、定位元素、执行动作、读取可见页面状态。完整 SDK API 会在 agent 首次使用时自动交付。
  - 既可以使用 AIArb 启动的独立浏览器，也可以接入你自己已登录的 Chrome（需安装 [Chrome 浏览器扩展](./chrome)）
