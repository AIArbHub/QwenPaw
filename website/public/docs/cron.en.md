...
In AIArb, scheduled tasks (cron jobs) let the system run actions
...
> If creation fails, check [FAQ](https://aiarb.cn/docs/faq) under
...
2. Fill in fields:
   ...
   - **Task type and content**:
     ...
     - **agent**: send `content.text` to AIArb and deliver its response.
...
You can also tell AIArb directly in your target channel:
...
See [CLI `aiarb cron` section](./cli#aiarb-cron). Common commands:

```bash
aiarb cron list
aiarb cron create ...
aiarb cron state <job_id>
aiarb cron run <job_id>
aiarb cron pause <job_id>
aiarb cron resume <job_id>
aiarb cron delete <job_id>
```
...
Example (ask AIArb every 2 hours and deliver the response):
...
AIArb uses five-field cron: **minute hour day month weekday** (no seconds).
...
- [CLI `aiarb cron` section](./cli#aiarb-cron) — command reference
