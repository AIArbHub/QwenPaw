...
Browser is a built-in AIArb capability. Through the `browser` tool an Agent
...
| ---------------------- | ----------------------------------------------------------------------- | ---------------------------------------- |
| **Standalone browser** | AIArb launches its own Chromium, isolated from your everyday browsing | None                                     |
...
The Agent uses AIArb's own Browser SDK (not Playwright), and the API surface
...
| Value            | Behaviour                                                                                         |
| ---------------- | ------------------------------------------------------------------------------------------------- |
| `auto` (default) | AIArb launches and manages a Chromium                                                           |
...
By default AIArb prefers the Chromium-based executable of your system default
browser and falls back to the Chromium bundled with AIArb. Use
...
> apply next. **You must restart the AIArb service** for it to take effect;
...
This setting is not hot-reloaded. Restart the AIArb service and try again;
...
Yes. With `headless` set to `auto`, AIArb runs headless inside containers and
...
- [Chrome extension](./chrome) — connect AIArb to your own Chrome
