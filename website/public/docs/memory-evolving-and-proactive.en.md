...
> **Beta Feature**: AIArb's ReMeLight memory manager embeds [ReMe](https://github.com/agentscope-ai/ReMe) as an in-process application. Auto Memory, Auto Resource, Auto Dream, search, and ReMe's low-level proactive topic reader are ReMe jobs. AIArb's `/proactive` command is a separate runtime feature that reads recent chat sessions and optional screen context.

AIArb stores memory as files under the agent workspace. Conversations are saved as JSONL source logs, useful conversation facts are written to daily Markdown notes, resources can be converted into daily notes, and Auto Dream periodically integrates reusable abstractions into digest memory.
...
| AIArb `/proactive` | `src/aiarb/agents/memory/proactive`                        | `/proactive [minutes                                                                                        | on                                                                                     | off]` idle loop | A proactive chat request sent through `/api/console/chat` |

The important boundary is that `memory/<date>/interests.yaml` is produced by Auto Dream and can be read by ReMe's `proactive` job, but AIArb's current `/proactive` implementation does not call that job.

---

## File Layout

The embedded ReMe config comes from `src/aiarb/agents/memory/reme_config.py` and the user-facing defaults come from `ReMeLightMemoryConfig`.
...
When flushed, AIArb calls ReMe's `auto_memory` job with:
...
If the job succeeds but no note was changed, AIArb does not push an inbox event for `auto_memory`. Otherwise it pushes an inbox event titled `Auto-memory result`.

---

## Auto Resource

AIArb configures a ReMe background job named `resource_watch_loop`. It watches `resource_dir` and dispatches change batches to `auto_resource`.
...
Files can be placed directly in the `resource_dir` root, in which case AIArb's configured timezone determines the
...
Each change item may contain `path` or `file_path` and a `change` value such as `added`, `modified`, or `deleted`. The ReMe step interprets changed resource files into daily notes. AIArb pushes an `Auto-resource result` inbox event only when the job reports a real modification.

---

## Auto Dream

Auto Dream is exposed in AIArb through:
...
AIArb runs the ReMe job named `auto_dream` with `needs_llm=True`, so the embedded ReMe app refreshes its LLM component from the active AIArb model before the job runs.
...
When Auto Dream completes, AIArb pushes an inbox event titled `Auto-dream result`.
...
AIArb's current `/proactive` command is implemented under `src/aiarb/agents/memory/proactive`. It is separate from ReMe's `proactive` job.
...
When enabled, AIArb stores an in-memory `ProactiveConfig` for the session and starts a background loop. The loop:
...
The AIArb `memory_search` tool runs ReMe's `search` job with `query`, `limit`, and `min_score`. The job is configured as hybrid workspace search with vector recall, BM25 keyword recall, RRF fusion, and wikilink expansion. The storage backend in AIArb's embedded ReMe config is local.
...
- AIArb `/proactive` currently uses recent chat/session/screen context rather than ReMe interest topics;
