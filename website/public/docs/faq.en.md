...
Upgrading from CoPaw or QwenPaw: AIArb no longer reads the legacy data directories (`~/.copaw`, `~/.qwenpaw`) or the legacy `COPAW_*` / `QWENPAW_*` environment variables. Manually rename your old data directory to `~/.aiarb` (or set `AIARB_WORKING_DIR` to the old path), and rename any environment variables to the `AIARB_*` equivalents.
...
1. Download a quantized AIArb-Flash model from
   ...
   [AIArb-Flash-4B-Q4_K_M](https://www.modelscope.cn/models/AgentScope/AIArb-Flash-4B-Q4_K_M).

   - Download with ModelScope CLI:

     ```bash
     modelscope download --model AgentScope/AIArb-Flash-4B-Q4_K_M README.md --local_dir ./dir
     ```
...
[issue](https://github.com/agentscope-ai/AIArb/issues) in the AIArb GitHub
