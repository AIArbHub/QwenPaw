...
AIArb uses **MCP (Model Context Protocol)** to connect to external services and provides a suite of **built-in tools** that enable agents to access filesystems, execute commands, browse the web, and more.

---

## Concepts

AIArb provides two types of tools for agents:

1. **Built-in Tools**: Ready-to-use tools provided by AIArb core, such as file operations, command execution, and browser automation
...
**MCP (Model Context Protocol)** allows AIArb to connect to external MCP servers, extending the agent's ability to access filesystems, databases, APIs, and other external resources.
...
AIArb supports three JSON formats—choose one:
...
AIArb provides a set of ready-to-use built-in tools that agents can directly call to perform various tasks.
...
- AIArb will call `delegate_external_agent` when appropriate, establish a continuous conversation with the external agent, and stream progress and results back into the current chat
...
- `browser(code)`: Execute async Python against AIArb's built-in Browser SDK to drive a real browser — navigate, locate elements, act, and read visible page state. The complete SDK API is delivered to the agent automatically on first use.
  - Works with a standalone browser launched by AIArb, or with your own signed-in Chrome (requires the [Chrome extension](./chrome))
