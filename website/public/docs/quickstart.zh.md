...
| **AIArb Platform** | 无本地环境或快速体验     | 注册即可免费体验，无需安装，浏览器直接使用，支持 Beta 与正式版本 | 注册 AIArb Platform 账号 |
...
镜像在 **Docker Hub**（`aiarb/aiarb`）。镜像 tag：`latest`（稳定版）；`pre`（PyPI 预发布版）。国内用户也可选用阿里云 ACR：`aiarb-registry.ap-southeast-1.cr.aliyuncs.com/aiarb/aiarb`（tag 相同）。
...
## 方式四：AIArb Platform（无需安装）

[AIArb Platform](https://platform.agentscope.io/) 是 AIArb 生态的云端体验平台，提供 AIArb 的一键部署和在线运行环境，同时支持 Beta 版本和正式版本试用。注册即可免费体验，无需在本地安装或配置运行环境；登录后一键部署，即可直接在浏览器中使用 AIArb。

![AIArb Platform 中的 AIArb 一键部署入口与建议反馈入口](https://img.alicdn.com/imgextra/i4/O1CN01eJWlFV24HEB3V2ATv_!!6000000007365-2-tps-2914-1446.png)
...
2. 打开 [AIArb 创空间](https://modelscope.cn/studios/fork?target=AIArb/AIArb)，一键配置即可使用。
