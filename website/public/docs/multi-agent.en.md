...
AIArb supports **multi-agent**, allowing you to run multiple independent AI agents in a single AIArb instance.
...
Simply put, **multi-agent** lets you run multiple "personas" in one AIArb, where each persona:
...
You might use AIArb across multiple platforms:
...
After starting AIArb, you'll see the **Agent Selector** in the **top-left corner** of the console:
...
**Note**: After deletion, the workspace directory is retained (to prevent accidental data loss). To completely remove it, manually delete the `~/.aiarb/workspaces/{agent_id}` directory.
...
If you previously used AIArb **v0.0.x**, upgrading to **v0.1.0** will **automatically migrate**:
...
2. **Verify Migration**
   ...
   - After starting AIArb, check the agent list in console
...
aiarb skills config --agent-id abc123
...
```
~/.aiarb/workspaces/{agent_id}/PROFILE.md
```
...
# Workspace: ~/.aiarb/workspaces/code_reviewer
...
Workspace: ~/.aiarb/workspaces/writer_bot
Profile: [Auto-generated detailed capability description]
```
...
aiarb channels list --agent-id abc123
aiarb cron list --agent-id abc123
aiarb skills list --agent-id abc123
...
- `aiarb channels` - channel management
- `aiarb cron` - cron jobs
- `aiarb daemon` - runtime status
- `aiarb chats` - chat management
- `aiarb skills` - skill management

**Commands NOT Supporting `--agent-id`** (global operations):

- `aiarb init` - initialization
- `aiarb providers` - model providers
- `aiarb models` - model configuration
- `aiarb env` - environment variables
...
cp -r ~/.aiarb/workspaces/abc123 ~/backups/agent-abc123-$(date +%Y%m%d)

# Backup all agents
cp -r ~/.aiarb/workspaces ~/backups/workspaces-$(date +%Y%m%d)
```
...
AIArb also supports spawning ephemeral sub-tasks **within the current project**.
...
| ---------------------------------------- | ---------------------------------------------------------------------------------------------------------------- |
| Coding Mode ON + project_dir is git repo | Creates a **git worktree** under `<project_dir>/.aiarb/worktrees/`. Subagent works in the isolated worktree.   |
| Coding Mode OFF + workspace is git repo  | Creates a **git worktree** under `<workspace_dir>/.aiarb/worktrees/`. Subagent works in the isolated worktree. |
...
AIArb copies these files into the worktree when it is created, so the
...
```bash
git worktree list
git worktree remove .aiarb/worktrees/<id>
```
