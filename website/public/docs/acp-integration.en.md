...
2. **WORKING_DIR config**: read `agents.active_agent` from `config.json` inside `WORKING_DIR` (default `~/.aiarb`; overridable via the `AIARB_WORKING_DIR` environment variable)
