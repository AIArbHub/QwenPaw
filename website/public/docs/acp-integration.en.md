...
AIArb supports **ACP (Agent Client Protocol)** in two complementary ways:

1. **AIArb using ACP as a Tool**: AIArb connects to external ACP runners and uses them as delegated collaborators
2. **AIArb as an ACP Server**: external clients connect to AIArb over ACP

This page explains both modes and the scenarios each one fits best.

---

## AIArb using ACP as a Tool

In this mode, AIArb acts as an **ACP client / orchestrator**, connecting to **configured and enabled external ACP runners** and bringing them into the current session as delegated collaboration capabilities.

The actual entry point for this mode is the built-in tool `delegate_external_agent`. It is intended for scenarios where AIArb needs to collaborate with other ACP-capable external agent runtimes, such as the built-in examples `opencode`, `qwen_code`, `claude_code`, and `codex`. For more ACP-compatible agents, see the official ACP agent list and integration guide: <https://agentclientprotocol.com/get-started/agents>. In other words, AIArb does not directly talk to arbitrary external agents. It talks to runners that have been registered in ACP configuration, and it starts, continues, responds to, and closes delegated collaboration sessions through them.

### What this mode does

In this mode, AIArb uses the built-in `delegate_external_agent` tool to:
...
Conceptually, this lets AIArb treat an external agent as a collaborative, tool-like capability while keeping AIArb as the primary orchestrator of the main conversation.
...
Once the command-line side is ready, you can configure a custom runner in AIArb or collaborate with one of the built-in runners directly.
...
When an external ACP runner asks for permission, AIArb does **not** decide on the user's behalf.
...
This keeps delegated ACP execution aligned with the same user-controlled safety model used elsewhere in AIArb.

### When to use ACP as a Tool

Use this mode when:

- you want AIArb to collaborate with another agent runtime
- you have a specialized ACP-compatible external runner for a certain class of tasks
- you want AIArb to remain the primary orchestrator while delegating part of the work outward

### ACP Tool vs MCP

ACP as a Tool and MCP solve different problems:

- **MCP** connects AIArb to external services and tool servers
- **ACP as a Tool** connects AIArb to an external **agent** runtime
...
## AIArb as an ACP Server

In this mode, AIArb exposes itself as an [Agent Client Protocol (ACP)](https://github.com/agentclientprotocol/python-sdk) compliant agent service over stdio JSON-RPC. External clients, such as [Zed](https://zed.dev), [OpenCode](https://github.com/nicholasgasior/opencode), or any ACP-compatible editor, can connect to AIArb via the `aiarb acp` command and interact with it programmatically.

### Quick Start

```bash
# Start AIArb as an ACP agent
aiarb acp

# Use a specific agent profile
aiarb acp --agent mybot

# Use a custom workspace directory
aiarb acp --workspace /path/to/workspace
...
2. **WORKING_DIR config**: read `agents.active_agent` from `config.json` inside `WORKING_DIR` (default `~/.aiarb`, or `~/.copaw` for legacy installations; overridable via the `AIARB_WORKING_DIR` environment variable)
...
| Aspect               | AIArb as an ACP Server                      | AIArb using ACP as a Tool                            |
| -------------------- | --------------------------------------------- | ------------------------------------------------------ |
| AIArb's role       | Server / target agent                         | Client / orchestrator                                  |
| Connection direction | External client connects to AIArb           | AIArb connects to an external runner                 |
| Main purpose         | Let editors or external clients drive AIArb | Let AIArb delegate work to another agent             |
| Typical entry point  | `aiarb acp`                                 | Delegation tool + ACP runner configuration             |
...
ACP in AIArb is not just one feature. It supports both directions:

- **Expose AIArb outward** as an ACP server
- **Reach outward from AIArb** to external ACP agents as delegated tools

If you are integrating AIArb into another client, start with **ACP Server**.
If you want AIArb to coordinate with another agent runtime, use **ACP as a Tool**.
