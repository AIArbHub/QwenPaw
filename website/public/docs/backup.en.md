...
**Backup & Restore** lets you create snapshots of your AIArb instance: visually create, export, import, and restore your entire agent environment. It is designed for scenarios such as **rolling back before a version upgrade, migrating between machines, or keeping a safety net before risky changes**.
...
A backup is a single zip file (stored at `~/.aiarb.backups/<backup_id>.zip`) that may contain up to four kinds of content:

| Module               | Path                                | Actual content                                                                                                                                                                                                            |
| -------------------- | ----------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Agent workspaces** | `~/.aiarb/workspaces/<agent_id>/` | Every file inside each agent's workspace, including persona, memory, skills, chat history, channel config, plus public mail configuration, local mail indexes, and `credentials.yaml` with encrypted mailbox credentials. |
| **Global settings**  | `~/.aiarb/config.json`            | Runtime parameters, security rules, and other global settings.                                                                                                                                                            |
| **Skill pool**       | `~/.aiarb/skill_pool/`            | The globally shared skill repository.                                                                                                                                                                                     |
| **Secrets**          | `~/.aiarb.secret/`                | **LLM provider configuration (including API keys)**, plus environment variables used by tools and skills.                                                                                                                 |
...
A backup ID has the format `aiarb-<version>-<timestamp>-<short8>`, which makes it easy to identify the source version and creation time across machines.
...
2. Tick what you need:
   ...
   - **Secrets**: whether to include the secrets (i.e. the `~/.aiarb.secret/` directory). Off by default; turning it on shows the same red sensitive-content warning.
...
When you click the **Restore** button on any backup row, AIArb first opens the **Pre-restore backup** dialog:
...
- **Pick agents one by one**: restore only the agents you tick; agents outside the restore scope are kept as they are. You can also specify the default storage location for newly added agents at restore time; if unspecified, they go to `~/.aiarb/workspaces/<agent_id>/`.
...
| Item             | Path / Default                 |
| ---------------- | ------------------------------ |
| Backup directory | `~/.aiarb.backups/`          |
...
Add `-v aiarb-backups:/app/working.backups` to your `docker run` command:
...
A: AIArb shows an overwrite confirmation; confirm it to continue importing and overwrite the existing backup.
...
- [Mailbox Management](./mailbox) — mailbox credentials and workspace state files
