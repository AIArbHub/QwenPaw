...
3. 进入 **网络配置** → **新建** → **WebSocket 客户端**（反向 WS）：
   ...
   - Access Token：与 AIArb 配置中的 `access_token` 保持一致（AIArb 监听回环地址时可不填，否则必填）
...
- **OneBot 实现与 AIArb 同机时，`ws_host` 保持 `127.0.0.1`**。这是默认值，无需设置 Token。
...
> **Docker Compose 提示：** AIArb 和 NapCat 一起用 Docker Compose 部署时，两个容器不在同一个回环网口上，因此需将 `ws_host` 设为 `0.0.0.0` 并**同时设置 `access_token`**，NapCat 的反向 WS 地址填 `ws://aiarb:6199/ws`（使用服务名）。不要将 6199 端口 publish 到宿主机，或按 `127.0.0.1:6199:6199` 的形式 publish 以保持仅本机可访问。
