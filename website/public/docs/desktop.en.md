# AIArb Desktop Application Guide
...
1. **Download the installer**
   Download `AIArb-Tauri-<version>-Windows-setup.exe` from the [Release page][releases]

2. **Run the installer**
   Double-click the `.exe` file and follow the installation wizard
   - Default installation location: `C:\Users\<your-username>\AppData\Local\AIArb Desktop`
...
#### **AIArb Desktop** (Recommended for daily use)
...
- **How to Launch**: Double-click the "AIArb Desktop" icon on desktop or Start Menu
- **Technical Note**: Native Tauri desktop app; runs the Python backend as a background sidecar

#### **AIArb Desktop (Debug)** (Debug Mode)
...
- **How to Launch**: Double-click the "AIArb Desktop (Debug)" icon in Start Menu
- **Log Contents**:
...
A: Use "AIArb Desktop (Debug)" mode to view terminal output for error messages

**Q: How to uninstall?**

A: Go to Windows Settings → Apps → Installed apps → Find "AIArb Desktop" → Uninstall
...
1. **Download the archive**
   Download `AIArb-Tauri-<version>-macOS.zip` from the [Release page][releases]

2. **Extract**
   Double-click the `.zip` file to extract and get `AIArb Desktop.app`

3. **Move to Applications folder (Optional)**
   Drag `AIArb Desktop.app` to the `/Applications` folder

### First Launch: Bypassing System Security Restrictions

#### Why manual trust is needed?

AIArb is **not Apple Developer-signed or notarized**, so macOS Gatekeeper will block it by default.
...
1. **Right-click** (or Control + click) on `AIArb Desktop.app`
...
2. Scroll down to find a message like:
   _"'AIArb' was blocked from use because it cannot verify the developer"_
...
- Double-click `AIArb Desktop.app` to launch
- The app runs in the background and opens its window
- App logs are written to: `~/Library/Logs/io.agentscope.aiarb.desktop/aiarb-desktop.log`
- Backend sidecar logs live in the working directory: `~/.aiarb/desktop.log`
...
AIARB_DESKTOP_DEBUG=1 "/Applications/AIArb Desktop.app/Contents/MacOS/aiarb-desktop"
```
...
```bash
# Tail the app log
tail -f ~/Library/Logs/io.agentscope.aiarb.desktop/aiarb-desktop.log
...
A: Drag `AIArb Desktop.app` to the Trash, then delete the `~/.aiarb` configuration folder
...
- **GitHub Issues**: [Submit an issue](https://github.com/agentscope-ai/AIArb/issues)
...
- **Log locations**:
  - Windows: View in the Debug shortcut terminal; app log `%LOCALAPPDATA%\io.agentscope.aiarb.desktop\logs\aiarb-desktop.log`; backend `%USERPROFILE%\.aiarb\desktop.log`
  - macOS: app log `~/Library/Logs/io.agentscope.aiarb.desktop/aiarb-desktop.log`; backend `~/.aiarb/desktop.log`
