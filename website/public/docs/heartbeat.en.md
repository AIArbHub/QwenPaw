...
In AIArb, **heartbeat** means: on a fixed interval, ask AIArb the
“questions” you wrote in a file, and optionally send the AIArb’s reply to
**the channel where you last chatted**. Good for “regular check-ins, daily
digests, scheduled reminders” — AIArb runs without you sending a
message.
...
1. In the current agent’s workspace there is a **heartbeat query file** (default
   name **HEARTBEAT.md**; rename with env **`AIARB_HEARTBEAT_FILE`**). Its
   content is **what to ask AIArb on each run** (one or more paragraphs; AIArb
...
2. When **`enabled` is true** in config, the system runs on your **every**
   ...
   the user message → AIArb replies.
3. **Whether the reply goes to a channel** is set by **target**:
   - **main** — Run AIArb only; don’t send the reply to any channel (e.g. local
     self-check, logs).
   - **last** — Send the reply to the **channel/session where you last talked
     to AIArb** (e.g. if you last used DingTalk, the heartbeat reply goes to
     DingTalk).
...
`<AIARB_WORKING_DIR>/workspaces/<agent_id>/HEARTBEAT.md`.
Default `AIARB_WORKING_DIR` is `~/.aiarb` (override with **`AIARB_WORKING_DIR`**);
`<agent_id>` is the current agent id (e.g. `default`).

The default filename is `HEARTBEAT.md`; use **`AIARB_HEARTBEAT_FILE`** to change
...
If you ran `aiarb init` without `--defaults`, you may be prompted to edit
...
Example (heartbeat on, AIArb only, no channel, every 30m) — in that agent’s
**`agent.json`**:
...
> [CLI](./cli) `aiarb cron create`) instead of heartbeat.
