...
The TUI ships with the `aiarb` CLI. Running `aiarb` with no arguments opens it directly:

```bash
aiarb                       # open a chat with the active agent
aiarb tui                   # same thing, with explicit options
aiarb tui --agent NAME      # chat with a specific agent
aiarb tui --resume <id>     # resume a previous session and continue it
```

To start a **project-bound (Coding Mode)** session, point it at a directory:

```bash
aiarb .                     # use the current directory as the project
aiarb tui /path/to/repo     # use an explicit directory
```
...
> No separate install or server to manage: the TUI launches its own `aiarb acp` backend using the interpreter it ships in, so it always drives the same install/venv as your `aiarb` command.
...
**Sessions & resume.** Every chat is a session shared with the rest of AIArb. Use `/resume` to pick a recent session from the suggestions, or `/resume list` to browse all of them. You can also resume on launch with `aiarb tui --resume <id>`.
...
<text x="146" y="98" text-anchor="middle" font-size="17" font-weight="700" fill="currentColor">AIArb TUI</text>
...
<text x="614" y="86" text-anchor="middle" font-size="17" font-weight="700" fill="currentColor">aiarb acp</text>
...
- [CLI](/docs/cli) — other `aiarb` subcommands
