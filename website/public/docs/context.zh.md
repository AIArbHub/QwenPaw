...
AIArb 当前默认的上下文策略是 **scroll**：旧轮次不会被总结后丢弃，而是先写入持久化 SQLite 历史库；当模型窗口接近上限时，再把中间历史从实时上下文中驱逐出去，并用一条紧凑的上下文内索引表示。之后 Agent 可以按需把原始历史读回来。

Scroll 是面向用户的默认方案。已有的 `strategy: "native"` 配置仍会为向后兼容和安全降级而被接受，但控制台不再提供策略切换入口。

## 三种记忆系统

AIArb 把记忆组织为三套互补的系统——工作记忆（Working）、情景记忆（Episodic）和语义记忆（Semantic）——大致对应人类记忆，每套由不同子系统负责：
...
- **安全降级**：如果 scroll 无法构建，或 recall 工具无法安全运行，AIArb 会退回 native 上下文管理，避免把历史驱逐到无法读取的位置。
...
如果当前 SQLite 支持 FTS5，AIArb 会维护 `conversation_history_fts` 全文索引；否则 `ms.search` 会降级为较慢的 `LIKE` 扫描。
...
Recall API 是情景记忆的接口：把工作记忆驱逐后留下的、持久且逐字的历史读回来。scroll 启用时，AIArb 会注入两个工具：
...
安全说明：`recall_history_python` 会运行模型生成的 Python。正常情况下，它需要治理层注入 sandbox 配置；如果没有 sandbox，它会默认拒绝执行。（`recall_history` 不受影响：它从不执行模型生成的代码，所以在没有可用沙箱后端或沙箱被禁用时仍能正常运行。AIArb 支持原生 Windows 沙箱后端，WSL2 本身不是启用沙箱的前提。）只有同时满足以下条件时才允许 REPL 非沙箱运行：

- 环境变量 `AIARB_ALLOW_UNSANDBOXED_RECALL` 为 truthy
...
启用统一 pruning 时，AIArb 会让 AgentScope 内置的 token 工具结果上限不再触发，避免已经按 bytes 裁剪的 preview 被二次截断并丢失按文本块隔离的恢复 metadata。关闭统一 pruning 时，AgentScope 的默认上限仍作为安全兜底保留。
...
AIArb 只会在上下文足够长，并且预计视觉替换确实能够节省 Token 时应用 Visual Compact。较短的请求或没有明显收益的请求会保持原样。
...
- **如何查看**
  1. 将环境变量 `AIARB_LOG_LEVEL` 设为 `debug`，然后重启 AIArb。
  2. 完成一次较长的请求后，打开工作目录下的 `aiarb.log`（也可以使用 `/daemon logs`），搜索 `Visual Compact transform`。
...
- AIArb 会在应用视觉压缩时提供精确原文恢复工具，但模型不一定总会主动调用，也可能搜索了错误的证据。
