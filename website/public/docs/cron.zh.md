...
在 AIArb 里，「定时任务（cron job）」用于让系统在指定时间自动执行动作，比如：
...
> 如果定时任务没有创建成功，可以参考 [FAQ](https://aiarb.cn/docs/faq) 的 **定时任务错误排查** 寻找原因
...
2. 填写信息：
   ...
   - **任务类型及内容**
     ...
     - 选择**agent**：填写**请求内容**，会定时向AIArb转发content.text中的请求文本
...
创建定时任务最简单的方式是直接与 AIArb 对话，让AIArb帮忙创建：
...
详见 CLI的 [aiarb cron](./cli#aiarb-cron) 章节。常用命令：
...
- [CLI](./cli#aiarb-cron) — `aiarb cron` 命令详解
