...
Even without Goal Mode or Mission Mode, AIArb has loop control mechanisms that protect Agent behavior. You can configure these in the Console under **Runtime Config → Agent Loop Settings**.
...
AIArb's loop system is fully pluggable. You can register custom loop behavior through the plugin API, implementing your own "when to stop, when to continue" logic.
...
```python
from aiarb.loop.gates import StopHandler
...
AIArb uses a **Gate system** to manage loop termination logic. Think of Gates as quality checkpoints on an assembly line — after each round of work, all Gates are checked in sequence.
