...
<p align="center">
  <img src="https://img.alicdn.com/imgextra/i3/O1CN01mG5Uot1GQdX33v4h4_!!6000000000617-55-tps-1200-640.svg" alt="AIArb 长期记忆从捕获、整理到检索与发现的全景" />
</p>
...
假设上周你告诉 AIArb：“生产发布前先验证 staging，发布说明要写清风险和回滚步骤。”几天后团队又补了一条例外：“紧急 hotfix 经负责人批准可以先发布，但事后必须补做检查。”
...
发送方式是回调 AIArb 自己的 API（`POST /api/console/chat`），会话固定为 `proactive_mode:<agentId>`，超时 300 秒。也就是说，**主动消息进入一个专属会话，不会插进你正在进行的对话中间。**
...
`/proactive` 在开启时会显示这条警告。只在这些访问确实合适时才开启，随时可以用 `/proactive off` 停掉。另外，监控配置**只存在于进程内存中**，AIArb 重启后需要重新开启。
...
开启后会返回当前空闲阈值和上面那条安全警告。参数写错时会提示用法。重启 AIArb 后配置丢失，需要重新执行。
