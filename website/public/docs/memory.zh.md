...
**长期记忆** 让 AIArb 拥有跨对话的持久记忆能力。默认后端会在 AIArb 进程内嵌入 ReMe 应用，并通过 ReMe jobs
...
| 能力               | 说明                                                                                    |
| ------------------ | --------------------------------------------------------------------------------------- |
| **嵌入式 ReMe**    | AIArb 在进程内启动 ReMe，并将当前 Agent 使用的 AIArb 模型注入到 ReMe 默认 LLM 组件  |
...
| **Inbox 通知**     | `auto_memory`、`auto_dream`、`auto_resource` 产生结果时，会推送到 AIArb inbox         |
