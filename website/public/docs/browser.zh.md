...
浏览器是 AIArb 的内置能力。Agent 通过 `browser` 工具编写异步 Python 代码，驱动一个真实的浏览器完成打开网页、填表、点击、读取页面内容等任务。它既可以启动一个独立的浏览器，也可以接入你自己日常使用、已经登录的 Chrome。
...
| 用哪个浏览器        | 说明                                                    | 前置条件                           |
| ------------------- | ------------------------------------------------------- | ---------------------------------- |
| **独立浏览器**      | AIArb 自己启动一个 Chromium，与你的日常浏览器互不干扰 | 无                                 |
...
Agent 使用的是 AIArb 自带的 Browser SDK（不是 Playwright），API 是封闭的一套：只有 SDK 明确提供的方法可用。完整 API 参考随内置的 **browser** 技能一起交付给 Agent，你不需要手动配置。
...
| 取值           | 行为                                                                               |
| -------------- | ---------------------------------------------------------------------------------- |
| `auto`（默认） | 由 AIArb 启动并管理一个 Chromium                                                 |
| `launch`       | 同 `auto`，显式声明由 AIArb 启动                                                 |
| `managed_cdp`  | 启动一个带调试端口的 Chromium 再通过 CDP 控制；端口见 `cdp_port`，`0` 表示自动分配 |
| `connect_cdp`  | 连接到一个已经在运行的 CDP 端点；必须同时设置 `cdp_url`                            |

默认情况下 AIArb 优先使用你系统默认浏览器的 Chromium 内核可执行文件，找不到时回退到 AIArb 自带的 Chromium。可用 `use_system_default` 和 `executable_path` 干预。
...
> 切换后卡片上会提示"重启服务后将切换为：…"。**必须重启 AIArb 服务**才会真正生效，这一项不支持热加载。

---

## 配置项

浏览器配置位于全局 `~/.aiarb/config.json` 的 `browser` 段，对所有智能体生效：
...
这一项不热加载。请重启 AIArb 服务后再试；工具卡片上出现"重启服务后将切换为：…"说明改动已保存、等待重启。
...
- [Chrome 浏览器扩展](./chrome) — 把 AIArb 连接到你自己的 Chrome
