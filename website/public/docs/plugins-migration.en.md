...
- Frontend plugins that use the `window.AIArb.*` Host SDK
...
The new version recommends using `aiarb_version`:
...
So if you drop a legacy plugin into the new version unchanged and it only has `"min_version": "1.1.10"`, the new version interprets it as `>=1.1.10, <1.2.0`, which is judged incompatible under AIArb 2.0.x.

### Manifest Field Reference

| Field                                                                                                               | Type     | Legacy                                   | New                                                                               | Migration advice                                            |
| ------------------------------------------------------------------------------------------------------------------- | -------- | ---------------------------------------- | --------------------------------------------------------------------------------- | ----------------------------------------------------------- |
| `aiarb_version`                                                                                                   | `object` | Undefined, ignored                       | New, recommended                                                                  | New plugins should add this field                           |
| `aiarb_version.min`                                                                                               | `string` | Undefined                                | Minimum compatible AIArb version, inclusive                                     | Set to the lowest new version you actually verified against |
| `aiarb_version.max`                                                                                               | `string` | Undefined                                | Highest compatible AIArb version, exclusive                                     | Recommended to set explicitly                               |
| `min_version`                                                                                                       | `string` | Supported, but not enforced at load time | Legacy field, only used for compatibility checks when `aiarb_version` is absent | Keep it if you need legacy compatibility                    |
...
The legacy version ignores the unknown `aiarb_version` field. The new version reads `aiarb_version` first, and only falls back to `min_version` / `max_version` when that field is absent.
...
Registers an AIArb middleware factory. The factory is called each time an agent is built, and returns a `MiddlewareBase` instance or `None`.
...
The new version continues to support the existing `window.AIArb.*` frontend Host SDK. Plugins using existing legacy frontend APIs can usually run without changes.

| API                                                            | Type       | Purpose                                                             |
| -------------------------------------------------------------- | ---------- | ------------------------------------------------------------------- |
| `window.AIArb.host`                                          | Compatible | Access React, Ant Design, API helpers, runtime state, etc.          |
| `window.AIArb.menu`                                          | Compatible | Register sidebar menu items                                         |
| `window.AIArb.route`                                         | Compatible | Register page routes                                                |
| `window.AIArb.slot`                                          | Compatible | Register UI slots                                                   |
| `window.AIArb.chat.requestPayload.add(pluginId, fn, opts?)`  | New        | Append or rewrite request body fields before a chat request is sent |
| `window.AIArb.chat.response.set(pluginId, { avatar, nick })` | New        | Set the avatar and nickname for the default AI reply card           |
...
The new version's plugin marketplace catalog filters entries by AIArb version. The filtering rules match the loader:

| Field                 | Type     | Read priority | Description                                                                        |
| --------------------- | -------- | ------------- | ---------------------------------------------------------------------------------- |
| `aiarb_version`     | `object` | 1             | Recommended field, same format as the plugin manifest                              |
| `aiarb_version.min` | `string` | 1             | Minimum compatible AIArb version, inclusive                                      |
| `aiarb_version.max` | `string` | 1             | Highest compatible AIArb version, exclusive                                      |
| `min_version`         | `string` | 2             | Legacy field, only used when `aiarb_version` is absent                           |
| `max_version`         | `string` | 2             | Legacy field, only used when `aiarb_version` is absent                           |
| No version constraint | -        | 3             | Treated as compatible, but omitting constraints is not recommended when publishing |

When publishing a new-version plugin, keep the version constraints in the packaged `plugin.json` consistent with the marketplace index entry. If an existing entry only has `"min_version": "1.1.10"`, it may get filtered out under AIArb 2.0.x.

## Migration Steps

1. Update `plugin.json`, adding `aiarb_version` with an explicit `max`.
...
6. Start AIArb and check the logs for `is incompatible` or plugin registration failure messages.
...
Yes. The plugin code should only use APIs that exist on both sides, or check the version before calling a new-version-only API. The manifest can keep both `min_version` and `aiarb_version`:
