...
3. Go to **Network Config** → **New** → **WebSocket Client** (reverse WS):
   ...
   - Access Token: same as `access_token` in AIArb config (required unless AIArb listens on loopback)
...
> **Docker Compose tip:** When running AIArb and NapCat in Docker Compose, the
...
> `ws://aiarb:6199/ws` (using the service name). Do not publish port 6199 to
