---
title: "Introducing AIArb Hub: A Managed AIArb for Every User"
date: 2026-08-19
author: AIArb Team
tags: [AIArb Hub, Self-hosted, Multi-user, Docker]
excerpt: "AIArb Hub brings unified accounts, isolated personal runtimes, Local and Docker backends, and practical security controls to self-hosted teams."
related:
  heading: "Get started with AIArb Hub"
  description: "Follow one guide from first initialization through runtime, security, and operations."
  items:
    - label: "Documentation"
      name: "Deploying and Managing AIArb Hub"
      href: "/docs/hub"
---

# Introducing AIArb Hub: A Managed AIArb for Every User

AIArb began with a personal model: one person installs one AIArb and owns its models, memory, Skills, and workspace. When a team wants to offer AIArb on its own server, different questions appear. Who can sign in? Where do user files and secrets live? Can one runtime affect another? Can a user undo an administrator's stop merely by refreshing the page? Who selects Docker images and resource limits?

AIArb Hub answers those questions with a unified entry point and one independently managed AIArb runtime per account. It does not change the single-user product: personal users continue to run `aiarb app`.
...
![AIArb Hub sign-in page and admin-center overview](https://img.alicdn.com/imgextra/i3/O1CN01pmq9btaadNF6mRvU_!!6000000004218-2-tps-3340-1772.png)

## One entry point, personal runtimes

```mermaid
flowchart LR
    Browser[User browser] --> Hub[AIArb Hub]
...
Hub owns accounts, authorization, runtime lifecycle, and request routing. Each user still sees the familiar AIArb Console, while conversations, files, model settings, and integration credentials stay with that user's runtime.

Users never need to enter internal ports or access a host `127.0.0.1` endpoint directly. They can work in AIArb, manage personal credentials, change their password, and restart a failed or normally stopped runtime. Backend, image, and capacity policy remain administrative decisions.
...
AIArb Hub is self-hosted software, not a SaaS operated by the AIArb team. The instance operator controls the server, database, backups, and processes and may be able to access user data. Users should sign in only to an instance run by themselves or a trusted organization.
...
Read the [AIArb Hub documentation](/docs/hub) to initialize an administrator and configure runtime, Docker, access security, OAuth, backups, and operations. Personal users do not need to migrate—`aiarb app` remains the original single-user AIArb.
