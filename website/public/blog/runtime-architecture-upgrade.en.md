...
`Envelope` (`src/aiarb/runtime/envelope.py`) is an SSE state machine that translates aiarb `EventType` events into the frontend's streaming envelope protocol, tracking per-request state (text blocks, reasoning blocks, tool calls, data blocks) and producing the correct event sequence.
