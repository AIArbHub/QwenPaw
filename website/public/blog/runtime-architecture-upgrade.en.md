---
title: "AIArb Runtime Architecture Upgrade: Design, Implementation & Extensibility"
date: 2026-07-07
author: AIArb Team
tags: [ArchitectureUpgrade, EightPhaseOrchestration, HookSystem, AIArb2.0]
cover: /blog/runtime-architecture-upgrade-cover.png
excerpt: "AIArb Runtime was refactored from a 650-line god method into an 8-phase orchestration engine—enabling one-feature-one-directory extensibility with zero changes to Runtime core code."
---

# AIArb Runtime Architecture Upgrade: Design, Implementation & Extensibility
...
```python
# src/aiarb/app/app_services/app_service_manager.py
...
The `Runtime` class (`src/aiarb/runtime/runtime.py`) is the per-workspace request orchestrator. Each request invokes `run()`, which produces SSE envelope objects through the 8-phase lifecycle.
...
Defined in `src/aiarb/runtime/phases.py`:
...
Each hook returns a `HookResult` containing one of three actions (`src/aiarb/runtime/hooks.py`):
...
`AgentBuilder` (`src/aiarb/runtime/builder.py`) is responsible for constructing a complete `AIArbAgent` for each request. It integrates:
...
`AgentExecutor` (`src/aiarb/runtime/executor.py`) drives the agent's reply stream and wraps a heartbeat mechanism — ensuring that during long idle periods (e.g., tool-guard approval waits), keep-alive envelopes are emitted instead of letting the SSE connection drop.

`Envelope` (`src/aiarb/runtime/envelope.py`) is an SSE state machine that translates agentscope `EventType` events into the frontend's streaming envelope protocol, tracking per-request state (text blocks, reasoning blocks, tool calls, data blocks) and producing the correct event sequence.
...
`HookRegistry` (`src/aiarb/runtime/hooks.py`) manages hook registration and execution:
...
`AgentMode` (`src/aiarb/modes/base.py`) is a **behavior bundle**: commands, tools, hooks, and prompt contributors packaged together.
...
All per-workspace registries reside in `WorkspacePlugins` (`src/aiarb/app/workspace/workspace_plugins.py`):
...
Previously, system prompt construction was scattered across mixin, runner, and env modules; adding a new prompt fragment required finding the correct injection point. Now, `PromptManager` (`src/aiarb/runtime/prompt_manager.py`) declaratively assembles the system prompt from ordered `PromptContributor` instances:
...
The previous pain point was that tools were hardcoded in the agent class; adding a tool required ~80 lines of invasive changes to the agent class. `ToolRegistry` (`src/aiarb/runtime/tool_registry.py`) replaces the old hardcoded tool dictionary with a declarative, filterable registry.
...
Previously, AIArb had **four parallel command mechanisms**: conversation, control, daemon, and skill, each method written 3 times. `SlashCommandRegistry` (`src/aiarb/runtime/slash_command_registry.py`) unifies them into a single dispatch point.
...
Addressing the previously listed pain point of "three duplicate external interfaces," `@api_action` (`src/aiarb/api_action.py`) fundamentally solves this problem: **one decorator simultaneously generates HTTP API, CLI subcommands, and slash commands**.
...
`ToolCoordinator` (`src/aiarb/tool_calls/`) provides **single tool call granularity** control. In the old system, `/stop` could only kill the entire agent. Now individual tool calls can be tracked, cancelled, or backgrounded.
...
`ServiceManager` (`src/aiarb/app/workspace/service_manager.py`) manages per-agent services, supporting:
...
Suppose you are a developer wanting to add a new Mode to AIArb. You **do not need to modify a single line of Runtime code**.
