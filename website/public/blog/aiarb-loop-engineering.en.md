---
title: "Knowing When to Continue and When to Stop: AIArb Loop Engineering & Custom Loops"
date: 2026-07-29
author: AIArb Team
...
excerpt: "AIArb ships Default, Goal, and Mission loops—and from 2.0.1 you can compose Gates in the Console with zero code to set completion criteria, resource budgets, and stop-loss boundaries."
---

# Knowing When to Continue and When to Stop: AIArb Loop Engineering & Custom Loops
...
AIArb's **Loop Engineering** provides an explicit control layer for both cases. If Skills define _what_ an Agent can do, Loop Engineering defines _how_ it keeps going and _when_ it hands control back to you.

AIArb 2.0 ships three built-in loops—Default, Goal, and Mission. Starting in AIArb 2.0.1, you can also create your own loops directly in the Console, with no code required.
...
After each model response or tool call, AIArb evaluates the Gates configured for the current mode.
...
Suppose you often ask AIArb for industry and technical research. Create a mode named "Deep Research" with slash command:
...
Visual custom Loops fit combining AIArb's seven Gates. If stop conditions depend on external system state, an independent review model, or a dedicated state machine, register new Agent Modes via plugins.

Oh-My-Paw (OMP Workflows) in AIArb 2.0.1 is an example of this extension layer:
