---
title: "Introducing the AIArb 2.0 Sandbox"
date: 2026-07-29
author: AIArb Team
...
excerpt: "Sandbox is AIArb's built-in isolation layer: kernel-level filesystem isolation, sensitive-path blocking, environment scrubbing, and approval escalation on violations—so agents only see and change what they should."
---

# Introducing the AIArb 2.0 Sandbox

## 1. What Is Sandbox

Sandbox is AIArb's built-in security isolation layer. When an AI Agent runs Shell commands or executes code, Sandbox confines those commands to a controlled environment—commands can only access files and resources you allow, and cannot reach sensitive information on the system.
...
AIArb Sandbox is built around four core principles:
...
AIArb's security decisions proceed in multiple phases:
...
On the AIArb Console Security settings page, toggle the Sandbox switch.
...
**Windows**: Run AIArb as administrator.
...
When a violation is detected, AIArb shows an approval card:
...
AIArb's governance system has four execution levels:
...
Windows therefore provides a **configuration fingerprint reuse** mechanism: the system computes a SHA256 fingerprint from `workspace_dir`, `mounts`, `deny_paths`, `network_allow`, and related parameters, and generates a deterministic sandbox name (`aiarb_<fingerprint>`). Subsequent calls with the same configuration reuse the existing Profile and ACLs, skipping creation—runtime cost comparable to Linux/macOS. Recreation is needed only when configuration changes (e.g. workspace switch, mount path change).

### 10.4 Cleanup

**Automatic cleanup (normal exit)**:

When the AIArb process exits, a cleanup function registered via `atexit` automatically:
...
The cleanup function is idempotent and safe to call multiple times. It also skips sandbox instances owned by other still-running AIArb processes.
...
When automatic cleanup fails, leftover sandbox artifacts (ACLs, user accounts, firewall rules) remain on the system. They are cleaned up when AIArb is started again and exits normally. You can also clean up manually:
...
This script scans all metadata files under `~/.aiarb/`, identifies orphan sandboxes (owner process no longer exists), and runs the full cleanup flow.
