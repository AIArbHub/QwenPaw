---
title: "在 AIArb 里使用 Codex 和 Qoder：Agent OS 的跨 Harness 实践"
date: 2026-08-05
author: AIArb Team
tags: [Agent OS, Harness, Codex, Qoder, Skills, MCP]
excerpt: "AIArb 从构建 Agent OS 出发，将 Codex、Qoder 等专业 Agent Harness 接入统一的工作区、聊天入口与能力控制面，让用户按任务选择执行后端，而不必重新搭建整个 Agent 环境。"
---

# 在 AIArb 里使用 Codex 和 Qoder：Agent OS 的跨 Harness 实践

AIArb 的起点，不是构建另一个聊天机器人，而是构建一个运行在用户自己环境中的 **Agent OS**。

模型决定 Agent 如何思考，但一个能够长期工作的 Agent，还需要自己的工作区、文件、会话、Skills、外部工具、权限边界和交互入口。AIArb 希望把这些部分组织成稳定、透明、可管理的运行环境，让 Agent 不只是存在于一次对话里，而是能够真正进入个人电脑和团队工作流。

但一个真正的 Agent OS，不应该只运行一种 Agent。

现在，除了 AIArb 原生 Agent 运行时，你还可以在 AIArb 中创建 **Codex** 或 **Qoder** Agent。它们继续使用各自原生的模型、推理循环、工具执行和权限机制，同时接入 AIArb 提供的 Agent 管理、工作区边界、聊天界面以及 Skills/MCP 能力控制面。

你既可以直接与这些第三方 Agent 对话，也可以让一个 AIArb Agent 主动调用它们：把代码实现交给 Codex，把独立复核交给 Qoder，再由 AIArb Agent 汇总结果并继续推进任务。
...
### 在 AIArb 中创建 Codex 或 Qoder Agent

在智能体管理页面创建 Agent 时，现在可以选择两类执行后端：

- **AIArb 原生智能体**：使用已配置的模型、Skills、工具、记忆和 AIArb Runtime；
- **第三方智能体**：使用已授权的外部 Agent Runtime，目前支持 Codex 和 Qoder。

选择第三方智能体后，AIArb 会检测本机可用的 Runtime。你可以使用自动检测结果，也可以手动指定 `codex`、`codex.exe`、`qodercli` 或 `qodercli.exe` 的完整路径。
...
保存后，这个 Codex 或 Qoder Agent 会和其他 AIArb Agent 一样出现在侧边栏中。你不需要打开另一个终端，也不需要离开 AIArb 的控制台。

### 在同一个聊天界面中使用不同 Harness

选择一个 Codex 或 Qoder Agent 后，就可以直接在 AIArb Chat 中发送任务。
...
### 让 AIArb Agent 指挥 Codex 和 Qoder

跨 Harness 还有另一种使用方式：不是用户手动切换 Agent，而是让一个 AIArb Agent 根据任务主动调用第三方 Harness Agent。

为 AIArb Agent 启用“多智能体协作”能力后，它可以先查询当前系统中可用的 Agent，再向目标 Agent 发起协作请求。目标 Agent 可以是另一个 AIArb 原生 Agent，也可以是以 Codex 或 Qoder 为执行后端的第三方 Agent。

例如，可以创建三个职责不同的 Agent：

| Agent      | 执行后端        | 职责                         |
| ---------- | --------------- | ---------------------------- |
| 项目负责人 | AIArb Runtime | 理解需求、拆分任务并汇总结果 |
...
整个过程都在 AIArb 内部完成。用户只需要在 Console Chat 中描述目标、查看进度并处理必要的审批，不需要进入终端。关键变化是：Codex 和 Qoder 不再只是两个可以手动切换的聊天对象，也可以成为 AIArb Agent 完成复杂任务时调度的专业执行单元。

### 按任务选择模型、推理强度和执行权限

不同 Harness 拥有自己的模型与执行选项。AIArb 不会把这些差异藏起来，而是在聊天工具栏中提供统一的设置位置。
...
AIArb 提供的是统一的管理入口，而不是用一套虚假的权限名称抹平所有差异。用户看到的仍然是每个 Harness 真正支持的能力。

### AIArb Skills 可以被第三方 Agent 继承

工作方式可以不同，但常用能力不应该每次重建。

对于 AIArb 管理的 Skills，系统会根据当前 Agent、工作区和频道计算本次会话真正启用的能力。当 Codex 或 Qoder 创建新会话时，这些 Skills 会通过 Harness 支持的原生方式注入。

例如，你可以在 AIArb 中启用一套团队代码审查 Skill，里面约定：
...
随后，无论选择 Codex 还是 Qoder，新会话都可以获得这套由 AIArb 管理的工作规范，而不需要分别维护两份 Skill。

Skills 页面会按管理权分组：

- **AIArb 管理**：可以创建、编辑、启停和删除，并在运行时投影到支持的第三方 Agent；
- **第三方智能体技能**：从当前 Harness 发现，只读展示名称、描述、来源和作用范围。

Provider 自己的 Skills 仍然属于 Provider。它们不会被自动复制到其他 Harness，也不会伪装成 AIArb 管理的能力。

![AIArb 管理与第三方 Agent Skills 分组](https://img.alicdn.com/imgextra/i4/O1CN016OT2kxnylGI7NCrU_!!6000000000211-2-tps-3632-1708.png)

_图 4：Skills 页面区分 AIArb 管理的可编辑能力与第三方 Agent 提供的只读 Skills。_

### MCP 配置也可以跟随 Agent 进入不同 Harness

MCP 同样由统一控制面管理。

在 AIArb 中配置并启用的 MCP 服务，可以在创建 Codex 或 Qoder 会话时转换为对应 Harness 的原生配置。用户不需要再手动编辑多份配置文件，也不需要把同一份凭据复制到多个全局目录。

同时，AIArb 会明确区分能力来源：

| MCP 来源       | 管理方式                   | 作用范围                   |
| -------------- | -------------------------- | -------------------------- |
| AIArb 管理   | 可编辑、可启停             | 可投影到支持的第三方 Agent |
| Codex 本地配置 | 只读发现                   | 仅 Codex                   |
| Qoder 本地配置 | 取决于 Provider 的发现能力 | 仅 Qoder                   |

AIArb 管理的 MCP 是可共享能力；Provider 本地 MCP 则是该 Provider 的私有能力。两者可以在同一个页面中展示，但不会被混为一谈。

![AIArb 管理与 Codex 本地 MCP 分组](https://img.alicdn.com/imgextra/i4/O1CN01qN3RSJOwRfE7Mhe4_!!6000000002101-2-tps-3628-1716.png)

_图 5：MCP 页面区分 AIArb 统一管理的配置与 Codex 本地只读 MCP，并明确各自的作用范围。_

### 选择 Agent，不再重建整个工作环境

跨 Harness 带来的直接变化，可以概括为下表：

| 能力                      | AIArb 原生 | Codex          | Qoder          |
| ------------------------- | ------------ | -------------- | -------------- |
| AIArb Chat              | 支持         | 支持           | 支持           |
...
| 工具调用展示              | 支持         | 支持           | 支持           |
| 会话恢复                  | 支持         | 支持           | 支持           |
| AIArb Skills            | 原生使用     | 运行时继承     | 运行时继承     |
| AIArb MCP               | 原生使用     | 运行时继承     | 运行时继承     |
| 执行权限                  | AIArb 治理 | Codex 原生策略 | Qoder 原生策略 |
| 被其他 AIArb Agent 调用 | 支持         | 支持           | 支持           |
...
AIArb Agent OS 管理的是 Agent 的运行环境。
...
它负责组织上下文、驱动模型、调用工具、处理审批、管理执行循环并输出事件。AIArb 原生 Runtime、Codex 和 Qoder 都可以承担这个角色，但它们拥有不同的模型接口、会话协议、工具事件和权限系统。

如果 Agent OS 与其中一种 Harness 永久绑定，它最终仍然只是一个拥有大量外围功能的 Agent 应用。要成为真正的运行环境，AIArb 需要将自己的资源和控制面与具体执行内核解耦。
...
flowchart TB
    ...
    subgraph BACKENDS["执行后端"]
        direction LR
        NATIVE["AIArb Runtime<br/>原生 Agent Loop"]
...
用户请求首先进入 AIArb 的统一入口，并被路由到目标 Agent 的 Workspace。Workspace 根据 Agent 配置判断应该使用 AIArb 原生 Runtime，还是交给第三方 Harness Runtime。
...
系统会根据目标 Agent ID 将协作请求发送给对应 Agent。目标 Workspace 加载自己的 Agent 配置：如果后端是 `aiarb`，请求进入原生 Runtime；如果后端是 `codex` 或 `qoder`，请求进入 Harness Runtime 和对应 Adapter。
...
这一层让 AIArb 的多智能体协作从“多个相同 Runtime 之间互相通信”，扩展为“一个 Agent OS 调度多种智能执行内核”。
...
Codex 与 Qoder 的流式事件协议不同。它们对文本、推理、工具调用、审批和错误都有自己的表达方式，但 AIArb Chat 不能为每种 Harness 重新实现一套消息系统。
...
AIArb 再把这些事件转换为现有的流式消息协议，发送到 Chat、桌面端或其他入口。

统一的不是 Agent 内部如何思考，而是 Agent OS 如何观察和呈现执行过程。

### 会话桥接：同时维护两套 Session 身份

第三方 Harness 通常拥有自己的 Session 或 Thread 标识，AIArb 也有用于聊天、历史记录和频道路由的 Session。

Harness Session Bridge 负责维护两者的关系：

- 为新的 AIArb Session 创建 Provider Session；
- 保存 AIArb Session 与 Provider Session 的映射；
- 重新打开聊天时恢复 Provider 历史；
- 在后续 Turn 中续接原来的 Provider Session；
- 将取消和审批路由到正确的执行实例；
- 隔离不同 Agent、Workspace 和 Session 的状态。

这使得用户可以从 AIArb 的会话列表重新进入一次 Codex 或 Qoder 对话，而不是每次都从一个全新的外部进程开始。
...
AIArb 继续作为 Skills 和 MCP 的统一控制面，但不会要求所有 Harness 使用完全相同的配置格式。
...
Codex app-server 支持设置额外的 Skill Roots。AIArb 可以直接把有效 Skill 目录交给 Codex：
...
MCP 则在启动 Codex app-server 时通过会话配置注入。只有由 AIArb 启动的 Codex 进程能够获得这些能力，用户独立打开 Codex 时不会看到 AIArb 的运行时注入。

#### Qoder：转换成受管理的本地 Plugin

Qoder 的 Skill 接口能够限制允许调用的名称，但不能直接指定任意 Skill 搜索目录。因此，AIArb 会根据当前能力生成一个受管理的本地 Plugin：
...
生成过程中使用文件复制而不是软链接，以保持 Windows、Linux 和 macOS 的一致性。随后，AIArb 通过 Qoder SDK 的 Plugin 与 Skill Allowlist 将能力注入会话。

AIArb MCP 则通过 Qoder Agent Options 传递给 Qoder SDK，同样不会写入用户的全局 Qoder 配置。
...
AIArb 原生 Runtime、Codex 和 Qoder 拥有不同的权限模型：

- AIArb 原生 Runtime 使用 Governance、Tool Guard 和 Sandbox；
- Codex 使用 Sandbox 与 Approval Policy 的组合；
- Qoder 使用自己的 Permission Mode。

这些模型无法在不损失语义的情况下被压缩成一个简单开关。因此，AIArb 采用两层设计：

1. 在统一位置展示和选择执行权限；
2. 将选择结果转换为 Harness 的原生设置。

如果某个 Harness 不支持特定策略粒度，AIArb 会通过 Capability 明确展示限制，而不会暗示策略已经完整生效。
...
跨 Harness 并不意味着 AIArb 接管 Provider 的所有配置。系统始终保留清晰的能力归属。
...
- Harness 后端选择和能力展示；
- 统一 Chat 入口与事件流；
- AIArb Session 与 Provider Session 的映射。
...
- AIArb 能力只注入由 AIArb 启动的 Provider 会话；
...
flowchart LR
    ...
    PROCESS["AIArb 启动的<br/>Codex / Qoder 会话"]
...
跨 Harness 最直观的结果，是用户可以在 AIArb 中使用 Codex 和 Qoder，并让 AIArb 管理的 Skills 与 MCP 跟随 Agent 进入新的执行环境。
...
- 能力控制面可以服务多种执行后端；
- AIArb Agent 可以按任务调用和调度不同 Harness；
...
AIArb 不需要重新实现每一种 Agent，也不希望定义世界上唯一正确的 Agent。它更希望提供一个稳定的操作系统，让不同 Agent 都能找到自己的运行位置。

> Agent 可以不同，工作方式可以不同，但工作环境不必每次重建。

这就是 AIArb 从 Agent 走向 Agent OS，也从一种 Runtime 走向多种 Harness 的下一步。
