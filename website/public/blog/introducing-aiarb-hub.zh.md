---
title: "Introducing AIArb Hub：为团队部署多租户 AIArb"
date: 2026-08-19
author: AIArb Team
tags: [AIArb Hub, 自托管, 多租户, Docker]
excerpt: "AIArb Hub 为团队自托管场景提供统一账户、独立运行环境、Local 与 Docker 后端，以及面向管理员的安全与运维控制。"
related:
  heading: "开始使用 AIArb Hub"
  description: "从首次初始化到运行环境和安全运维，完整文档集中在同一个入口。"
  items:
    - label: "文档"
      name: "AIArb Hub：部署与管理多租户 AIArb"
      href: "/docs/hub"
---

# Introducing AIArb Hub：为团队部署多租户 AIArb

AIArb 最初围绕个人设备设计：一个人安装一个 AIArb，配置自己的模型、记忆、Skills 和工作区。这种方式足够直接，也让数据和能力始终围绕个人展开。

但当一个团队希望在自己的服务器上提供 AIArb 时，问题就变了：谁可以登录？不同用户的文件和密钥放在哪里？一个人的进程异常会不会影响其他人？管理员停止实例后，用户访问页面会不会又把它自动拉起？Docker 镜像和资源上限又应该由谁决定？

AIArb Hub 就是对这些问题的回答。它是面向团队自托管场景的多租户 AIArb 平台：在统一入口后，为每个账户运行一个独立、可管理的 AIArb，而不是把同一个 AIArb 页面简单分享给所有人。
...
![AIArb Hub 登录页与管理中心概览](https://img.alicdn.com/imgextra/i3/O1CN01pmq9btaadNF6mRvU_!!6000000004218-2-tps-3340-1772.png)
...
Hub 负责账户、授权、运行环境生命周期和请求路由；每个用户看到的仍然是熟悉的 AIArb Console。对话、文件、模型配置和集成凭据属于个人运行环境，不会因为共用一个登录入口而混在一起。

这套模式也保留了原有单机产品。个人用户继续运行 `aiarb app` 即可，Hub 是一个明确、独立的多用户启动方式，不会改变本地 App 的启动和使用链路。
...
Local 后端复用宿主机的 AIArb 与 Python 环境，但不会在隔离能力缺失时直接启动普通进程：Linux 使用 Bubblewrap，macOS 使用 Seatbelt，Windows 使用 AppContainer 和 Job Object，并在启动前验证运行环境只能看到允许的路径。

Windows Local 要求 Windows 10 1507 或更高版本，并要求 Hub 以管理员权限运行。Hub 在运行环境存续期间为其 AppContainer SID 启用 Windows loopback exemption，由 AppContainer 主动建立反向 TCP 隧道，管理入口通过隧道代理其中的 AIArb，并在停止时撤销规则、关闭隧道。该规则同时意味着 Windows Local 可以访问宿主机 loopback，因此它不提供独立网络边界。管理员权限、文件 ACL、进程树约束、规则配置、隧道连接或真实运行环境健康检查任一失败，运行环境都会拒绝启动。
...
AIArb Hub 是自托管软件，不是由 AIArb 团队代为运营的 SaaS。实例运营者控制服务器、数据库、备份和进程，也可能接触用户在该实例中保存的数据。因此，用户只应登录本人或可信组织部署的 Hub。

每个 AIArb 都运行在受约束的环境中，文件、进程、设备和网络能力可能不如个人电脑上的完整版本。Linux、macOS 和 Windows 的 Local 进程沙箱共享各自的宿主机内核，Docker 容器共享 Docker Engine 使用的 Linux 内核。它们能够降低账户之间相互影响的风险，但不为每个用户提供独立内核。
...
如果你准备在自己的服务器上运行 AIArb Hub，可以阅读 [AIArb Hub 文档](/docs/hub)，完成管理员初始化，并继续配置运行环境、Docker、安全、OAuth、备份和监控。

个人用户无需迁移：`aiarb app` 仍然是原来的单机 AIArb。只有当你确实需要为多个人提供统一、自托管的入口时，AIArb Hub 才会出现。
