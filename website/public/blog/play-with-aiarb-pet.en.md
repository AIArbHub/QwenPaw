---
title: "Play with AIArb-Pet"
date: 2026-07-01
tags: [Plugin, DesktopPet, PixelPets, SkillCreation]
excerpt: "AIArb introduced the plugin system in version 1.1.3 and the pet system in version 1.1.8. I was not a big fan of the official pet templates, so I had never configured them. Personally, I prefer pixel-style pet designs."
related:
  ...
  items:
    ...
    - label: "Built-in Skill"
      name: "make-skill"
      href: "https://aiarb.cn/docs/skills#Create-from-current-session-via-make-skill-Beta"
...
AIArb introduced the plugin system in version 1.1.3 and the pet system in version 1.1.8. I was not a big fan of the official pet templates, so I had never configured them. Personally, I prefer pixel-style pet designs.

This time I spent some time letting AIArb generate a batch of pet templates by itself. Using `make-skill` and `subagents`, the pet generation process can be autonomously distilled, summarized, and reused, which significantly accelerates the workflow.

## TL;DR Workflow

- AIArb version: v1.12

  - Note: The pet generation workflow in AIArb 2.0 may differ due to plugin system updates.
...
In the official plugin store, download the AIArb Pet plugin. ![image](https://img.alicdn.com/imgextra/i1/O1CN01niHx461stRq372qrr_!!6000000005824-2-tps-2335-330.png)
...
- QA-source-index: Helps AIArb understand how its own pet system is designed.

  - You can also use the `aiarb-docs-zh` skill from the AIArb skill marketplace.
...
Start a new chat to help AIArb understand its own pet system. You can see that after asking questions, AIArb invokes the **QA-source-index** skill and starts understanding the pet plugin system.
...
After the first round of conversation, AIArb understands its pet mechanism. Now we can ask it to create a pet.
...
See the image below. AIArb completed the task in one shot, but there were still a few minor issues we could continue to tune:
...
In version 1.11, AIArb introduced `make-skill`, which can distill user dialogue history and tool invocations into a reusable skill. With this capability, we turned pet creation into a reusable skill that includes my personalized preferences and habits, as well as reusable execution scripts.
...
This article demonstrates how to use AIArb to create pets, and how to leverage Make-skill to distill the workflow so that large-scale generation of pixel-style Pokemon pets becomes possible afterward. Going forward, the author plans to continue with:

- Adapting to the 2.0 pet system.
- Using AIArb to self-update pet interactions and enrich pet system capabilities.
