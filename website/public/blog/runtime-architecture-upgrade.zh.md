---
title: "AIArb Runtime 架构升级解析：架构、实现与扩展"
date: 2026-07-07
author: AIArb Team
tags: [架构升级, 八阶段编排, Hook机制, AIArb2.0]
cover: /blog/runtime-architecture-upgrade-cover.png
excerpt: "AIArb Runtime 从 650 行 god method 重构为 8 阶段编排引擎，通过 Hook、AgentMode 与声明式注册实现「一个功能一个目录、Runtime 零侵入扩展」。"
---

# AIArb Runtime 架构升级解析：架构、实现与扩展
...
在 Runtime 架构升级之前，AIArb 的请求处理集中在一个 `AgentRunner.query_handler()` 方法中——这是一个超过 **650 行的 god method**。每个新功能（任务模式、编程模式、计划模式、定时任务隔离、Skill 注入……）都需要**侵入式修改**这个核心函数，不断增加 if/else 分支和交叉的状态管理。
...
`Runtime` 类（`src/aiarb/runtime/runtime.py`）是 per-workspace 的请求编排器。每个请求调用 `run()`，通过 8 阶段生命周期产出 SSE 信封对象。
...
定义在 `src/aiarb/runtime/phases.py` 中：
...
每个 hook 返回 `HookResult`，包含三种动作之一（`src/aiarb/runtime/hooks.py`）：
...
`AgentBuilder`（`src/aiarb/runtime/builder.py`）负责为每个请求构建一个完整的 `AIArbAgent`，它整合了：
...
`AgentExecutor`（`src/aiarb/runtime/executor.py`）驱动 agent 的 reply stream，并包装心跳机制——确保在长时间空闲期间（如 tool-guard 审批等待），发出 keep-alive 信封而不是让 SSE 连接断开。

`Envelope`（`src/aiarb/runtime/envelope.py`）是 SSE 状态机，负责将 agentscope 的 `EventType` 事件翻译为前端的流式信封协议，跟踪每个请求的状态（文本块、推理块、工具调用、数据块）并产出正确的事件序列。
...
`HookRegistry`（`src/aiarb/runtime/hooks.py`）管理 hook 的注册和执行：
...
`AgentMode`（`src/aiarb/modes/base.py`）是一个**行为包**：命令、工具、hook 和 prompt contributor 打包在一起。
...
所有 per-workspace 的注册表都位于 `WorkspacePlugins`（`src/aiarb/app/workspace/workspace_plugins.py`）：
...
之前 system prompt 构建分散在 mixin、runner 和 env 模块中，新增一个 prompt 片段需要找到正确的注入点。现在，`PromptManager`（`src/aiarb/runtime/prompt_manager.py`）从有序的 `PromptContributor` 实例中声明式组装 system prompt：
...
之前的痛点是 tool 硬编码在 agent 类中，每次加 tool 必须侵入 agent 类 ~80 行。`ToolRegistry`（`src/aiarb/runtime/tool_registry.py`）用声明式、可过滤的注册表替代了旧的硬编码 tool 字典。
...
之前 AIArb 有**四套并行命令机制**：conversation、control、daemon 和 skill，每个方法手写 3 遍。`SlashCommandRegistry`（`src/aiarb/runtime/slash_command_registry.py`）将它们统一为一个分发点。
...
对于之前列出的痛点 "对外接口三套重复"，`@api_action`（`src/aiarb/api_action.py`）从根本上解决了这个问题：**一个装饰器，同时生成 HTTP API、CLI 子命令和 slash 命令**。
...
`ToolCoordinator`（`src/aiarb/tool_calls/`）提供了**单 tool call 粒度**的控制。旧系统中 `/stop` 只能杀掉整个 agent。现在可以追踪、取消或后台化单个 tool 调用。
...
`ServiceManager`（`src/aiarb/app/workspace/service_manager.py`）管理 per-agent 服务，支持：
...
假设你是一个开发者，想在 AIArb 中新增一个 Mode。你**不需要修改 Runtime 任何一行代码**。
