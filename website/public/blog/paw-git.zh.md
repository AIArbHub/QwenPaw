---
title: "PawGit：为 AIArb Agent 会话状态提供可恢复性"
date: 2026-07-07
author: AIArb Team
...
excerpt: "长会话里 Agent 会被脏上下文拖住。PawGit 为 AIArb Agent 会话状态提供 checkpoint、timeline 与 rewind，让你在不必新开窗口重喂 prompt 的前提下，回到之前还干净的状态。"
related:
  heading: "相关插件：PawGit"
  description: "AIArb 会话与记忆版本控制插件，支持状态快照、DAG 时间线、会话回滚与状态重置。"
  linkText: "查看并安装 PawGit →"
  linkUrl: "https://platform.agentscope.io/plugins/pawgit"
---

# PawGit：为 AIArb Agent 会话状态提供可恢复性

如果你经常用 AIArb 做复杂任务，应该很熟悉这种时刻：
...
PawGit 的切入点就在这里，它不是项目 Git 的替代品，它想做的是另一件事：**给 AIArb 的 Agent 会话状态加上存档点**。

### 2. PawGit 如何保存 Agent 会话状态

PawGit 是一个 AIArb 插件，目前可以在官方 platform 下载：[https://platform.agentscope.io/plugins/pawgit](https://platform.agentscope.io/plugins/pawgit)。
...
有了 PawGit，现在用 AIArb 做稍微有风险的 Agent 任务前，我会先打一条命名快照。
...
接下来我可以放心让 AIArb 去读代码、比较方案、尝试实现，而不用担心一旦聊偏，就只能在一团脏上下文里继续硬撑。命名快照会进入 PawGit 的永久快照区域，它不会被普通 GC 清掉：
...
看到这里，你可能会觉得：怎么都需要手动输入 command，好麻烦。别着急，PawGit 也提供了 skill 和 tool。除了手动输入这些 slash command，PawGit 也配备了相关的 skill 和 tool，可以在技能和工具页面查看。目前针对 AIArb 2.0 的版本正在开发；在 1.0 版本中，skill 的名称为 `pawgit`。
