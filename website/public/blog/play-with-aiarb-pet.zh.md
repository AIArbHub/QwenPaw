...
excerpt: "AIArb在1.1.3版本引入了插件系统，并在1.1.8版本引入了宠物系统。但官方的宠物模板个人不太喜欢，就一直没有配置。个人一直比较喜欢像素风的宠物类型"
related:
  heading: "相关能力"
  items:
    - label: "插件"
    ...
    - label: "内置技能"
      name: "make-skill"
      href: "https://aiarb.cn/docs/skills#通过-make-skill-从当前会话创建-Beta"
      description: "将当前会话中的操作流程与经验沉淀为可复用 Skill。本文使用该能力将桌宠制作流程封装为可复用 Skill。"
---

# Play with AIArb-Pet

AIArb在1.1.3版本引入了插件系统，并在1.1.8版本引入了宠物系统。但官方的宠物模板个人不太喜欢，就一直没有配置。个人一直比较喜欢像素风的宠物类型。

这次抽时间尝试让AIArb自己生产了一批宠物模板，使用make-skill和subagents，生成宠物的过程可以自主沉淀总结归纳，显著加速流程。

## 太长不看版流程：

- AIArb版本：v1.12

  - Note：AIArb 2.0版本的宠物生产流程，因插件系统更新可能有所不同
...
在官方插件处，下载AIArb Pet插件。![image](https://img.alicdn.com/imgextra/i1/O1CN01niHx461stRq372qrr_!!6000000005824-2-tps-2335-330.png)
...
- QA-source-index：便于AIArb了解自己的宠物系统如何设计

  - 也可以用AIArb skill市场的`aiarb-docs-zh`技能
...
开启一个新会话，让AIArb了解自己的宠物系统。可以看到发出问题后，AIArb调用**QA-source-index**技能，开始理解宠物插件系统。
...
经过第一轮对话，AIArb了解了自己的宠物机制，下面就可以让它创建宠物了！
...
见下图。可以看到AIArb一次就完成了任务，但仍然有一些小问题，我们可以继续微调。
...
在版本1.11中，AIArb引入了`make-skill`，可以沉淀用户对话历史和工具调用为skill。借助该技能，我们将宠物创建变成可复用skill，该技能包含我的个性化偏好和习惯，同时也有可复用的执行脚本。
...
本文展示如何使用AIArb进行Pet创建，借助Make-skill技能沉淀流程，为之后批量生产像素风宝可梦宠物提供支持。再之后作者将继续考虑：

- 适配2.0版本宠物系统
- 使用AIArb，自更新宠物交互，丰富宠物系统功能
