                    ...
                    <ReactMarkdown
                      ...
                      components={{
                        ...
                        code: ({ className, children, ...props }) => {
                          ...
                          if (langCode === "mermaid") {
                            // Children may be array of strings; join to preserve newlines
                            let chart = Array.isArray(children)
                              ? children.join("")
                              : String(children);
                            chart = chart.replace(/\n$/, "");

                            // Replace encoded or literal <br/> with actual newlines
                            chart = chart.replace(/&lt;br\s*\/?&gt;/gi, "\n");
                            chart = chart.replace(/<br\s*\/?\>/gi, "\n");

                            // Decode other HTML entities safely
                            try {
                              const ta = document.createElement("textarea");
                              ta.innerHTML = chart;
                              chart = ta.value ?? chart;
                            } catch (e) {
                              // ignore and use original chart
                            }

                            // Normalize some mermaid edge label patterns that can
                            // confuse the parser when labels are placed outside
