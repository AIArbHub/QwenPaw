...
$desktopExe = Join-Path $PSScriptRoot "aiarb-desktop.exe"
$stdoutLog = Join-Path $env:TEMP "aiarb-desktop-debug.stdout.log"
$stderrLog = Join-Path $env:TEMP "aiarb-desktop-debug.stderr.log"
...
$desktop = Get-Process -Name "aiarb-desktop" -ErrorAction SilentlyContinue |
  Where-Object { $_.Path -eq $desktopExe } |
  Select-Object -First 1
if ($desktop) {
  Write-Host "Existing AIArb Desktop process detected; quit it from the tray before reproducing startup issues."
  $desktop = $null
} else {
...
$specs = @(
  [pscustomobject]@{ Label = "backend"; Paths = $env:AIARB_BACKEND_LOGS -split ";" },
  [pscustomobject]@{ Label = "shell"; Paths = $env:AIARB_SHELL_LOGS -split ";" }
)
$states = @()
  ...
  while ($true) {
    ...
    if ($desktop) {
      ...
      if ($desktop.HasExited) {
        Write-Host "AIArb Desktop exited with code $($desktop.ExitCode)."
        Show-DesktopOutput
