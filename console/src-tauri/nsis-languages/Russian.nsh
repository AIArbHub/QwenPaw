...
LangString qwenpawCliPathPageDescription ${LANG_RUSSIAN} "This adds the bundled qwenpaw command to your user PATH. You can change it later by reinstalling or uninstalling AI Arb Desktop."
...
LangString qwenpawStopBackendPrompt ${LANG_RUSSIAN} "AI Arb  всё ещё запущен и использует файлы, которые установщику нужно заменить.$\n$\nЗакройте AI Arb  (проверьте системный трей), затем нажмите «Повторить», чтобы продолжить. Нажмите «Отмена», чтобы прервать установку."
