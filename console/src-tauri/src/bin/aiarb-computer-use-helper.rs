...
fn main() {
    eprintln!("aiarb-computer-use-helper is only supported on Windows and macOS");
    std::process::exit(2);
}
...
fn main() {
    ...
    if !args.first().is_some_and(|value| value == "serve") {
        eprintln!(
            "usage: aiarb-computer-use-helper serve --pipe <endpoint> \
             (capability via AIARB_CU_CAPABILITY)",
        );
        std::process::exit(2);
    }
    #[cfg(target_os = "macos")]
