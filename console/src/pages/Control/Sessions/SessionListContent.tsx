...
interface SessionListContentProps {
  /** Agent ID — the underlying useSessions hook already reads from agentStore,
   *  this prop is kept for API compatibility and future per-agent filtering. */
  agentId: string;
  /** When true, render the filter bar + batch action toolbar. */
  showHeader?: boolean;
}

/**
 * SessionListContent — extracted from SessionsPage.
 *
 * Contains: optional filter bar, active/archived Tabs, Table (desktop) / Card
 * list (mobile), and the edit SessionDrawer.
 *
 * Used by:
 *  - SessionsPage (showHeader = true)
 *  - WorkbenchPage "对话日志" tab (showHeader = false)
 */
