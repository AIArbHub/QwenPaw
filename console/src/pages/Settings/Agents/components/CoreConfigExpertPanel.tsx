/**
 * CoreConfigExpertPanel — Expert-mode editor for agent core config files.
 *
 * Displays the list of core configuration files (SOUL.md, PROFILE.md, MEMORY.md,
 * etc.) with drag-to-reorder, enable/disable toggles for system prompt inclusion,
 * and a side-by-side file editor. Used inside AgentDetailDrawer's persona tab
 * when the user switches to "expert" mode.
 */
...
export const CoreConfigExpertPanel: React.FC<CoreConfigExpertPanelProps> = ({
  ...
  // Split files: enabled (draggable) and disabled (static)
  const enabledFileObjs = files.filter((f) => enabledFiles.includes(f.filename));
  const disabledFileObjs = files.filter((f) => !enabledFiles.includes(f.filename));
    ...
    <Spin spinning={loading}>
      <div style={{ display: "flex", gap: 16, minHeight: 400 }}>
        ...
        <div style={{ width: 280, flexShrink: 0 }}>
          ...
          {/* Enabled files (draggable, reorderable) */}
          <DndContext
            sensors={sensors}
          ...
          {/* Disabled files (static, can enable) */}
            ...
            <>
              ...
              >
                ...
                <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                  <span style={{ fontSize: 12, color: hasChanges ? "#faad14" : "#52c41a" }}>
