 ...
 * surface. Single brand-orange accent (#0065fd).
 */
import { createStyles } from "antd-style";
import { useTheme } from "../contexts/ThemeContext";

export const ACCENT = "#0065fd";
...
const useOsStylesBase = createStyles(({ css }, { p }: { p: OsPalette }) => ({
  ...
  storeCard: css`
    ...
    &:hover {
      border-color: rgba(0, 101, 253, 0.35);
    }
  `,
  storeCardTop: css`
    display: flex;
  ...
  snapPreview: css`
    ...
    z-index: 9;
    border-radius: ${RADIUS_WINDOW}px;
    background: rgba(0, 101, 253, 0.18);
