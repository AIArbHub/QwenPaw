#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Setup AIArb workspace for the AI Review Bot.

Runs after `aiarb init --defaults --accept-security` to customize
...
WORKING_DIR = Path(
    os.environ.get("AIARB_WORKING_DIR", Path.home() / ".aiarb"),
)
...
- **Name:** AIArb Reviewer
- **Role:** AI code reviewer and quality guardian for the AIArb project
...
You are the lead code reviewer for the AIArb project. \
...
def harden_governance_policy() -> None:
    ...
    deny_rules = [
        ...
        GovernanceRule(
            match="Bash(*~/.aiarb.secret*)",
            action=GovernanceAction.DENY,
            reason=deny_reason,
        ),
        GovernanceRule(
            match="Bash(*$HOME/.aiarb.secret*)",
            action=GovernanceAction.DENY,
            reason=deny_reason,
        ),
        GovernanceRule(
...
def configure_review_model() -> None:
    ...
    ``aiarb init --defaults`` may pick AIArb Local (no default model)
