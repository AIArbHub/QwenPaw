# -*- coding: utf-8 -*-
"""Cleanup script: removes all AIArb sandbox profiles, ACLs, users, and state.
...
This script performs cleanup for BOTH sandbox backends:
  ...
  A. AppContainer sandboxes (allow_read_all=False):
     For each container metadata file in ~/.aiarb/containers/*.json:
  ...
  B. Restricted-token sandboxes (allow_read_all=True):
     For each sandbox metadata file in ~/.aiarb/sandboxes/*.json:
  ...
  After all entries are processed:
     - Removes any remaining NTFS junctions in ~/.aiarb/junctions/
     - Removes the AiarbUsers local group (if empty)
...
def _remove_python_dir_acl_marker() -> None:
    """Removes the .aiarb_acl_granted marker from the Python install directory.

    This marker is written by _ensure_python_dir_group_acl() after granting
    RX to AiarbUsers on the Python dir. When cleanup removes the group
...
def _cleanup_sandbox_group() -> None:
    """Removes the AiarbUsers local group if it exists."""
    ...
    # Remove the .aiarb_acl_granted marker from Python directory.
    # Without this, the next sandbox creation would see the stale marker
    # and skip re-granting the ACL to the (re-created) AiarbUsers group.
...
def _cleanup_state_dirs(state_dir: Path) -> None:
    ...
    # Remove any remaining files in .aiarb (stray files, logs, etc.)
    if state_dir.is_dir():
