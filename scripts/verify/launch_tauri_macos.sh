...
unzip -q dist/AIArb-Tauri-*-macOS.zip -d dist/verify-tauri
...
ps -ef | grep -iE "aiarb|tauri" | grep -v grep || echo "  (no matching processes)"
...
PORT_FILE="$HOME/.aiarb/desktop_port"
PORT=""
...
rm -f "$HOME/.aiarb/workspaces/default/BOOTSTRAP.md"
