...
"""
PyInstaller spec file for AIArb Desktop (Tauri sidecar).
...
bundle also includes a aiarb CLI executable for the Windows installer PATH
option.
"""
...
MAIL_MCP_SRC = REPO_ROOT / "packages" / "aiarbmail-mcp" / "src"
...
# from aiarb/console, so Tauri can navigate to the backend-hosted same-origin
...
_data_dirs = [
    ...
    ("app/channels/yuanbao/proto", "aiarb/app/channels/yuanbao/proto"),
    ("providers/data", "aiarb/providers/data"),
]
datas = [
...
datas.append(
    (
        ...
        "aiarb/browser/control_link/injected",
    ),
)
...
a = Analysis(
    ...
    hiddenimports=[
        ...
        *collect_submodules("aiarbmail_mcp"),
        ...
        *collect_submodules("aiarb.agents.acp"),
        # PawApp SDK modules are imported by installed app plugins at runtime.
        *collect_submodules("aiarb.pawapp"),
        # ASGI app entry points
        "aiarb.app._app",
        "aiarb.app.multi_agent_manager",
        "aiarb.app.chats",
        "aiarb.app.task_tracker",
        "aiarb.runtime.commands",
        # Backup modules are exposed through aiarb.backup.__getattr__, which
