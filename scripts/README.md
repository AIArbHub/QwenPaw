...
- Builds the console frontend (`console/`), copies `console/dist` to `src/aiarb/console/dist`, then builds the wheel. Output: `dist/*.whl`.
...
- Default tag: `aiarb:latest`. Uses `deploy/Dockerfile` (multi-stage: builds console then Python app).
- Example: `bash scripts/docker_build.sh myreg/aiarb:v1 --no-cache`.
