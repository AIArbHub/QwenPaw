...
$LauncherBat = Join-Path $EnvRoot "AIArb Desktop.bat"
@"
@echo off
cd /d "%~dp0"
...
$DebugBat = Join-Path $EnvRoot "AIArb Desktop (Debug).bat"
@"
@echo off
cd /d "%~dp0"
...
echo [Launch] Starting AIArb Desktop with log-level=%AIARB_LOG_LEVEL%...
echo Press Ctrl+C to stop
echo.
"%~dp0python.exe" -u -m aiarb desktop --log-level %AIARB_LOG_LEVEL%
echo.
echo [Exit] AIArb Desktop closed
pause
"@ | Set-Content -Path $DebugBat -Encoding ASCII

# VBScript launcher (no console window)
$LauncherVbs = Join-Path $EnvRoot "AIArb Desktop.vbs"
@"
Set WshShell = CreateObject("WScript.Shell")
batPath = CreateObject("Scripting.FileSystemObject").GetParentFolderName(WScript.ScriptFullName) & "\AIArb Desktop.bat"
