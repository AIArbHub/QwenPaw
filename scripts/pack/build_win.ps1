...
$Archive = Join-Path $Dist "aiarb-env.zip"
...
$VersionFile = Join-Path $RepoRoot "src\aiarb\__version__.py"
$CurrentVersion = ""
...
$RunWheelBuild = $true
if ($CurrentVersion) {
  $wheelGlob = Join-Path $Dist "aiarb-$CurrentVersion-*.whl"
  ...
  if ($existingWheels.Count -gt 0) {
    ...
    $oldWheels = Get-ChildItem -Path (Join-Path $Dist "aiarb-*.whl") -ErrorAction SilentlyContinue
...
$LauncherBat = Join-Path $EnvRoot "AI Arb Desktop.bat"
@"
@echo off
cd /d "%~dp0"
...
REM Log level: env var AIARB_LOG_LEVEL or default to "info"
if not defined AIARB_LOG_LEVEL set "AIARB_LOG_LEVEL=info"

REM Set SSL certificate paths for packaged environment
REM Use temp file to avoid for /f blocking issue in bat scripts
set "CERT_TMP=%TEMP%\aiarb_cert_%RANDOM%.txt"
...
if not exist "%USERPROFILE%\.aiarb\config.json" (
  "%~dp0python.exe" -u -m aiarb init --defaults --accept-security
)
"%~dp0python.exe" -u -m aiarb desktop --log-level %AIARB_LOG_LEVEL%
"@ | Set-Content -Path $LauncherBat -Encoding ASCII

# Debug launcher .bat (shows console)
$DebugBat = Join-Path $EnvRoot "AI Arb Desktop (Debug).bat"
@"
@echo off
cd /d "%~dp0"
...
REM Debug mode: use debug log level by default (can override with AIARB_LOG_LEVEL)
...
echo ====================================
echo AI Arb Desktop - Debug Mode
echo ====================================
...
echo [Launch] Starting AI Arb Desktop with log-level=%AIARB_LOG_LEVEL%...
echo Press Ctrl+C to stop
echo.
"%~dp0python.exe" -u -m aiarb desktop --log-level %AIARB_LOG_LEVEL%
echo.
echo [Exit] AI Arb Desktop closed
pause
"@ | Set-Content -Path $DebugBat -Encoding ASCII

# VBScript launcher (no console window)
$LauncherVbs = Join-Path $EnvRoot "AI Arb Desktop.vbs"
@"
Set WshShell = CreateObject("WScript.Shell")
batPath = CreateObject("Scripting.FileSystemObject").GetParentFolderName(WScript.ScriptFullName) & "\AI Arb Desktop.bat"
WshShell.Run Chr(34) & batPath & Chr(34), 0, False
Set WshShell = Nothing
"@ | Set-Content -Path $LauncherVbs -Encoding ASCII

# Create aiarb.cmd wrapper in env root so "aiarb" resolves to this
# instead of Scripts\aiarb.exe whose embedded Python path may be stale
# after conda-pack/unpack.
$AiarbCmd = Join-Path $EnvRoot "aiarb.cmd"
@"
@"%~dp0python.exe" -u -m aiarb %*
"@ | Set-Content -Path $AiarbCmd -Encoding ASCII
...
if (-not $Version) {
  ...
  try {
    $Version = (& (Join-Path $EnvRoot "python.exe") -c "from importlib.metadata import version; print(version('aiarb'))" 2>&1) -replace '\s+$', ''
...
Write-Host "[build_win] AIARB_VERSION=$Version OUTPUT_EXE will be under $Dist"
$OutInstaller = Join-Path (Join-Path $RepoRoot $Dist) "AIArb-Setup-$Version.exe"
